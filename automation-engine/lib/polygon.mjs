...
function sweepAndCollect({ cardFile, cardName, envelopeMhz, fromMhz, toMhz, maxDepthMv, timeoutMs }) {
  ...
  const run = spawnSync(process.execPath, [
    ...
    // ⚠️ СМЕРТЬ КАРТЫ ВЗВЕДЕНА, И ЭТО НЕ УКРАШЕНИЕ — ЗАМЕР 2026-08-29 21:0x. Невзведённая ступень
    // зависания возвращает `ETIMEDOUT`, слой вердикта честно называет это CRASH, спуск считает CRASH
    // законной стеной и идёт дальше: у P1 граница легла РОВНО на 1000 мВ (1010 ПРОШЛО, 1000 CRASH при
    // крае карты 871 мВ), то есть ловушка срабатывала, а зависания в журнале не было ни одного.
    // Без этого флага сторож И3 нечем покраснить в принципе, и весь полигон судит на одну причину
    // меньше, чем думает.
    //
    // Здесь это ЗАКОННО ровно потому, что движок поднят ОТДЕЛЬНЫМ процессом: умирает ребёнок, а
    // отчитывается родитель — та же форма, которой `watchdog --drill` доказывает открепление.
    // Ставится в ОБЩЕЙ части обеих дорог намеренно: у сгенерированной карты `hangAtOrBelowMv` не
    // бывает вовсе (генератор такого поля не пишет), поэтому флаг для неё — ноль изменений, а две
...
export function reportLines(batch) {
  ...
  // ⚠️ СЕМЕНА И АМПЛИТУДА — ЧАСТЬ СТРОКИ ПРОИСХОЖДЕНИЯ, А НЕ УКРАШЕНИЕ (E67-AC6 дословно: «каждый
  // файл сгенерированной карты И КАЖДЫЙ ПОЛИГОН-ОТЧЁТ несут строку „СГЕНЕРИРОВАНО семенем N,
  // амплитуда A — вымысел²…“»). Файл карты нёс их с фазы 3, отчёт — нет: строка была общей и
  // потому невоспроизводимой. Отчёт без семян — это утверждение, которое нельзя перепроверить.
  L.push(`СГЕНЕРИРОВАНО семенами ${batch.seedBase}…${batch.seedBase + batch.count - 1}, `
    + `амплитуда ${batch.amplitude} — ВЫМЫСЕЛ²: этих карт не существует и как замера. Зелёный полигон — `
    + 'утверждение о ЛОГИКЕ ДВИЖКА на широком поле поведений, и НИ О ЧЁМ БОЛЬШЕ: ни о кремнии '
    + 'владельца, ни о том, что найденный край верен — правильного края у вымысла нет.');
  return L;
}
...
function cmdSelftest() {
  ...
  // ⚠️ СУДИТСЯ ПОЛНАЯ ФОРМА, А НЕ КОРОТКАЯ ПОДСТРОКА (риск 3 `plans/72`): «ВЫМЫСЕЛ²» само по себе
  // зазеленело бы на любой чужой строке, где это слово упомянуто. E67-AC6 требует строку С СЕМЕНАМИ
  // И АМПЛИТУДОЙ — то есть воспроизводимую, — поэтому блок требует их ОДНОЙ строкой вместе с
  // границей утверждения. Отчёт без семян есть утверждение, которое нельзя перепроверить.
  ok('отчёт несёт строку происхождения ЦЕЛИКОМ: семена · амплитуда · вымысел² · граница (E67-AC6)',
    rep.some((l) => /СГЕНЕРИРОВАНО семенами 1…2, амплитуда 0\.7 — ВЫМЫСЕЛ²/u.test(l)
      && l.includes('правильного края у вымысла нет')));
