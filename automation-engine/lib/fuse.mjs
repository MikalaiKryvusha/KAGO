...
/**
 * 🔴 `bugs/131` — РОДОСЛОВНАЯ ПОРОГА ВХОДА 3, И ОНА СЕГОДНЯ ПУСТА.
 *
 * Число выше выведено ПО ДРУГОЙ ВЕЛИЧИНЕ, чем та, которой судит судья: по архиву `nvidia-smi`,
 * где мощность усреднена внутри прожига и снимается раз в секунду, а судью кормят МГНОВЕННЫМ
 * значением каждые 2 мс (`bugs/125`, замер отменён). Попытка вывести порог заново из колец самого
 * судьи (`plans/94` Ш6, прибор `npm run ratiofloor`, шесть колец) дала ОТРИЦАТЕЛЬНЫЙ ответ:
 *
 *   просадка доли    смерть 17:53: 0,21 за 266 мс │ здоровье: 0,21-0,24 за 230-364 мс
 *   площадка мощности смерть 17:53: 2060 мс       │ ВЫЖИВШИЙ прогон 09:59: 1971 мс
 *
 * Ни одна из двух величин смерть от здоровья не отделяет. То есть у порога нет родословной НИ В
 * СТАРОЙ величине (отменена), НИ В НОВОЙ (не выводится).
 *
 * ПОЧЕМУ ЭТО КОД, А НЕ СТРОКА В STATUS. Взведённый вход, который не обнаруживает, но способен
 * сработать, — это чистый вычет: он может оборвать работу владельца, не давая взамен ничего. Канон
 * проекта уже говорит «ВЗВЕДЁН ≠ ДОКАЗАН» и «невзведённый вход не трипает никогда, а „отсутствует“
 * не читается как „в порядке“». Здесь это правило становится исполнимым: решение о взведении
 * выводится ИЗ РОДОСЛОВНОЙ, а не принимается заново каждым, кто пишет строку запуска.
 *
 * САМОСНИМАЮЩИЙСЯ ПО ПОСТРОЕНИЮ: появится замер, отделяющий смерть от здоровья, — сюда впишут его
 * метку, и вход взведётся сам. Править код будет не нужно, забыть будет нечего.
 */
export const POWER_COLLAPSE_PROVENANCE = null;

/**
 * ВЗВОДИТСЯ ЛИ ВХОД 3 — форма та же, что у входа 2 (`armMDecision`), и по той же причине: решение
 * с ПРИЧИНОЙ, которую видно в журнале прогона, а не молчаливое число в строке запуска.
 *
 * @returns {{ armed: boolean, armPowerRatio: number|null, why: string }}
 */
export function armPowerDecision({ provenance = POWER_COLLAPSE_PROVENANCE, ratio = POWER_COLLAPSE_RATIO } = {}) {
  if (provenance === null) {
    return {
      armed: false,
      armPowerRatio: null,
      why: 'вход 3 (обвал мощности) НЕ ВЗВЕДЁН: у порога нет родословной. Старый замер отменён '
        + '(bugs/125 — доля считалась по секундному среднему, судья судит мгновенным), новый не '
        + 'выводится (plans/94 Ш6: смерть 0,21/266 мс неотличима от здоровья 0,21-0,24/230-364 мс, '
        + 'а площадка замершей мощности у выжившего прогона 1971 мс против 2060 у смерти). '
        + 'НАБЛЮДЕНИЕ идёт: доля пишется в кольцо каждый такт и копится на будущий вывод.',
    };
  }
  return {
    armed: true,
    armPowerRatio: ratio,
    why: `вход 3 взведён: доля ${ratio} от бегущего пика · родословная ${provenance}`,
  };
}
...
async function cmdSelftest() {
  ...
  // ---- ⚡ `bugs/131`: ВХОД 3 ВЗВОДИТСЯ ПО РОДОСЛОВНОЙ, А НЕ ПО КОНСТАНТЕ.
  //   АДРЕСАТЫ МУТАЦИЙ: вернуть `armPowerRatio: POWER_COLLAPSE_RATIO` в строку запуска →
  //   «взведён при пустой родословной»; снять ветку `provenance === null` → то же самое.
  ok('🔴 при ПУСТОЙ родословной вход 3 НЕ взводится, и причина названа числами замера', (() => {
    const d = armPowerDecision();
    return d.armed === false && d.armPowerRatio === null
      && /bugs\/125/u.test(d.why) && /0,21/u.test(d.why) && /НАБЛЮДЕНИЕ идёт/u.test(d.why);
  })());
  ok('появится родословная — вход 3 взведётся САМ, правки кода не потребуется', (() => {
    const d = armPowerDecision({ provenance: 'замер 2026-10-01, N смертей', ratio: 0.33 });
    return d.armed === true && d.armPowerRatio === 0.33 && /родословная замер 2026-10-01/u.test(d.why);
  })());
  ok('невзведённый вход 3 НЕ трипает ни при какой глубине провала — «отсутствует» ≠ «в порядке»', (() => {
    const v = judgeLiveness({
      nowMs: 10_000, lastBeatMs: 9_999, armNMs: 60,
      power: { mw: 1000, peakMw: 300_000, ratio: null, establishedMw: POWER_ESTABLISHED_MW, lowForMs: 99_999, holdMs: POWER_LOW_HOLD_MS },
    });
    return v.tripped === false && v.powerRatio !== null;
  })());
