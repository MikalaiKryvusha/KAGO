...
export function harvestPairs(rungs) {
  ...
  const halfPairs = [];
  const wastedBurns = [];
  // Ключ «заказанная частота + выданная частота»: первый прожиг такой пары внутри одного спуска —
  // РАЗВЕДКА (он и сообщает, куда села карта), второй и далее — избежная трата.
  const seenInDescent = new Set();
  let burnsHeld = 0;
  ...
  for (const r of Array.isArray(rungs) ? rungs : []) {
    ...
    if (mv >= seen.deepestMv) {
      // ─── РАЗВЕДОЧНЫЙ ПРОЖИГ ОТДЕЛЁН ОТ ИЗБЕЖНОГО, И РАЗДЕЛЕНИЕ ВВЕДЕНО ПОСЛЕ ЗАМЕРА ────────────
      //
      // 🔴 ЧЕСТНО ПРО ПОРЯДОК СОБЫТИЙ: поле заведено ПОСЛЕ того, как починка фазы 2 довела трату с
      // 30 до 7 и стало видно, что оставшиеся семь — по одному на заказанную частоту. Строгий счёт
      // (`wastedBurns.length`) при этом НЕ ТРОНУТ и печатается по-прежнему: величина не подгоняется
      // под результат, она РАЗДЕЛЯЕТСЯ на две разные величины.
      //
      // ПОЧЕМУ ЭТО НЕ ОСЛАБЛЕНИЕ. Спуск не может знать, на какую частоту сядет карта, ПОКА НЕ
      // прожжёт первую ступень: выданная частота — наблюдение, а не предсказание. Первый прожиг на
      // данной выданной частоте внутри спуска покупает ровно это знание, и дальше спуск им
      // пользуется. Прожиг, купивший наблюдение, «в пустую» не является; предсказать его — это
      // выдумать факт, что запрещено (`PHILOSOPHY.md` → три двери).
      //
  ...
  return {
    ...
    wastedBurns,
    // ИЗБЕЖНАЯ ПОЛОВИНА — та, за которую отвечает движок: он УЖЕ знал, где карта, и всё равно не
    // углубился. Цель эпика 47 — ноль ЗДЕСЬ; строгий `wastedBurns` при этом остаётся на виду.
    avoidableBurns: wastedBurns.filter((w) => !w.discovery),
    // РАЗВЕДОЧНАЯ — первый прожиг на выданной частоте внутри спуска. Он покупает НАБЛЮДЕНИЕ (куда
    // села карта), без которого переезд цели был бы предсказанием, а не наблюдением.
    discoveryBurns: wastedBurns.filter((w) => w.discovery),
...
export function hangFloors(records) {
  ...
  // ─── СТЕНА НЕ МОЖЕТ СТОЯТЬ ВЫШЕ ТОГО, ЧТО ЭТА ЖЕ ЧАСТОТА УЖЕ ВЫДЕРЖАЛА ПРОЖИГОМ ────────────────
  //
  // РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-08-31 10:40 через контур: `interviews/022` Q1 = **B**, и с добавлением
  // его словами — *«И перемерить частоты»*. Расписка — `interviews/decisions/interview_022_….json`.
  //
  // ЧТО БЫЛО. Вечер 30.08 дал по ОДНОЙ записи `ЗАВИС` на 2872 и 2835 МГц у самого стока (глубина
  // 25 мВ). По правилу R18 (`bugs/23`) это навсегда закрыло обе частоты — «ступень не встаёт на
  // него и ниже». А журнал этих же частот содержит 44 и 26 ПРОШЕДШИХ прожигов, самый глубокий на
  // **910 и 890 мВ**, то есть на 125–145 мВ ГЛУБЖЕ стены; само напряжение стены выдерживало прожиг
  // по семь-восемь раз. Сплошная сверка нашла ШЕСТЬ таких мест, а не два.
  //
  // Это два взаимно противоречащих факта об одной частоте, и побеждал МЕНЕЕ доказанный: одно
  // зависание оказывалось старше десяти прохождений.
  //
  // ПОЧЕМУ ОПРОВЕРГНУТАЯ СТЕНА СНИМАЕТСЯ ЦЕЛИКОМ, А НЕ ОПУСКАЕТСЯ ДО ДОКАЗАННОГО. Пол включающий
  // («ступень не встаёт НА него и ниже» — `descentLadder`, `next <= hangFloorMv`), поэтому стена,
  // опущенная РОВНО до доказанного напряжения, запретила бы само это напряжение — новое
  // противоречие вместо старого. Опускать её «на шаг сетки ниже» значит заводить арифметику по
  // сетке и ломать соседний прибор: `bracketedEdge` увидел бы стену вплотную под прошедшей
  // ступенью, счёл край ВЗЯТЫМ В СКОБКИ и закрыл бы частоту вместо спуска — ровно противоположно
  // тому, что решение должно дать. Снятие — единственное прочтение без новых сущностей, и оно же
  // исполняет вторую половину слова владельца: частота спускается ЗАНОВО и находит край ПРОЖИГОМ.
  //
  // ⚠️ ЧТО ЭТО НЕ ДЕЛАЕТ. Не отменяет пол зависания. Зависание ГЛУБЖЕ доказанного стену сохраняет
  // полностью — меняется ровно один случай: запись, которой эта же частота уже противоречит.
  // Настоящий вероятностный край (факт 28: ступень, однажды повесившая машину, а в другой раз
  // прошедшая) стену НЕ теряет, если он глубже доказанного.
  //
  // ОСЬ КЛЮЧА У ОБЕИХ УЛИК ОДНА — ЗАКАЗАННАЯ частота (`i.frequencyMhz` с намерения), и это не
  // совпадение: `provenRungs` заведена как ЗЕРКАЛО этой функции и ключуется так же. Сравнивать
  // здесь урожай (он ключуется ВЫДАННОЙ частотой) значило бы сверять две разные оси — класс
  // `bugs/10` и `bugs/36`, за который проект уже платил дважды.
  const proven = provenRungs(records);
  for (const [mhz, floor] of [...out]) {
    const pass = proven.get(mhz);
    if (!pass || !Number.isFinite(pass.voltageMv)) continue;
    if (floor.voltageMv > pass.voltageMv) out.delete(mhz);
  }
  return out;
}
...
export function selfTest() {
  ...
  try {
    ...
    // ─── СТЕНА НЕ СТОИТ ВЫШЕ ДОКАЗАННОГО (`ideas/16`, `interviews/022` Q1 = B) ───────────────────
    //
    // Решение владельца 2026-08-31. Четыре блока держат ОБЕ стороны правила, и вторая пара важнее
    // первой: сужение, снимающее и настоящие стены, — это отмена пола, а не его починка.
    //
    // АДРЕСАТЫ МУТАЦИЙ, НАЗВАННЫЕ ДО ПРОГОНА:
    //   HM. убрать снятие целиком                    → «опровергнутая стена СНЯТА»
    //   HN. снимать при `>=` вместо `>`              → «стена РОВНО на доказанном — стоит»
    //   HP. снимать всегда, не сверяясь с доказанным → «стена ГЛУБЖЕ доказанного — стоит»
    {
      // (1) ОПРОВЕРГНУТАЯ: зависание у стока, а прожиг этой же частоты проходил на 125 мВ глубже.
      //     Живой случай 2872 МГц: стена 1035, доказано 910.
      const jw = openJournal({ dir: join(sandbox, 'wall-above-proven') });
      writeIntent(jw, { seq: 1, frequencyMhz: 2872, voltageMv: 910 });
      writeVerdict(jw, { seq: 1, at: null, outcome: RUNG_OUTCOME.PASSED, verdict: config.VERDICT.PASS, servingMvAfter: 910 });
      writeIntent(jw, { seq: 2, frequencyMhz: 2872, voltageMv: 1035 });
      closeHangs(jw, { at: '2026-08-30T21:43:06+03:00' });
      ok('ideas/16: ОПРОВЕРГНУТАЯ СТЕНА СНЯТА — она стояла выше того, что частота уже выдержала',
        hangFloors(readJournal(jw).records).has(2872), false);

      // (2) КОНТРОЛЬ AC3, и он тут главный: зависание ГЛУБЖЕ доказанного — настоящий край, стена стоит.
      const jd = openJournal({ dir: join(sandbox, 'wall-below-proven') });
      writeIntent(jd, { seq: 1, frequencyMhz: 2842, voltageMv: 900 });
      writeVerdict(jd, { seq: 1, at: null, outcome: RUNG_OUTCOME.PASSED, verdict: config.VERDICT.PASS, servingMvAfter: 900 });
      writeIntent(jd, { seq: 2, frequencyMhz: 2842, voltageMv: 870 });
      closeHangs(jd, { at: '2026-08-16T23:40:00+03:00' });
      ok('ideas/16 AC3: стена ГЛУБЖЕ доказанного СТОИТ — настоящий край ничего не теряет',
        hangFloors(readJournal(jd).records).get(2842)?.voltageMv ?? 'СНЯТА', 870);

      // (3) ВЕРОЯТНОСТНЫЙ КРАЙ (факт 28): одно и то же напряжение и вешало, и проходило. Стена стоит —
      //     снятие здесь стёрло бы наблюдение, которое проект видел на живой карте.
      const je = openJournal({ dir: join(sandbox, 'wall-equals-proven') });
      writeIntent(je, { seq: 1, frequencyMhz: 2775, voltageMv: 840 });
      writeVerdict(je, { seq: 1, at: null, outcome: RUNG_OUTCOME.PASSED, verdict: config.VERDICT.PASS, servingMvAfter: 840 });
      writeIntent(je, { seq: 2, frequencyMhz: 2775, voltageMv: 840 });
      closeHangs(je, { at: '2026-08-26T20:00:00+03:00' });
      ok('ideas/16: стена РОВНО на доказанном СТОИТ — это вероятностный край, а не противоречие',
        hangFloors(readJournal(je).records).get(2775)?.voltageMv ?? 'СНЯТА', 840);

      // (4) НЕТ ПРОШЕДШИХ ПРОЖИГОВ ВОВСЕ — сверять не с чем, стена стоит.
      const jn = openJournal({ dir: join(sandbox, 'wall-no-proof') });
      writeIntent(jn, { seq: 1, frequencyMhz: 2900, voltageMv: 1000 });
      closeHangs(jn, { at: '2026-08-30T21:43:06+03:00' });
      ok('ideas/16: без единого прошедшего прожига стена СТОИТ — опровергать её нечем',
        hangFloors(readJournal(jn).records).get(2900)?.voltageMv ?? 'СНЯТА', 1000);
    }
