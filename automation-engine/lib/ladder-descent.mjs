...
/**
 * ЗАМОК КАК ВЕРХНЯЯ ГРАНИЦА — второй, И ОН НЕ ЗАКРЕПЛЕНИЕ (`plans/45`, эпик 43 фаза 2).
 *
 * ─── ПОЧЕМУ ЭТО ОТДЕЛЬНЫЙ СТРОИТЕЛЬ, А НЕ ФЛАГ У `candidateProfile` ───────────────────────────────
 *
 * Два профиля выражают ДВА РАЗНЫХ ОБЕЩАНИЯ, и путать их дорого:
 *   · `candidateProfile` — ЗАКАЗ частоты (`min = max`). Обещание: карта стоит ровно здесь, и
 *     доказывается оно постоянством частоты под нагрузкой (`verifyLockUnderLoad`).
 *   · этот — ГРАНИЦА (`min` = пол лестницы, `max` = потолок). Обещание: карта НИКОГДА НЕ ВЫШЕ, а
 *     ниже она свободна, и доказывается оно `judgeDeliveredClock` («не выше, и недобор — это замер»).
 * Требовать постоянства от карты под ГРАНИЦЕЙ — это ровно конфликт 2026-08-14, где три прошедших
 * прожига были объявлены НЕИЗВЕСТНО.
 *
 * ─── ПОЧЕМУ `min` — ПОЛ ЛЕСТНИЦЫ, А НЕ САМ ПОТОЛОК ────────────────────────────────────────────────
 *
 * Слово владельца (`GOAL.md`): *«я хочу, чтобы карта сама могла и разгоняться и снижать частоты»*, и
 * `min = max` этому прямо противоречит — проект отверг `-lgc` именно за это. Факт 38 в `STATUS.md`
 * называет лекарство: *«`-lgc <minGpuClock,maxGpuClock>` — это ДИАПАЗОН, а `1500,1500` лишь его
 * вырожденный случай»*; диапазон «пол простоя … потолок» и есть потолок.
 *
 * ⚠️ ЧТО ЗДЕСЬ НЕ ВЫДУМАНО: пол берётся С ИЗМЕРЕННОЙ ЛЕСТНИЦЫ САМОЙ КАРТЫ (её нижняя ступень), а не
 * назначается круглым числом — то же правило, что у `snapToLadder`. Своего влияния у `min` на этой
 * карте, скорее всего, нет вовсе: `researches/11` §1, проба 3 — заказ `-lgc 2700,2700` карта отдала
 * как 2692, то есть НИЖЕ собственного `min`. Значит `min` — просьба, которую арбитраж вправе не
 * исполнить, и ставить туда потолок было бы обещанием без обеспечения.
 *
 * [NOT-TESTED] at birth — flipped by the blocks in `--selftest` and their mutations.
 */
export function ceilingLockProfile(capMhz, card) {
  const floorMhz = card?.ladder?.mhz?.[0];
  return {
    name: `ceiling-${capMhz}`,
    title: `Граница ${capMhz} МГц`,
    // ТОТ ЖЕ КЛАСС, ЧТО У ЗАКРЕПЛЕНИЯ, И ПО ТОЙ ЖЕ ПРИЧИНЕ (`bugs/05`): граница держится секунды,
    // снимается в `finally`, не отгружается и не запоминается как состояние загрузки — ворота
    // квалификации, чей предмет это то, что владелец нажимает, к ней не относятся.
    kind: 'measurement',
    settings: {
      powerLimitWatts: null,
      graphicsClockLockMhz: { min: floorMhz, max: capMhz },
      // Кривую пишет АТОМ, отдельной записью, и она уже стоит к моменту постановки границы. Профиль
