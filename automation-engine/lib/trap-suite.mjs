...
export async function runTrapSuite() {
  ...
  if (t6card) {
    ...
    pending('T6: НИ ОДИН ПРОЖИГ НЕ КУПЛЕН ВПУСТУЮ — каждый повтор на выданной частоте ушёл ГЛУБЖЕ',
      `сегодня пустых прожигов ${wasted6.length} из ${h6.burnsHeld} `
      + `(${h6.burnsHeld ? Math.round((wasted6.length / h6.burnsHeld) * 100) : 0} % полосы), на `
      + `${new Set(wasted6.map((w) => w.deliveredMhz)).size} выданных частотах: `
      + `${[...new Set(wasted6.map((w) => w.deliveredMhz))].slice(0, 3).join(', ')} МГц. `
      + 'Механизм: спуск продолжает ЗАКАЗЫВАТЬ частоту, которой карта не даёт, и заново проходит лестницу '
      + 'на выданной, где глубина уже доказана. Закрывает фаза 2 эпика 47 (plans/49) — переезд цели на выданную частоту');
