...
export async function selfTest() {
  ...
  // кривая, — поэтому закреплённая ветка тут не бывала.
  {
    ...
    ok('СПАСЕНИЕ ПОД ЖИВОЙ СТУПЕНЬЮ (bugs/88): рука 2 обнулила кривую — и ступень ВСЁ РАВНО пошла на прожиг, на ЗАВОДСКОЙ кривой',
      ...
      [true, true, 0]);
    // 🔴 И ЦЕНА ЭТОГО, ХУЖЕ ЧЕМ ПРЕДПОЛАГАЛ ТИКЕТ: ступень не просто дожгла — она ЗАКРЫЛАСЬ PASS'ом
    // и записала андервольт, которого не было. Прожиг шёл на стоке, а строка утверждает «790 мВ
    // обслуживает 2145 МГц и ПРОШЛО». Это ложная доказанная земля, то есть пробитый барьер «в
    // документ идёт только подтверждённое» (`ЗАКАЗ.md` §3).
    ok('СПАСЕНИЕ ПОД ЖИВОЙ СТУПЕНЬЮ (bugs/88): ступень закрывается ЛОЖНЫМ PASS — прожиг на стоке зачтён как доказанный андервольт',
      [rescued.rec.outcome, rescued.rec.verdict], ['passed', V.PASS]);
    // И третья половина той же находки: у ступени НЕТ НИ ОДНОГО поля, по которому она могла бы
    // узнать о спасении. Пока его нет, чинить дефект нечем — это AC2 тикета.
    ok('СПАСЕНИЕ ПОД ЖИВОЙ СТУПЕНЬЮ (bugs/88): в записи ступени нет ни одного признака спасения (AC2 — сегодня узнать неоткуда)',
