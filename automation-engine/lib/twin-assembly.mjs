...
/**
 * ⚡ УСТАВКА ОКНА ЗДОРОВЬЯ ДЛЯ СТЕНДА — `plans/88` §4c, заведена 2026-09-07 по разбору `bugs/108`.
 *
 * Живой путь требует 300 тактов судьи в секунду. Стенд столько не даёт и дать не может: репетиция
 * гонит на ОДНОЙ машине и полосу, и судью, и пробу, и прожиг. Уставка живого пути превращает
 * полуоткрытое окно в СТЕНУ — судья входит в него и не выходит, полоса ждёт возврата, которого не
 * будет, и прогон умирает по внешнему таймауту, ничего об этом не сказав.
 *
 * ДВА ЗАМЕРА, ОБА 2026-09-07, И ВТОРОЙ ПОПРАВИЛ ПЕРВЫЙ — поэтому здесь стоят оба:
 *
 * | прогон | секунд | мин | медиана/среднее | макс | секунд < 40 |
 * |---|---|---|---|---|---|
 * | зависший (уставка 300, окно-стена) | 299 | 54 | 65 / 67,9 | 401 | **0** |
 * | прошедший (уставка 40) | 57 | **21** | — / 72,5 | 395 | **13** |
 *
 * 🔴 ПЕРВАЯ РЕДАКЦИЯ ЭТОГО КОММЕНТАРИЯ ГОВОРИЛА «секунд ниже 40 — ни одной», и это было верно
 * РОВНО ДЛЯ ОДНОЙ ВЫБОРКИ. Во втором прогоне их тринадцать из пятидесяти семи. Окно всё равно
 * закрылось тринадцать раз — потому что ему нужны три ПОДРЯД здоровых секунды, а не все подряд, — и
 * расписка каждого возврата это печатает: `ticksPerSec=64 · healthySeconds=3/3 · уставка 40/с`.
 *
 * Значит 40 обосновано НЕ «полом, ниже которого не бывает» (такого пола у стенда нет), а тем, что
 * при нём три здоровых секунды подряд набираются регулярно, а замершая машина по-прежнему не
 * набирает их вовсе. Это и есть работа окна: отличать здоровую машину от вставшей, а не мерить
 * разрешение таймера Windows.
 *
 * ⚠️ ЧИСЛО СТЕНДА, А НЕ КАРТЫ. Оно не имеет отношения к вечеру владельца и не смеет утечь на живой
 * путь: там уставка остаётся измеренной живой (`JUDGE_HEALTHY_TICKS_PER_SEC = 300`).
 *
 * 🔴 НАЗВАННЫЙ РИСК, НЕ ЗАКРЫТЫЙ ЗДЕСЬ: если стенд однажды просядет ниже 40 надолго, окно снова
 * станет стеной — и это будет выглядеть КАК ЗАВИСАНИЕ, а не как отказ, потому что судья досидит
 * своё окно 600 с молча. Лечение не в числе, а в том, чтобы недостигнутое окно здоровья кончалось
 * НАЗВАННЫМ отказом раньше внешнего таймаута. Записано в `bugs/108` следующим шагом.
 */
export const TWIN_HEALTHY_TICKS_PER_SEC = 40;
...
export async function makeTwinAssembly({
  ...
  const device = {
    ...
    //
    // 🔴 ФОРМА СТРОКИ — РОВНО ТА, ЧТО ПИШЕТ НАСТОЯЩИЙ СЭМПЛЕР: шапка `{i:-1, meta}` плюс строки
    // `{i, sample}` (`bugs/89`, тот же класс, что `bugs/87` в фейке порта). Прежняя редакция писала
    // ПЛОСКИЙ объект, и это была не мелочь: ветка ПОТОЛКА плоский терпит (`r.sample ?? r`), а ветка
    // ЗАКРЕПЛЕНИЯ идёт через `summarizeSamples`, который отбирает `r.i >= 0 && r.sample` и на плоской
    // строке видит НОЛЬ проб. Цена: закреплённая ступень на двойнике заканчивалась `unknown`
    // («под нагрузкой не снято ни одной пробы частоты») ВСЕГДА — то есть ветка, которой держится вся
    // нижняя половина диапазона и которая сорвалась на карте владельца 31.08 (`bugs/86`), не
    // репетировалась на стенде НИ РАЗУ. Репетиции смертей идут на 2842…2812, где держатель — кривая.
    //
    // 🔴 И ВТОРАЯ ПОЛОВИНА ТОЙ ЖЕ ДЫРЫ: было ОДНО значение, размноженное `Array.from` двенадцать раз,
    // — минимум = медиана = максимум, то есть разброс на двойнике недоказуем ПО ПОСТРОЕНИЮ (ровно то
    // поле, ради которого заведён `bugs/87`). Теперь двенадцать НЕЗАВИСИМЫХ чтений модели: ни одного
    // выдуманного числа — если модель различает состояния между чтениями, проба это покажет; если
    // нет (закреплённая карта стоит где заперли), она честно вернёт одно и то же.
    //
    // GAP, названный а не заклеенный: у двойника нет ВРЕМЕНИ СТЕНЫ внутри прожига
    // (`burnRealSeconds`), поэтому все двенадцать чтений берутся в ОДИН момент — после прожига.
    // Значит стенд моделирует разброс между СОСТОЯНИЯМИ модели и НЕ моделирует разброс, растянутый
    // по времени прожига; второе даёт только живой сэмплер.
    //
    // 🔴 И ЧЕСТНАЯ ГРАНИЦА ДОКАЗАТЕЛЬСТВА, ПОТОМУ ЧТО ЕЁ ИЗМЕРИЛИ, А НЕ ПРЕДПОЛОЖИЛИ. Дефект (1) —
    // форма строки — доказан мутацией TA8 (краснеет). Дефект (2) — размножение одного значения —
    // мутацией TA8b НЕ ДОКАЗЫВАЕТСЯ: она прошла ЗЕЛЁНОЙ, и это находка, а не облегчение
    // ([[EXP-0205]]). Причина честная и неустранимая без выдумки: `deliveredNowMhz()` — функция
    // СОСТОЯНИЯ модели, и в пределах одного состояния двенадцать чтений дают одно и то же число.
    // То есть половина (2) починена ПО ПОСТРОЕНИЮ (чтений теперь двенадцать, а не одно
    // скопированное), но НЕ НАБЛЮДАЕМА на сегодняшней модели. Выдумать дрожание, чтобы блок
    // покраснел, значило бы завести в предохранительном приборе число из головы (`PHILOSOPHY.md` →
    // три двери) — то самое, чем `bugs/87` и опасен.
    startSampler: ({ pinMhz, capMhz, offsetMhz }) => {
      const file = join(runDir, `sampler-${samplerN++}-${pinMhz ? `pin-${pinMhz}` : `cap-${capMhz}`}-${offsetMhz}.jsonl`);
      return {
        file,
        kill: () => {
          const lines = [JSON.stringify({ i: -1, meta: { sampler: 'цифровой двойник', period_ms: 500 } })];
          for (let i = 0; i < 12; i++) {
            lines.push(JSON.stringify({ i, sample: { 'utilization.gpu': 99, 'clocks.gr': vc.deliveredNowMhz() } }));
          }
  ...
  return {
    ...
    riders: {
      judgeArgs: ['--judge',
        ...
        // ⚡ ОКНО ЗДОРОВЬЯ НАЗЫВАЕТ СВОИ ЧИСЛА — заведено 2026-09-07, и это ОТМЕНА ВЫВОДА
        // `plans/88` §4c(5), а не восполнение забытой проводки. Разница важна, и вот она.
        //
        // §4c(5) замерил стенд ПОСЛЕ проводки — **418 · 438 · 426 · 414** тактов/с — и заключил
        // дословно: *«стенд даёт 414…438 и принимает измеренную уставку без единого своего числа.
        // Одно число в одном месте уцелело; догадка была бы дороже замера»*. Вывод был сделан
        // ЧЕСТНО и по замеру. Он просто не воспроизводится.
        //
        // ЗАМЕР 2026-09-07 на той же репетиции, 299 секунд строки жизни: минимум **54**, медиана
        // **65**, среднее **67,9**, максимум 401. Медиана 65 — это разрешение таймера Windows по
        // умолчанию (16 мс); 401 — то же самое при разрешении 1 мс. **Судья своё разрешение НЕ
        // ПОДНИМАЕТ** — `timeBeginPeriod(1)` живёт только в `death-watch.mjs`, в `fuse.mjs` его нет
        // вовсе. Значит такт судьи на стенде зависит от того, что в этот момент делает ОКРУЖЕНИЕ,
        // и «414…438» было измерено в удачном состоянии машины, а не в свойстве стенда.
        //
        // Уставка 300 на такте с медианой 65 — это СТЕНА: `bugs/72` · [[EXP-0193]].
        //
        // ЦЕНА, ЗАПЛАЧЕННАЯ ЦЕЛИКОМ: репетиция `strangle` 07.09 — судья вошёл в полуоткрытое окно
        // (`half-open` 10:54:27.782) и НЕ ВЫШЕЛ НИКОГДА; строки `rearm` в журнале нет вовсе. Полоса
        // встала в «⏳ ЖДУ АПВ» и провисела до внешнего таймаута 300 с. Пять дней это списывалось на
        // шторм спасений (`bugs/108`), а шторм к тому дню уже был убит починкой `bugs/111`:
        // срабатывание было ОДНО.
        //
        // ЧИСЛО ВЫВЕДЕНО ЗАМЕРОМ ТОГО ЖЕ ПРОГОНА, а не выбрано: 299 секунд строки жизни судьи дали
        // минимум 54 такта/с, медиану 65, среднее 67,9, максимум 401; секунд ниже 40 — НИ ОДНОЙ.
        // 40 стоит с запасом под измеренным полом и совпадает с числом, которым уже обоснована
        // фикстура набора `fuse` (там тот же режим: процесс без `timeBeginPeriod`).
        // ⚠️ ВЫБОРКА ЧЕСТНО УЗКАЯ: в том прогоне полоса до тяжёлой нагрузки не дошла — она стояла в
        // ожидании. Дойдёт и просядет ниже 40 — окно снова станет стеной, и репетиция это ПОКАЖЕТ
        // так же громко, как показала сегодня; тогда число правится следующим замером, а не вкусом.
        // 🔴 `bugs/121`, 08.09: ЭТОЙ ДВЕРИ БОЛЬШЕ НЕТ, И ЭТО НЕ ПОТЕРЯ, А СНЯТИЕ ПАРЫ. Уставка
        // стенда была нужна ровно потому, что проба мерила РАЗРЕШЕНИЕ ТАЙМЕРА — о чём комментарий
        // выше говорит прямо. Проба переведена на РАЗРЫВ такта, а он от кванта таймера не зависит:
        // одно число (60 мс) судит и стенд, и живой путь. Две уставки на одно решение были парой,
        // и жила эта пара ровно до дня, когда живой путь встал на первом же спасении.
...
/**
 * ТРИ ПРЕДИКАТА РЕПЕТИЦИИ СМЕРТИ, ВЫНЕСЕННЫЕ ИЗ НЕЁ (`bugs/104`) — и вот зачем.
 *
 * Обе исправленные проверки пять дней стояли красными, и никто этого не увидел: репетиция НЕ входит
 * в батарею (она читает живые журналы и стоит ≈12 с на профиль), а гоняют её руками раз в неделю.
 * Инлайн-условие внутри такой функции нельзя ни замутировать, ни накрыть — значит нельзя доказать,
 * что оно ВООБЩЕ умеет краснеть. Вынесенные, они покрываются `twin --selftest`, то есть батареей,
 * и определение при этом остаётся ОДНО: репетиция зовёт эти же функции, а не свою копию условия.
 *
 * ⚠️ ЧЕГО ЭТО НЕ ЛЕЧИТ, И ЭТО НАЗВАНО: батарея докажет, что предикат умеет краснеть, но НЕ заметит,
 * что он устарел по СМЫСЛУ — именно так и вышло 31.08, когда полоса научилась идти дальше. От этого
 * лечит только прогон самой репетиции; он внесён в реестр пар при закрытии сессии.
 */
export function causeForProfile(profile) {
  return profile === 'progress-stall' ? 'progress-stall' : 'beat-silence';
}

/** Срабатывания есть, и причина у КАЖДОГО — ожидаемая. «Ровно одно» опровергнуто (`bugs/104`). */
export function everyIntentHasCause(intents, expectedCause) {
  return Array.isArray(intents) && intents.length >= 1
    && intents.every((x) => x?.cause === expectedCause);
}

/** Сколько раз полоса объявила, что ПРОДОЛЖАЕТСЯ после спасения (слово владельца, `interviews/024` = E). */
export function countBandWentOn(log) {
  return (String(log ?? '').match(/ПОЛОСА ПРОДОЛЖАЕТСЯ/gu) ?? []).length;
}
...
async function mainRehearseDeath(profile, { withWindow = true, armNMs = null } = {}) {
  ...
  // `bugs/104` — обе величины считаются ОДИН раз и по имени, чтобы проверка печатала СВОЁ число:
  // красная проверка без числа и есть то, что пять дней читали как «ну красное и красное».
  const expectedCause = causeForProfile(profile);
  const bandWentOn = countBandWentOn(log);
  const checks = [
    ...
    // ✏️ ПЕРЕПИСАНО 2026-09-04 (`bugs/104`): было `intents.length === 1`. Условие родилось 29.08,
    // когда срабатывание КОНЧАЛО полосу; 31.08 полоса научилась идти дальше по слову владельца
    // (`interviews/024` = E, `bugs/90`), и с профилем «прогресс встал» каждая частота полосы даёт
    // своё срабатывание. Проверка краснела пять дней, а репетицию никто не гонял — она не входит
    // в батарею. СТРОГАЯ ПОЛОВИНА СОХРАНЕНА И ДАЖЕ УСИЛЕНА: причина сверяется у КАЖДОГО намерения,
    // а не у первого, и требуется хотя бы одно. Ослаблено ровно «ровно одно» — то, что опровергнуто.
    // 🔴 ПЕРВОЙ СТРОКОЙ — ФАКТ, КОТОРОГО ЗДЕСЬ НЕ БЫЛО ВОВСЕ (`bugs/109`, найден 2026-09-05).
    //
    // Код прогона ПЕЧАТАЛСЯ строкой отчёта и не проверялся ничем. 05.09 профиль `strangle` был убит
    // по таймауту 300 с (`код null`), закрыл ОДНУ частоту из трёх вместо трёх за 7,5 с — и репетиция
    // объявила себя зелёной, 8 из 8. Улики убитого прогона неотличимы от улик прошедшего:
    // срабатывания были, руки отработали, кольцо на месте. Единственный различающий факт был у
    // функции в руках и лежал на полу — ровно то, что приказ владельца про приёмку и запрещает:
    // «по коду выхода, никогда по фильтрованному grep» ([[EXP-0237]]).
    //
    // Проверяется `!== null`, а НЕ «код равен такому-то»: у репетиции смерти ШТАТНЫЙ исход — код 1
    // (полоса встала на первом шаге от стока), и требование нуля завело бы второе решение о том,
    // какой исход правильный, там, где нужен ОДИН факт: прогон кончился сам.
    [`прогон под репетицией ЗАВЕРШИЛСЯ САМ, а не был убит (код ${r.status}${r.error ? ` · ${r.error.code ?? r.error.message}` : ''})`,
      r.status !== null],
    [`срабатывания есть, и причина у КАЖДОГО — ${expectedCause} (намерения в журнале судьи: ${intents.length})`,
      everyIntentHasCause(intents, expectedCause)],
    ...
    // ✏️ ПЕРЕВЁРНУТА 2026-09-04 (`bugs/104`, B104-AC2). Здесь проверялось, что полоса ВСТАЛА, — то
    // есть репетиция требовала ровно того поведения, которое владелец ОТМЕНИЛ 31.08: *«Сколько угодно
    // спасений разрешаю… Запустил и забыл — оно само все сделало»* (`interviews/024` = E). Теперь
    // утверждается новое поведение, а не терпится: полоса ПРОДОЛЖИЛАСЬ после спасения.
    [`полоса ПРОДОЛЖИЛАСЬ после спасения (строк «ПОЛОСА ПРОДОЛЖАЕТСЯ»: ${bandWentOn})`, bandWentOn >= 1],
...
//   TA8. `device.startSampler`: вернуть ПЛОСКУЮ строку пробы (или одно значение, размноженное
//        двенадцать раз) → «ЗАКРЕПЛЕНИЕ: закреплённая ступень двойника ДОХОДИТ ДО ВЕРДИКТА»
//        (`bugs/89`: ветка закрепления читает `summarizeSamples`, который плоскую строку не видит)
//   TA9. блок 7б: снять вызов руки 2 из `applyProfile` → «СПАСЕНИЕ ПОД ЖИВОЙ СТУПЕНЬЮ» (`bugs/88`)
export async function selfTest() {
  ...
  // 🔴 `bugs/108` / `plans/88` §4c — ОКНО ЗДОРОВЬЯ НАЗЫВАЕТ ЧИСЛА СТЕНДА, И ЭТОГО НЕ БЫЛО ДО 07.09.
  //
  // Без флага судья на двойнике требует 300 тактов/с — уставку ЖИВОГО пути, которой стенд физически
  // не даёт (замер того же дня: минимум 54, медиана 65). Окно становится СТЕНОЙ: судья входит в
  // полуоткрытое состояние и не выходит, строки `rearm` в журнале нет вовсе, полоса виснет в
  // «ЖДУ АПВ» до внешнего таймаута. Блок проверяет ПРОВОДКУ, а не решение окна: само решение
  // доказано в наборе `fuse`, а здесь ломается именно то, что 07.09 и было сломано.
  //
  // Проверяется И ЗНАЧЕНИЕ, а не только присутствие флага: флаг с уставкой живого пути — та же
  // стена, только с видимостью проводки.
  // ✏️ ПЕРЕПИСАН 08.09 (`bugs/121`). Прежде блок требовал, чтобы двойник получал СВОЮ уставку
  // такта. Теперь он требует обратного: никакой своей уставки у стенда нет — обе среды судит один
  // разрыв. Сторож остался сторожем ПРОВОДКИ, только сторожит он снятую пару, а не её наличие.
  ok('ВСАДНИКИ: у двойника НЕТ своей уставки окна здоровья — разрыв такта судит обе среды (bugs/121)',
    [asmArmed.riders.judgeArgs.includes('--rearm-healthy-ticks'),
      asmArmed.riders.judgeArgs.includes('--rearm-healthy-gap')],
    [false, false]);
  ...
  // 7б. ЗАКРЕПЛЕНИЕ (`bugs/89`, TA8) И СПАСЕНИЕ ПОД ЖИВОЙ СТУПЕНЬЮ (`bugs/88`, TA9).
  //
  // Оба блока гоняют НАСТОЯЩИЕ `runRung` + `runStep` + оракул по ступени 2145 МГц / 790 мВ — той
  // самой, что сорвалась на карте владельца 31.08. Частота выбрана НЕ произвольно: 2145 лежит НИЖЕ
  // пола потолка (2157 = верх − 1000, факт 38), значит держатель — ЗАКРЕПЛЕНИЕ, а не кривая. Вся
  // прежняя жизнь стенда (смоук, репетиции смертей, настройка порога) идёт на 2842…2812, где держит
  // кривая, — поэтому закреплённая ветка тут не бывала.
  {
    const engineMod = await import('../engine.mjs');
    const hand = await import('./fuse-rescue-hand.mjs');

    // Одна ступень, с необязательной подложенной рукой 2 спасения ПОСЛЕ записи кривой и закрепления,
    // но ДО прожига — ровно фаза A хронологии `bugs/88` (трип случился, прожига ещё не было).
    const runPinnedRung = async ({ rescueBeforeBurn = false } = {}) => {
      const a = await makeTwinAssembly({ seed: 7 });
      const nonZero = () => a.vc.curveBackend.readOffsetsSync().filter((o) => o !== 0).length;
      const seen = { burnAfterRescue: false, rescued: false, offsetsAtBurn: null, deliveredAtBurn: null };
      const dev = { ...a.device, burn: { ...a.device.burn } };
      const realApply = a.device.applyProfile;
      dev.applyProfile = async (be, profile, opts) => {
        const r = await realApply(be, profile, opts);
        if (rescueBeforeBurn) {
          // РУКА 2 — НАСТОЯЩИЙ `doStockRescue` по ТОМУ ЖЕ экземпляру двойника: `vc` инъектируется,
          // и это объявленный шов самого модуля руки (её шапка: «`vc` is injectable so a selftest
          // can hold offsets on the SAME instance and observe them zeroed»). Отдельный ПРОЦЕСС руки
          // держал бы СВОЙ экземпляр — именно та названная граница модели, из-за которой сквозная
          // репетиция этот дефект показать не может.
          const r2 = await hand.doStockRescue({ nvapiModule: await hand.buildTwinNvapiModule({ vc: a.vc }) });
          seen.rescued = r2.ok === true;
        }
        return r;
      };
      const realBurn = a.device.burn.stressTest;
      dev.burn.stressTest = async (o) => {
        seen.offsetsAtBurn = nonZero();
        seen.deliveredAtBurn = a.vc.deliveredNowMhz();
        if (seen.rescued) seen.burnAfterRescue = true;
        return realBurn(o);
      };
      const rec = await engineMod.runRung({
        points: a.vc.curveBackend.points(),
        clockMhz: 2145, voltageMv: 790, seconds: 1, sustain: 1,
        pinCard: a.probedCard, canPin: true, envelopeMhz: a.card.card.maxGraphicsMhz,
        runStepFn: (args) => vf.runStep({ ...args, device: dev }),
      });
      return { rec, seen };
    };

    // TA8 — мутация-адресат: вернуть `startSampler` к ПЛОСКОЙ строке (или к одному значению,
    // размноженному двенадцать раз) → этот блок красный: `outcome` станет `unknown`.
    const clean = await runPinnedRung();
    ok('ЗАКРЕПЛЕНИЕ: закреплённая ступень двойника ДОХОДИТ ДО ВЕРДИКТА (форма строки сэмплера — как у настоящего, bugs/89)',
      [clean.rec.outcome, clean.rec.verdict, clean.rec.holder, clean.rec.deliveredMhz],
      ['passed', V.PASS, 'закрепление частоты', 2145]);

    // TA9 — мутация-адресат: снять вызов руки 2 из `applyProfile` → блок красный (спасения не было,
    // а он утверждает, что прожиг случился ПОСЛЕ него).
    const rescued = await runPinnedRung({ rescueBeforeBurn: true });
    ok('СПАСЕНИЕ ПОД ЖИВОЙ СТУПЕНЬЮ (bugs/88): рука 2 обнулила кривую — и ступень ВСЁ РАВНО пошла на прожиг, на ЗАВОДСКОЙ кривой',
      [rescued.seen.rescued, rescued.seen.burnAfterRescue, rescued.seen.offsetsAtBurn],
      [true, true, 0]);
    // 🔴 ЦЕНА ЭТОГО, ХУЖЕ ЧЕМ ПРЕДПОЛАГАЛ ТИКЕТ: ступень БЕЗ ВХОДА ПРЕДОХРАНИТЕЛЯ закрывается
    // PASS'ом и записывает андервольт, которого не было. Прожиг шёл на стоке, а строка утверждает
    // «790 мВ обслуживает 2145 МГц и ПРОШЛО» — ложная доказанная земля, то есть пробитый барьер
    // «в документ идёт только подтверждённое» (`ЗАКАЗ.md` §3).
    //
    // ⚠️ БЛОК ДЕРЖИТ ИМЕННО «БЕЗ ВХОДА», И ЭТО НЕ ФОРМАЛЬНОСТЬ. `fuseTripsFn` здесь намеренно НЕ
    // проведён: блок описывает состояние движка, в котором ступень о спасении узнать НЕ МОЖЕТ, —
    // то самое, что было на карте владельца 31.08. Лечение живёт этажом выше (`engine.runRung`,
    // вход предохранителя) и доказано СВОИМИ блоками и пятью мутациями в наборе движка; здесь
    // остаётся зафиксированной цена его отсутствия.
    ok('СПАСЕНИЕ ПОД ЖИВОЙ СТУПЕНЬЮ (bugs/88): БЕЗ входа предохранителя ступень закрывается ЛОЖНЫМ PASS — прожиг на стоке зачтён как доказанный андервольт',
      [rescued.rec.outcome, rescued.rec.verdict], ['passed', V.PASS]);
    // И третья половина той же находки: без проведённого входа у ступени НЕТ НИ ОДНОГО поля, по
    // которому она могла бы узнать о спасении. Это AC2 тикета, и это же — измеренная причина, по
    // которой вход пришлось заводить.
    ok('СПАСЕНИЕ ПОД ЖИВОЙ СТУПЕНЬЮ (bugs/88): без входа в записи ступени нет ни одного признака спасения (AC2)',
      Object.keys(rescued.rec).some((k) => /rescue|fuse/iu.test(k)), false);

    // ─── А ТЕПЕРЬ ТО ЖЕ САМОЕ С ПРОВЕДЁННЫМ ВХОДОМ — ЛЕЧЕНИЕ НА ДОРОГЕ ДВОЙНИКА ─────────────────
    //
    // Пара блоков выше без этой пары была бы описанием болезни без доказательства выздоровления.
    // Здесь тот же путь, та же рука, та же ступень — и один проведённый вход.
    {
      const a = await makeTwinAssembly({ seed: 7 });
      let trips = 0;
      const dev = { ...a.device, burn: { ...a.device.burn } };
      ...
      dev.applyProfile = async (be, profile, opts) => {
        const r = await realApply(be, profile, opts);
        await hand.doStockRescue({ nvapiModule: await hand.buildTwinNvapiModule({ vc: a.vc }) });
        trips = 1;                       // судья трипнул: рука отработала, счёт в журнале вырос
        return r;
      };
      const rec = await engineMod.runRung({
        points: a.vc.curveBackend.points(),
  ...
  // ── bugs/104 — ПРЕДИКАТЫ РЕПЕТИЦИИ СМЕРТИ, И КАЖДАЯ МУТАЦИЯ КРАСИТ СВОЮ ────────────────────────
  // Обе проверки простояли красными пять дней. Здесь доказывается не «они зелёные», а то, что они
  // ВООБЩЕ УМЕЮТ КРАСНЕТЬ — иначе починка была бы заменой одного слепого условия на другое.
  ok('bugs/104: причина профиля выводится по имени, а не переписана в трёх местах',
    [causeForProfile('progress-stall'), causeForProfile('strangle'), causeForProfile('instant')],
    ['progress-stall', 'beat-silence', 'beat-silence']);
  ok('bugs/104: НЕСКОЛЬКО срабатываний законны — «ровно одно» опровергнуто прогоном (3 · 9 · 5)',
    everyIntentHasCause([{ cause: 'beat-silence' }, { cause: 'beat-silence' }, { cause: 'beat-silence' }], 'beat-silence'),
    true);
  ok('bugs/104 МУТАЦИЯ 1: ОДНО чужое намерение среди своих — проверка КРАСНЕЕТ',
    everyIntentHasCause([{ cause: 'progress-stall' }, { cause: 'beat-silence' }], 'progress-stall'),
    false);
  ok('bugs/104 МУТАЦИЯ 2: срабатываний НЕТ вовсе — проверка КРАСНЕЕТ (репетиция без отказа не репетиция)',
    [everyIntentHasCause([], 'beat-silence'), everyIntentHasCause(null, 'beat-silence')],
    [false, false]);
  ok('bugs/104: «ПОЛОСА ПРОДОЛЖАЕТСЯ» считается, а не сопоставляется — число идёт в текст проверки',
    countBandWentOn('… ⚡ СПАСЕНИЕ на 2842 МГц … ПОЛОСА ПРОДОЛЖАЕТСЯ\nдальше\nПОЛОСА ПРОДОЛЖАЕТСЯ'), 2);
  ok('bugs/104 МУТАЦИЯ 3: полоса ВСТАЛА (строки продолжения нет) — проверка КРАСНЕЕТ',
    countBandWentOn('⚡ СПАСЕНИЕ сработало, судья вышел с кодом 2') >= 1, false);
