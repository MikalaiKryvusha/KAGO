...
/**
 * ЧТЕНИЕ ДОКУМЕНТА КРИВОЙ ДЛЯ СПИСКА ПРОФИЛЕЙ — СВОЁ, А НЕ ЧЕРЕЗ `curve-store` (`bugs/97`).
 *
 * `curve-store` транзитивно тянет зонды карты, а список профилей обязан печататься на машине без
 * видеокарты — это условие соседних блоков набора, и ломать его ради одной строки нельзя.
 * Читается ровно то, что нужно строке: список строк документа. Ни валидации, ни смещений — их
 * считает применение против живой таблицы.
 *
 * МОЛЧА возвращает `null` на любой беде: список профилей не имеет права упасть из-за того, что
 * документ кривой отсутствует или битый. Отсутствие документа — это САМА ПО СЕБЕ новость для
 * читателя («ссылка ведёт в никуда, применение откажет»), и она печатается строкой, а не
 * исключением.
 */
function readCurveDocQuietly(name, dir = fileURLToPath(new URL('../../curves/', import.meta.url))) {
  try {
    const p = path.join(dir, `${String(name).replace(/[^\w.-]/g, '')}.json`);
    if (!existsSync(p)) return null;
    const doc = JSON.parse(readFileSync(p, 'utf8'));
    return Array.isArray(doc?.frequencies) ? doc : null;
  } catch { return null; }
}
...
function renderSettings(s) {
  ...
  // ─── ЗАМОК-ГРАНИЦА И ЗАКРЕПЛЕНИЕ — ДВА РАЗНЫХ ОБЪЕКТА, И ОДНИМ СЛОВОМ ИХ НАЗЫВАТЬ НЕЛЬЗЯ ────────
  //
  // Канон различает их прямо (`GPU_TUNING_RAILS.md` §3): ЗАКРЕПЛЕНИЕ — это `-lgc min=max`, приказ
  // держать одну частоту, и его доказательство «частота ПОСТОЯННА»; ЗАМОК-ГРАНИЦА — это
  // `-lgc <пол, потолок>`, карта свободна вниз, и доказательство у него другое — «никогда ВЫШЕ».
  // Требовать постоянства под границей — это дословно конфликт 2026-08-14, где три ступени с
  // прошедшими прожигами были отчитаны НЕИЗВЕСТНО.
  //
  // Здесь стояло «фиксация частоты» на оба случая. Пока у отгружаемых профилей замка не было,
  // строка врала только про пробные; с 2026-09-01 (решение владельца `interviews/025`) замок стоит
  // во всех трёх рабочих режимах диапазоном 180…3090, и владелец читал бы «фиксация» там, где карта
  // свободна во всём своём диапазоне. Одно понятие — одно слово (THE REGISTER).
  lines.push(`    ${l === null ? 'частота           свободна (-rgc, замка нет)'
    : l.min === l.max ? `ЗАКРЕПЛЕНИЕ частоты ${l.min} МГц (пин: карта обязана держать ровно её)`
      : `ЗАМОК-ГРАНИЦА      ${l.min}…${l.max} МГц (карта свободна ВНИЗ, выше ${l.max} не уходит)`}`);
  ...
  // 🔴 ССЫЛКА НА ДОКУМЕНТ КРИВОЙ — ВТОРАЯ ФОРМА ТОГО ЖЕ ПОЛЯ, И БЕЗ НЕЁ СТРОКА ВРАЛА (`bugs/97`).
  //
  // Найдено ВЛАДЕЛЬЦЕМ 2026-08-31, его вопросом: *«какого хуя мы гоняем тесты, узнаём пол точек — и
  // это НЕ ПРЕВРАЩАЕТСЯ ДИНАМИЧЕСКИ в обновлённую VF кривую профилей?»* Превращается: все три
  // режима несут `curveRef: "measured"` и питаются живым документом. Но ЭТА строка читала ТОЛЬКО
  // `curveRaiseAndCapMhz`, и на профиле со ссылкой печатала «заводская (все смещения 0)» — то есть
  // ровно противоположное правде, в том самом месте, где владелец решает, что взводить.
  //
  // Цена уже уплачена дважды за один вечер: по этой строке я сам доложил владельцу, что Silent Cold
  // и Optimised несут заводскую кривую и андервольта не дают. Прибор соврал — и через меня соврал
  // ему. Это класс `bugs/40`: заголовок совпал, под ним не читали.
  //
  // ⚠️ ЗДЕСЬ ПЕЧАТАЮТСЯ ЧИСЛА ДОКУМЕНТА, А НЕ СМЕЩЕНИЯ. Смещения считаются при ПРИМЕНЕНИИ, против
  // ЖИВОЙ таблицы карты (`resolveProfileCurve` → `offsetsFor`), потому что таблица едет с прогревом.
  // Сказать здесь «смещение будет таким-то» значило бы назвать число, которого никто не мерил.
  // Список отвечает на другой, честный вопрос: ЧТО ЛЕЖИТ В ДОКУМЕНТЕ, которым профиль питается.
  //
  ...
  if (raise === null && ref !== null) {
    ...
    if (doc === null) {
      lines.push(`    кривая V/F         🔴 ССЫЛКА «${ref}» — документ НЕ НАЙДЕН, применение откажет · ${capSaid}`);
    } else {
      const tuned = doc.frequencies.filter((r) => Number.isFinite(r?.voltageMv)
        && Number.isFinite(r?.stockVoltageMv) && r.voltageMv < r.stockVoltageMv);
      const depths = tuned.map((r) => r.stockVoltageMv - r.voltageMv).sort((a, b) => a - b);
      lines.push(`    кривая V/F         ЖИВАЯ ССЫЛКА на документ «${ref}»: строк ${doc.frequencies.length}, `
        + `с НАЙДЕННЫМ андервольтом ${tuned.length}`
        + (depths.length ? ` (глубина ${depths[0]}…${depths[depths.length - 1]} мВ, медиана ${depths[Math.floor(depths.length / 2)]} мВ)` : '')
        + ` · ${capSaid}`);
      lines.push('                       смещения считаются ПРИ ПРИМЕНЕНИИ против живой таблицы карты, а не хранятся');
    }
  } else {
...
function cmdSelftest() {
  ...
  // ─── ИНВАРИАНТ ОТГРУЖАЕМЫХ РЕЖИМОВ: ЗАМОК — ГРАНИЦА, А НЕ ПИН (`plans/83` Ш3) ─────────────────
  //
  // Проверяются НЕ фикстуры, а сами артефакты в `profiles/`, и это другой род блока: остальные
  // спрашивают «отказывает ли сторож формата», этот — «что мы отгружаем владельцу». Поэтому он не
  // в списке фикстур, а отдельным проходом.
  //
  // Правило владельца, которое здесь охраняется (`GOAL.md` → «⭐ ЧТО ТАКОЕ ТЮНИНГ VF-КРИВОЙ»):
  // *«Карточка сама принимает решение, на какую частоту ей гнаться»* — то есть в РЕЖИМЕ карта обязана
  // сохранять свой диапазон. `-lgc min=max` это приказ держать одну частоту (`researches/11`: замок
  // это потолок, а не приказ), и в отгружаемом режиме он запрещён. Пробным профилям (`*.local`,
  // без поля `mode`) пин законен — им он и нужен, чтобы держать частоту на время замера.
  //
  // Читается с диска, потому что защищаемое живёт на диске: блок над фикстурой сказал бы, что
  // валидатор умеет считать min и max, и промолчал бы о том, что кто-то поставил пин в `optimised`.
  let extra = 0;
  {
    extra += 1;
    const shipped = [];
    for (const f of listProfileFiles()) {
      const { profile } = loadProfileFile(f);
      if (!profile || profile.mode === undefined) continue;   // не режим — пин законен
      shipped.push(profile);
    }
    const pins = shipped
      .map((p) => ({ name: p.name, lock: p.settings?.graphicsClockLockMhz ?? null }))
      .filter((x) => x.lock !== null && x.lock.min === x.lock.max);
    if (shipped.length === 0) {
      failed++;
      console.log('ПРОВАЛ отгружаемые режимы: ни одного профиля с полем `mode` не найдено — блок мерит не то');
    } else if (pins.length === 0) {
      console.log(`OK   отгружаемые режимы (${shipped.length}) не несут ЗАКРЕПЛЕНИЯ частоты — только границу или ничего`);
    } else {
      failed++;
      console.log('ПРОВАЛ отгружаемый режим несёт ПИН вместо границы: '
        + pins.map((x) => `${x.name} (${x.lock.min}=${x.lock.max})`).join(', ')
        + ' — карта обязана сохранять свой диапазон (слово владельца)');
    }
  }

  console.log('');
  console.log(`САМОПРОВЕРКА ФОРМАТА: ${blocks.length + extra} блоков, провалов ${failed}.`);
  return failed === 0 ? 0 : 1;
}
