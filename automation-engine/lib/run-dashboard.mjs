...
export async function selfTest() {
  ...
  //     BF. поменять 20 секунд подачи на другое число                 → блок «ДВАДЦАТЬ СЕКУНД»
  {
    ...
    //     BL. убрать место под шаг из плитки напряжения       → «У НАПРЯЖЕНИЯ ЕСТЬ МЕСТО ПОД ШАГ»
    {
      ...
      // ⚠️ ЕДИНИЦЫ У КАЖДОЙ ВЕЛИЧИНЫ (`bugs/42`). Владелец прочёл на странице «сток 995» — голое
      //    число там, где рядом печатаются и МГц, и Вт. Сторож требует «мВ» ИМЕННО у стока, а не
      //    где-нибудь в строке: единица у соседнего андервольта уже была, и она-то и маскировала дыру.
      check('У СТОКА НА СТРАНИЦЕ ЕСТЬ ЕДИНИЦА — число без неё читатель обязан угадывать',
        /сток \$\{r\.stockVoltageMv\} мВ/.test(page),
        'строка стока печатает голое число: единица стоит только у андервольта в конце');
    }

    // ─── ОКНО ГАСИТ САМА ОСТАНОВКА ПРОГОНА, А НЕ ПАМЯТЬ АГЕНТА (`bugs/42`, четвёртый случай) ──────
    //
    // Вечером 2026-08-23 остановленный прогон оставил окно визуализатора открытым и ЗВУЧАЩИМ;
    // владелец из другой комнаты считал, что работа идёт. Дыра была структурной: обработчики выхода
    // регистрировались ВНУТРИ ветки «окна нет — поднимаю сам», поэтому прогон, начатый при уже
    // открытом окне, не заводил их вовсе.
    //
    // Сторож судит ПОРЯДОК В ИСХОДНИКЕ движка, потому что судить его иначе нечем: проверить это
    // прогоном значит запустить настоящую развёртку с записью в карту. Проверка адресная — та же
    // форма, которой набор `thermal` доказывает свою инертность (адресами строк, а не прозой).
    //   АДРЕСАТ МУТАЦИИ, НАЗВАННЫЙ ДО ПРОГОНА:
    //     BW. вернуть регистрацию внутрь ветки подъёма  → «ГАШЕНИЕ ОКНА РЕГИСТРИРУЕТСЯ ДО ВЕТКИ»
    {
      // Путь от рабочего каталога — та же договорённость, что у страницы выше в этом наборе.
      const enginePath = join('automation-engine', 'engine.mjs');
      ...
      check('и он снимает ОБЕ половины: процесс сервера и само окно',
        /const shutWindow = \(\) => \{[\s\S]{0,240}?dashProc\?\.kill\(\)[\s\S]{0,240}?dash\.closeWindow\(\)/.test(engineSrc),
        'гашение не трогает либо процесс сервера, либо окно — звук живёт в окне');
    ...
    //     CE. не звать wireCurveWidget() при подъёме страницы           → «ПРОВОДКА ВИДЖЕТА ПОДКЛЮЧЕНА»
    {
      ...
      // ШАПКА — ТРИ УТВЕРЖДЕНИЯ, НЕ ДВА (`bugs/100`, класс EXP-0078). Над завершённым прогоном шапка
      // обязана говорить «завершён», как вкладка и строка состояния; безусловный вызов
      // `head('Выполняется…')` в кадре — ровно тот дефект. Проверяется ВЫЗОВ с веткой, а не упоминание.
      //     CL. вернуть безусловный head('Выполняется…') в кадре              → «ШАПКА НАД ЗАВЕРШЁННЫМ ПРОГОНОМ»
      check('ШАПКА НАД ЗАВЕРШЁННЫМ ПРОГОНОМ говорит «завершён»: вызов head() в кадре несёт ветку finished (bugs/100)',
        /head\(pulse\.run && pulse\.run\.finished === true\s*\?\s*'Прогон завершён[^']*'\s*:\s*'Выполняется автоматический тюнинг-прогон видеокарты'\)/.test(page)
          && !/^\s*head\('Выполняется автоматический тюнинг-прогон видеокарты'\);/m.test(page),
        'в кадре стоит безусловная шапка «Выполняется…» или у вызова head() нет ветки finished');
    }
  }
...
async function main(argv) {
  // 🔴 `--help` ОТВЕЧАЕТ И ВЫХОДИТ — ПЕРВОЙ ВЕТКОЙ, ДО ЧЕГО БЫ ТО НИ БЫЛО С ПОСЛЕДСТВИЯМИ.
  // Владелец 08.09: *«Моя ошибка — чини. Завтра ты опять её допустишь. Она не на тебя должна
  // опираться, а на код»*. Прежде этот прибор на `--help` МОЛЧА ПОДНИМАЛ СЕРВЕР: занимал порт 7311,
  // открывал окно браузера и вешал вызывающего навсегда (замерено: код 124 под таймаутом 6 с).
  // Спрашивающий «что ты умеешь» получал работающий сервер — ответ хуже отказа, потому что выглядит
  // как зависание. Порядок веток здесь — не стиль, а сам предохранитель: `helpAnswersFirst` в
  // `tools/help-guard.mjs` прогоняет этот файл с `--help` под таймаутом и требует код 0 без порта.
  if (argv.includes('--help') || argv.includes('-h')) {
    console.log('Использование: node automation-engine/lib/run-dashboard.mjs [флаги]');
    console.log('  ЧТО ЭТО: окно наблюдения за прогоном. Поднимает СЕРВЕР на 127.0.0.1 и держит его');
    console.log('           до Ctrl+C. Карты не касается вовсе — только читает файлы.');
    console.log('');
    console.log('  --port <N>        порт сервера (по умолчанию 7311)');
    console.log('  --no-window       не открывать окно браузера, поднять только сервер');
    console.log('  --close           погасить И окно, И сервер на порту, затем выйти');
    console.log('  --selftest        прогнать самопроверку и выйти, сервер не поднимается');
    console.log('  --pulse <путь>    файл пульса      (по умолчанию боевой)');
    console.log('  --telemetry <путь> файл телеметрии (по умолчанию боевой)');
    console.log('  --curve <путь>    документ кривой  (по умолчанию боевой)');
    console.log('  --journal <путь>  журнал развёртки (по умолчанию боевой)');
    console.log('');
    console.log('  ⚠️ БЕЗ ФЛАГОВ ЭТО СЕРВЕР: он НЕ ВЕРНЁТ управление. Для разового вопроса —');
    console.log('     `--help` или `--selftest`; чтобы убрать за собой — `--close`.');
    return 0;
  }
