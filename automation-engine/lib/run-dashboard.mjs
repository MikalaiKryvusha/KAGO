...
export async function selfTest() {
  ...
  //     BF. поменять 20 секунд подачи на другое число                 → блок «ДВАДЦАТЬ СЕКУНД»
  {
    ...
    check('ЧАСЫ ПРОГОНА ВЕДЁТ КАДР, а не приход пульса — только так они идут в такт стенным',
      /function renderRunClock\(\{\s*frozen\s*\}\)/.test(page)
        && /^\s*renderRunClock\(\{ frozen \}\);/m.test(page),
      'нет функции часов или её не зовёт кадр с вердиктом frozen (вызов, а не упоминание)');
