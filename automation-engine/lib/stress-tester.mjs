...
export function runBurst({ name, exe = null, args = [], sustainSeconds = 0, run = null, progressFile = null } = {}) {
  ...
  // ⚡ СВИДЕТЕЛЬ СТРОКИ ПРОЖИГА (`bugs/132`). 09.09 машина зависла, а вход 2 предохранителя не имел
  // ни одного удара за весь прогон — и установить, дошёл ли флаг до горна, оказалось НЕЧЕМ: строка
  // запуска нигде не печаталась. Каждое звено по отдельности проверялось живыми опытами и работало,
  // а прогон всё равно молчал. Поэтому первая строка каждого прогона печатается вслух: прибор
  // обязан показывать, ЧТО он сделал, а не то, что должен был.
  if (!runBurst.__witnessed) {
    runBurst.__witnessed = true;
    console.log(`ПЕРВЫЙ ПРОЖИГ: ${binary} ${argv.join(' ')}`);
    console.log(progressFile
      ? `   вход 2: файл сердцебиения ПРОВЕДЁН до горна — ${progressFile}`
      : '   🔴 вход 2: файл сердцебиения ДО ГОРНА НЕ ДОШЁЛ — предохранитель будет слеп к остановке работы');
  }
