...
// 🔴 `bugs/103` (2026-09-04): здесь стояло голое слово `'АВАРИЙНАЯ'`, и после переименования
// предохранителя в САЗ (`researches/32`, сессия 73) оно совпадало со строкой ПОДЪЁМА защиты —
// «⚡ АВАРИЙНАЯ ЗАЩИТА (САЗ, двойник): … НЕ ВЗВЕДЕНА (только наблюдение)» — на КАЖДОЙ карте полигона,
// при коде 0: И2 краснел на здоровом прогоне (3 из 3 карт 04.09). Маркер — это ОБЪЯВЛЕНИЕ СТОПА, а
// не упоминание защиты: живая строка срабатывания — «СРАБОТАЛА АВАРИЙНАЯ ЗАЩИТА (САЗ) на …».
export const DEFAULT_STOP_MARKERS = Object.freeze([
  'ОСТАНОВЛЕН', 'СТОП', 'ПРЕДОХРАНИТЕЛЬ СРАБОТАЛ', 'ТРИП', 'ОТВЕРГ ЗАПИСЬ', 'СРАБОТАЛА АВАРИЙНАЯ ЗАЩИТА',
]);
...
function cmdSelftest() {
  ...
  // ─── `bugs/103`: УПОМИНАНИЕ ЗАЩИТЫ — НЕ ОБЪЯВЛЕНИЕ СТОПА ─────────────────────────────────────
  // Строка подъёма САЗ печатается на КАЖДОМ прогоне; маркер, ловивший её, красил здоровый полигон
  // 3 из 3 карт (04.09). Обе стороны: баннер при коде 0 — зелёный; живая строка срабатывания при
  // коде 0 — красный. Мутация: вернуть голое `'АВАРИЙНАЯ'` в список → первый блок краснеет.
  const bannerEv = { ...healthy, exitCode: 0,
    reportLines: ['⚡ АВАРИЙНАЯ ЗАЩИТА (САЗ, двойник): pid 32164, НЕ ВЗВЕДЕНА (только наблюдение); протокол в песочнице …'] };
  ok('И2 МОЛЧИТ на строке ПОДЪЁМА защиты при коде 0 (bugs/103): упоминание САЗ — не объявление стопа',
    guardStopIsNamedAndExitCodeAgrees(bannerEv).ok === true);
  const tripEv = { ...healthy, exitCode: 0,
    reportLines: ['  СРАБОТАЛА АВАРИЙНАЯ ЗАЩИТА (САЗ) на 2700 МГц / 995 мВ (срабатываний за ступень: 4). МАШИНА ЖИВА'] };
  ok('И2 КРАСНЕЕТ на живой строке СРАБАТЫВАНИЯ защиты при коде 0 — объявление стопа поймано в новой редакции маркера',
    guardStopIsNamedAndExitCodeAgrees(tripEv).ok === false);
