...
async function main(argv) {
  ...
  // ⚠️ ЭТА СТРОКА ВРАЛА ПЯТЬ ДНЕЙ (`bugs/26`, второй симптом): развёртка НЕ закрепляет частоту на
  // каждой ступени. Выше пола потолка держит выпрямленная кривая и замка нет вовсе; закрепление
  // остаётся держателем только ниже пола (факт 38, R11). Теперь стенд слушается той же формы, что
  // и живой прогон, и говорит об этом то же самое.
  console.log('  ФОРМА:     ту же, что и живой прогон: выше пола потолка держит ВЫПРЯМЛЕННАЯ КРИВАЯ и');
  console.log('             замка нет; ниже пола держатель — ЗАКРЕПЛЕНИЕ частоты (факт 38, R11).');
  console.log('             Заказать частоту у карты нельзя — под кривой она проседает, и замер');
  console.log('             ложится в строку ВЫДАННОЙ частоты (слово владельца 2026-08-23).');
