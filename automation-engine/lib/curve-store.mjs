...
function cmdSelftest() {
  ...
  console.log('\n— ЧТО ВООБЩЕ МОЖЕТ ПРОТИВОРЕЧИТЬ ЗАМЕРУ —');
  {
    ...
    ok('ДВА НАСТОЯЩИХ ЗАМЕРА ПРОТИВОРЕЧАТ ПО-ПРЕЖНЕМУ, и отказ называет ОБЕ частоты',
      ...
      // ⚠️ ЗДЕСЬ СТОЯЛ ТРЕТИЙ АРГУМЕНТ `true` — след ЧУЖОГО ДИАЛЕКТА `ok` (`bugs/106`). У этой
      // батареи подпись `(имя, условие, ПОДРОБНОСТЬ)`, и `true` уезжало в поле, которое печатается
      // только у красных. Сам блок краснеть умел — условие настоящее; но форма на один шаг от той,
      // что 05.09 обнаружилась в двух строках `fuse.mjs` и держала их зелёными структурно. Снято,
      // чтобы сторож диалекта стартовал с нуля, а не с «одного известного исключения».
  }
