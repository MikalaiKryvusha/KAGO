...
export const TRAPS = Object.freeze([
  ...
  {
    name: 'T9_edge_within_one_session',
    klass: 'B',
    traps: 'край лежит В ПРЕДЕЛАХ одной сессии спуска от стока — то есть движок обязан НАЙТИ его '
      + 'и закрыть строку, а не упереться в потолок сессии. Ровно этого не умеет ни одна другая '
      + 'ловушка: у T1 запас 2,9 мВ (не проходит НИЧЕГО), у остальных край в сотнях мВ вниз, '
      + 'куда потолок сессии 30 мВ не пускает.',
    mustDo: 'закрыть строку РОВНО на политике запаса над последней прошедшей ступенью: '
      + 'глубочайшая прошедшая — ближайшее значение сетки ВЫШЕ края, закрытая строка — на один '
      + 'шаг сетки выше неё (MARGIN_STEPS_ABOVE_LAST_STABLE)',
    otherwise: 'движок отгружает последнюю прошедшую БЕЗ запаса (край на карте владельца поедет '
      + 'от нагрева, и строка станет отказом на первом же тёплом вечере) — либо кладёт запас '
      + 'наугад, и тогда числа документа не выводятся из политики вовсе',
    judgedBy: 'E3-AC2 (`plans/16`), по СУЩЕСТВУ действующей политики, а не по устаревшей букве '
      + 'критерия: критерий писался при правиле «край + 10 мВ», а действует '
      + '`MARGIN_STEPS_ABOVE_LAST_STABLE = 1` — один шаг сетки над последней ПРОШЕДШЕЙ',
    // 🔴 ЖДЁТ ФИКСТУРЫ С ЗАКРЕПЛЕНИЕМ ЧАСТОТЫ, И ПРИЧИНА СНЯТА ЗАМЕРОМ, А НЕ НАЗВАНА СО СЛОВ.
    //
    // Прогон 2026-09-07 через `runSweep` на этой самой карте: край 1022,7 мВ при стоке 1045, и
    // спуск НЕ ВСТРЕТИЛ его ни разу — на 1020 мВ карта выдала 2835 МГц и ПРОШЛА, на 810 выдала
    // 2062 и прошла, на 450 выдала 180 и прошла; строка закрылась `lever-limited` на 450 мВ.
    // Причина не в карте и не в якоре: край привязан к ЧАСТОТЕ, а незакреплённая частота падает
    // вместе с напряжением, и спуск едет по краю сверху, не пересекая его.
    //
    // Значит утверждение AC2 требует прогона с ЗАКРЕПЛЁННОЙ частотой (`pinMhz`), которого у
    // `runSweep` стенда сегодня нет наружу. Держать ряд красным между этим знанием и фикстурой
    // нечестно перед следующей сессией; молча зазеленить — нечестно перед документом.
    openPhase: 'фикстура с закреплением частоты (`pinMhz`) — без замка спуск едет вниз вместе с '
      + 'частотой и края не встречает; замерено 2026-09-07, числа в комментарии ловушки',
    // 🔴 ПОЧЕМУ 22 мВ, А НЕ КРУГЛОЕ ЧИСЛО. Край обязан попасть МЕЖДУ узлами сетки: карта, чьи края
    // сидят ровно на ступенях, отвергается собственным валидатором (и правильно — тогда «прошло»
    // и «не прошло» решает округление, а не кремний). При стоке 1045 на 2842 МГц якорь 22 даёт
    // край ≈1023, между узлами 1020 и 1025. Спуск грубым шагом 25 мВ уходит на 1020 — НИЖЕ края,
    // отказ; уточнение шагом 5 мВ находит проход на 1025; политика запаса закрывает строку на 1030.
    // Все три числа выводятся из карты и политики, а не назначены блоком.
    //
    // Шум держится МАЛЫМ намеренно: при амплитуде в единицы мВ край гулял бы через узел сетки, и
    // утверждение о «ровно одном шаге» проверяло бы генератор шума, а не движок.
    fiction: {
      anchors: [{ mhz: 180, belowStockMv: 22 }, { mhz: 3090, belowStockMv: 22 }],
      noiseAmplitudeMv: 0.3, driftMaxMv: 0.5, noiseSeed: 20260907,
    },
  },
]);
...
async function main() {
  ...
  if (argv.includes('--derive-traps')) {
    ...
    console.log(`\n${PROVABILITY_LINE}`);
    // Здесь карт МНОГО и они СОЗДАЮТСЯ этой же командой — своего зерна и своего профиля у отчёта
    // нет, есть коммит, который фиксирует и список ловушек, и генератор.
