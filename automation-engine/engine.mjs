...
export async function runRung({
  ...
  if (fuseTripsFn && tripsBefore !== null) {
    ...
    if (Number.isFinite(tripsNow) && tripsNow > tripsBefore) {
      ...
      record.decidedBy = 'предохранитель';
      record.rescuedByFuse = true;
      record.stallDepthMv = Number.isFinite(depthMv) ? depthMv : null;
      const rescueWhy = `СПАСЕНО ПРЕДОХРАНИТЕЛЕМ на ${clockMhz} МГц / ${measuredMv} мВ `
        + `(трипов за ступень: ${tripsNow - tripsBefore}). МАШИНА ЖИВА — руки сняли нагрузку и вернули `
        + 'карту к стоку. Для ЧАСТОТЫ это КРАЙ: лаг телеметрии есть отказ независимо от того, шёл ли '
...
export async function sweepFrequency({
  ...
  for (const rung of rungs) {
    ...
    if (r?.outcome === 'hung' && r?.rescuedByFuse === true) {
      ...
      //
      // 🔴 ПОЧЕМУ ЭТО СЕГОДНЯ НЕ ИСПОЛНЯЕТСЯ, А ОТКАЗЫВАЕТ ВСЛУХ. Поиск шагом 5 мВ — это НОВЫЕ
      // ПРОЖИГИ, а судья предохранителя после своего трипа ВЫХОДИТ (одна из двух причин, ради
      // которых существует `plans/81`), и прогон без предохранителя запрещён той же фазой эпика.
      // Жечь здесь значило бы исполнить половину его приказа ценой нарушения другого его правила.
      // Отказ назван его словами и снимается шагом Ш5 плана 81 (судья переживает трип) — не раньше.
      if (lastPass === null) {
        out.halted = true;
        out.rescueNeedsFineSearch = true;
        out.why = `СПАСЕНИЕ НА ПЕРВОМ ЖЕ ШАГЕ ОТ СТОКА на ${frequencyMhz} МГц: прошедших прожиг ступеней `
          + 'выше нет, и по слову владельца это значит, что СТОК УЖЕ БЛИЗКО К КРАЮ — частоту надо '
          + `переискать шагом ${minStepMv} мВ. Сейчас это НЕ делается: поиск требует новых прожигов, а `
          + 'судья предохранителя после трипа вышел, и прогон без предохранителя запрещён. Частота '
          + 'ждёт прогона с судьёй, переживающим трип (plans/81 Ш5). Строка в документ НЕ пишется';
  ...
  return (async () => {
    ...
    try {
      ...
      //   FC. поставить блок ПОСЛЕ пульса                      → «СПАСЕНИЕ РЕШАЕТ РАНЬШЕ ПУЛЬСА»
      {
        ...
        ok('СПАСЕНИЕ ОТМЕНЯЕТ PASS: прожиг после обнуления кривой ничего не доказывает (bugs/88)',
          ...
          [config.VERDICT.HUNG, 'hung', 'предохранитель', true]);
        ...
        ok('СПАСЕНИЕ РЕШАЕТ РАНЬШЕ ПУЛЬСА: оно отменяет сам прожиг, а пульс — только его вердикт',
          withPulse.decidedBy, 'предохранитель');
    ...
    //   FF. писать строку и при пустом `lastPass`            → «СПАСЕНИЕ НА ПЕРВОМ ШАГЕ — НЕ СТРОКА»
    {
      ...
      const rescueAt = (mv) => async ({ voltageMv }) => (voltageMv > mv
        ...
        : { outcome: 'hung', verdict: config.VERDICT.HUNG, rescuedByFuse: true, decidedBy: 'предохранитель', why: 'СПАСЕНО ПРЕДОХРАНИТЕЛЕМ' });
      ...
      // Спасение на ПЕРВОЙ ЖЕ ступени: прошедших нет. Слово владельца — это значит, что сток уже
      // близко к краю, и частоту надо переискать шагом 5 мВ; сегодня это требует новых прожигов при
      // вышедшем судье, поэтому частота ОТКАЗЫВАЕТ вслух и строку НЕ пишет.
      const first = await freqOK({ runRungFn: rescueAt(1020) });
      ok('СПАСЕНИЕ НА ПЕРВОМ ШАГЕ — НЕ СТРОКА: прошедших нет, отгружать нечего, и это сказано вслух',
        [first.verdict, first.halted, first.rescueNeedsFineSearch ?? false],
        [null, true, true]);
      ok('ОТКАЗ НАЗЫВАЕТ ПРАВИЛО ВЛАДЕЛЬЦА И НЕДОСТАЮЩЕЕ УСЛОВИЕ, а не «не получилось»',
        /СТОК УЖЕ БЛИЗКО К КРАЮ/u.test(first.why ?? '') && /предохранител/u.test(first.why ?? ''), true);
    }
...
async function mainSweep(argv, arg) {
  ...
  try {
    ...
    fuseTripsFn: () => {
      try { return fuseMod.tripCount(readFileSync(fuseJournalPath, 'utf8').split(/\r?\n/u)); } catch { return 0; }
    },
