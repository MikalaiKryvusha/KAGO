...
// ⚡ `bugs/110`: пишущие в карту режимы движка — один список на всех читателей. Разбор — в модуле.
import { ENGINE_CARD_WRITING_MODES } from './lib/entry-points.mjs';
...
export function nextRungFrom({
  ...
  // ─── РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-08-31, `interviews/021`, ДОСЛОВНО: «шагаем теми шагами, какие есть» ─
  //
  // Развилка: сетка карты местами идёт через 10 мВ (32 зазора из 127 точек — 830 · 855 · 880 · 805
  // отсутствуют), и на глубине, где политика владельца просит 5 мВ, мельче взять НЕЧЕГО. Вариантов
  // было три — шагнуть на то, что есть · встать и закрыть частоту доказанным · спрашивать владельца
  // на каждой дыре (в полосе 2887…2745 таких мест одиннадцать на восьми частотах). Выбран ПЕРВЫЙ.
  // Вопрос задан с ценой каждого варианта и с рекомендацией; решение его, не агента.
  //
  // ⚠️ ЧТО ЭТО НЕ ОТМЕНЯЕТ: стену `bugs/03` (`maxStepMv`) ниже. Вынужденный сеткой шаг проходит
  // через неё, а не мимо: 10 мВ вдвое меньше разрешённых 35, а шаг шире стены отказывает и здесь.
  //
 ...
 * RECON:    researches/29 — перепись архива `runs/sweep/journal.jsonl`: 5 подстоев за всю
 *           историю, все в день смерти машины, на здоровых прогонах НОЛЬ. Веб-разведки о порогах накопителей НЕ проводилось
...
/**
 * ТИКЕТЫ, ПОКА ОТКРЫТЫЕ КОТОРЫХ ВЛАДЕЛЕЦ ЗАПРЕТИЛ ЖИВОЙ ПРОГОН (слово 2026-08-31).
 *
 * Оба — про то, что 30 августа НЕ остановило смерть машины: у канала драйвера нет голоса, хотя он
 * кричал 110 секунд, и шаг спуска меряется от текущего положения вместо стока (последний шаг перед
 * зависанием был 10 мВ при политике 5).
 *
 * Список — данные, а не логика: чтобы снять запрет, тикету ставят тег `DONE`, а не правят код.
 */
export const LIVE_RUN_BLOCKERS = [
  { id: '77', why: 'шаг спуска меряется не от стока — последний шаг перед смертью был 10 мВ при политике 5' },
  { id: '79', why: 'у канала драйвера нет голоса — он кричал 110 секунд, и это время было потрачено впустую' },
];

/**
 * @guard live-run-blockers
 * THREAT:         живой прогон на карте владельца, пока открыты тикеты, названные им как условие
 *                 допуска, — то есть повторение вечера 30.08 без починки того, что его не остановило
 * PROVED-AGAINST: семь блоков самопроверки на подставленном каталоге: оба открыты · один закрыт ·
 *                 оба закрыты (запрет снимается САМ) · тикета нет вовсе · каталог не читается ·
 *                 `DONE` у чужого тикета · адресность списка
 * GAP:            🔴 доказан на ИМЕНАХ ФАЙЛОВ, а не на живом прогоне. Сухой прогон до этого места
 *                 не доходит (он в карту не пишет), а единственный путь, где гейт срабатывает, —
 *                 тот самый живой прогон, который он запрещает. Значит проверено, что функция
 *                 считает верно, и НЕ проверено, что вызов стоит там, где я думаю
 * ON-REAL-PATH:   ✅ 2026-08-31 08:2x — УВИДЕН. Живой прогон полосы 2145…2000 МГц на карте владельца
 *                 напечатал «ЗАПРЕТ ВЛАДЕЛЬЦА (bugs/77, bugs/79): снят — оба тикета закрыты» ПЕРЕД
 *                 первой записью в карту (`runs/sweep-session70.log`). Зазор выше закрыт
 *                 НАБЛЮДЕНИЕМ, а не рассуждением: вызов стоит там, где я думал
 *
 * Какие из запрещающих тикетов ещё открыты. Открытость определяется ИМЕНЕМ ФАЙЛА по общей
 * конвенции проекта (`NN_DONE_*` — закрыт), поэтому гейт снимается сам при закрытии тикета.
 *
 * ⚠️ ЕСЛИ КАТАЛОГ `bugs/` НЕ ЧИТАЕТСЯ — гейт считает тикеты ОТКРЫТЫМИ. Направление отказа выбрано
 * в сторону безопасности: сломанный сторож на пути к карте владельца обязан запрещать, а не
 * разрешать. Это обратное правилу безопасного режима ниже — и разница намеренная: там сломанный
 * сторож защищает ДИСКИ и не имеет отношения к карте, здесь он защищает саму машину.
 */
export function liveRunBlockers(bugsDir = null, readdirFn = null) {
  const dir = bugsDir ?? join(dirname(fileURLToPath(import.meta.url)), '..', 'bugs');
  let names;
  try { names = (readdirFn ?? readdirSync)(dir); } catch { return LIVE_RUN_BLOCKERS; }
  return LIVE_RUN_BLOCKERS.filter((b) => {
    const file = names.find((n) => n.startsWith(`${b.id}_`));
    return !file || !file.includes('_DONE_');
  });
}
 ...
 */
export const STALL_KIND = Object.freeze({
  pulse: 'подстой пульса',
  driver: 'крик драйвера',
  // 🔴 ТРЕТЬЕГО РОДА ЗДЕСЬ НЕТ, И ЕГО ОТСУТСТВИЕ — РЕШЕНИЕ ВЛАДЕЛЬЦА, А НЕ НЕДОДЕЛКА.
  //
  // 2026-08-31 утром я завёл здесь `rescue: 'срабатывание аварийной защиты'` (Ш6 `plans/81`) — то
  // есть провёл срабатывания в накопитель, ТРИ записи которого ОСТАНАВЛИВАЮТ полосу. В тот же день
  // владелец это отменил дословно (`interviews/024` = E, 17:33): *«Сколько угодно спасений
  // разрешаю. KAGO — это автоматизированный движок тюнинга видеокарты. В идеале он должен работать
  // по принципу "Запустил и забыл — оно само все сделало"»*.
  //
  // Спасение — это УСПЕХ защиты, а не довод о том, что машине плохо. Подстой пульса и крик драйвера
  // говорят «прибор наблюдает неладное»; срабатывание говорит «неладное СЛУЧИЛОСЬ и уже устранено,
  // машина жива, защита вернулась на пост». Складывать это в один счёт с двумя наблюдениями значит
  // копить успехи как предупреждения — и вставать посреди ночи ровно тогда, когда защита работает
  // лучше всего. Череду спасений лечат ПРИЧИНОЙ (`bugs/86`, `bugs/92`), а не счётчиком.
  //
  // Спасения видны в `report.rescues` и в сводке отдельной строкой (P81-AC6) — но в решение об
  // остановке не входят вовсе. Возвращать сюда `rescue` — значит возвращать отменённое: блок
  // батареи «спасения НЕ доезжают до накопителя» краснеет ровно на такую правку.
});

/**
 * КТО ВЫНЕС ВЕРДИКТ СТУПЕНИ, КОГДА ЕЁ ПРЕРВАЛА ЗАЩИТА — одно имя в одном месте.
 *
 * Строка стояла литералом в ПЯТИ местах (ступень, три блока набора, сборка двойника) — пара
 * «правда ↔ зеркало» впятеро: переименование в одном месте оставило бы накопитель молча слепым,
 * ровно как в `bugs/84`, где он читал поле, которого никто не выставляет. Пару лучше УБРАТЬ, чем
 * за ней следить (`PHILOSOPHY.md` → DRY).
 */
export const DECIDED_BY_FUSE = 'аварийная защита';
...
export async function runRung({
  ...
  driverEventsFn = null,
  // ⚡ СПАСЕНИЕ ПРЕДОХРАНИТЕЛЯ КАК ВХОД СТУПЕНИ (`bugs/88`, решение владельца `interviews/023`):
  // `() => number` — сколько трипов записано в журнале судьи ПРЯМО СЕЙЧАС. Тот же шов и та же
  // необязательность, что у пульса и голоса драйвера: не передали — прежнее поведение.
  //
  // 🔴 СЧЁТ, А НЕ ВРЕМЯ, и это не педантизм. Штамп трипа ставит СУДЬЯ своими часами, штамп ступени
  // — ДВИЖОК своими; сравнивать такие концы это ловушка [[EXP-0207]], за которую проект уже
  // заплатил целой фазой эпика. Ступень спрашивает счёт ДО работы и ПОСЛЕ — разность от часов не
  // зависит вовсе. Ровно тем же способом Ш2 плана 81 отбирает расписки руки 2.
  fuseTripsFn = null,
  runStepFn,
  buildVector = null,
  ...
  // ⚡ И ЗДЕСЬ ЖЕ — СКОЛЬКО ТРИПОВ БЫЛО ДО ЭТОЙ СТУПЕНИ (`bugs/88`). Считается В ТОЙ ЖЕ точке, что
  // открывается окно пульса, и по той же причине: всё, что случится дальше, принадлежит ЭТОЙ
  // ступени. Прибор недоступен — `null`, и вход не голосует вовсе (R4b: прибор без данных молчит).
  const tripsBefore = fuseTripsFn ? fuseTripsFn() : null;
  let atom = null;
  const attempts = [];
  ...
  // ---- 8b-fuse. СПАСЕНИЕ ПРЕДОХРАНИТЕЛЯ — И ОНО РЕШАЕТ РАНЬШЕ ВСЕХ (`bugs/88`, `interviews/023`).
  //
  // РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-08-31 14:03, дословно: *«Мы знаем на какой частоте на каком напряжении
  // был лаг, и машина чуть не зависла — от этого напряжения берем выше ближайшее прошедшее прожиг
  // + еще один шаг… эта частота закрывается как протюненая — и просто дальше по плану тюнинг
  // прогон продолжается со следующей частоты по плану»*.
  //
  // 🔴 ПОЧЕМУ ЭТОТ ВХОД ПЕРВЫЙ, А НЕ ТРЕТИЙ. Пульс и голос драйвера ОТМЕНЯЮТ ВЕРДИКТ прожига;
  // спасение отменяет САМ ПРОЖИГ. Рука 2 возвращает карту к стоку, и прожиг, начавшийся после,
  // шёл на ЗАВОДСКОЙ кривой — измерено на двойнике 2026-08-31: ненулевых смещений 0, а ступень
  // закрывалась `PASS` и писала андервольт, которого не было (`bugs/88`). Значит вердикт оракула
  // здесь не «сомнителен», а НЕ ОТНОСИТСЯ К ДЕЛУ, и порядок блоков — это приоритет.
  //
  // ⚠️ И ЭТО НЕ «ЗАВИСАНИЕ», А СПАСЕНИЕ, поэтому текст говорит про машину отдельно от частоты —
  // слово владельца от 2026-08-28: *«спасение меняет исход для МАШИНЫ (жива), но не исход для
  // ЧАСТОТЫ (край найден, спуск на эту ступень закрыт)»*. Исход `hung` взят не как описание
  // события, а как МЕХАНИЗМ: он уже кормит пол зависания (R18) и тормоз «двух подряд» без единой
  // правки, а `hungBy` несёт настоящую причину и отличает спасение от смерти машины в разборе.
  if (fuseTripsFn && tripsBefore !== null) {
    const tripsNow = fuseTripsFn();
    if (Number.isFinite(tripsNow) && tripsNow > tripsBefore) {
      record.verdict = config.VERDICT.HUNG;
      record.decidedBy = DECIDED_BY_FUSE;
      record.rescuedByFuse = true;
      record.stallDepthMv = Number.isFinite(depthMv) ? depthMv : null;
      const rescueWhy = `СРАБОТАЛА АВАРИЙНАЯ ЗАЩИТА (САЗ) на ${clockMhz} МГц / ${measuredMv} мВ `
        + `(срабатываний за ступень: ${tripsNow - tripsBefore}). МАШИНА ЖИВА — АВН снял нагрузку, ВЗН вернул `
        + 'карту на заводское напряжение. Для ЧАСТОТЫ это КРАЙ: лаг телеметрии есть отказ независимо от того, шёл ли '
        + 'в тот момент прожиг. Вердикт оракула этой ступени НЕ ОТНОСИТСЯ К ДЕЛУ — прожиг после '
        + 'спасения идёт по ЗАВОДСКОЙ кривой и ничего не доказывает';
      onEvent?.({ kind: 'fuse-rescue-rung', frequencyMhz: clockMhz, text: rescueWhy, trips: tripsNow - tripsBefore });
      return close({ ...record, outcome: 'hung', why: rescueWhy });
    }
  }
...
/**
 * НАПРЯЖЕНИЕ ОТГРУЗКИ ОТ ПОСЛЕДНЕЙ ПРОШЕДШЕЙ СТУПЕНИ — правило владельца в ОДНОМ месте.
 *
 * Его переанкеровка 2026-08-17: *«последняя стабильная до отказа точка (соседка отказа сверху)
 * + 5 мВ»*, и она же подтверждена 2026-08-31 (`interviews/023`): *«от этого напряжения берем выше
 * ближайшее прошедшее прожиг + еще один шаг»*. Запас спрашивается МИНИМАЛЬНЫМ шагом карты —
 * `marginAboveLastStableMv` БРОСАЕТ на более грубом, чтобы удвоенная подушка не написалась случайно.
 *
 * ⚠️ ВЫНЕСЕНО ФУНКЦИЕЙ, А НЕ СКОПИРОВАНО: с 2026-08-31 это правило зовут ДВА места — уточнение края
 * (`refineEdge`) и закрытие частоты по спасению предохранителя (`sweepFrequency`). Две копии одной
 * арифметики — пара «правда↔зеркало» ровно там, где её цена это отгружаемое напряжение на карте
 * владельца. Пару, которую можно УБРАТЬ, реестр канона предпочитает паре, за которой надо следить.
 *
 * @returns {{margin:object, wanted:number, shipMv:number|null}} `shipMv === null` — такого напряжения
 *   (или выше) в сетке карты нет; просить его не у кого, и выдумывать нельзя.
 */
export function shipVoltageFor({ voltageGridMv = [], lastPassMv = null, minStepMv = 5 } = {}) {
  const grid = [...voltageGridMv].sort((a, b) => a - b);
  const margin = marginAboveLastStableMv(minStepMv);
  const wanted = lastPassMv + margin.millivolts;
  // Подтягиваем ВВЕРХ к напряжению, которое у карты есть: округление в сторону БОЛЬШЕГО запаса,
  // потому что альтернатива — отгрузить ближе к измеренному отказу, чем позволяет политика владельца.
  const shipMv = grid.filter((v) => v >= wanted).sort((a, b) => a - b)[0] ?? null;
  return { margin, wanted, shipMv };
}
...
export async function refineEdge({
  ...
  const { margin, wanted, shipMv } = shipVoltageFor({ voltageGridMv: grid, lastPassMv: pass, minStepMv });
  if (shipMv === null) {
    return {
...
export async function sweepFrequency({
  ...
  zones = config.DESCENT_ZONES,
  onEvent = null,
  // ⚡ ВНЕШНИЙ СТОП, СПРАШИВАЕМЫЙ ПЕРЕД КАЖДОЙ СТУПЕНЬЮ (`bugs/101`, находка 2). Тот же крючок, что
  // `sweepRange` зовёт перед каждой ЧАСТОТОЙ; здесь он опускается на ступень. Возвращает `null`
  // («жги») либо `{ why, rescue }` — тогда спуск кончается ДО того, как ступень тронет карту.
  stopWhen = null,
} = {}) {
  if (typeof runRungFn !== 'function') {
  ...
  for (const rung of rungs) {
    // ─── ЗАЩИТА СПРАШИВАЕТСЯ ПЕРЕД КАЖДОЙ СТУПЕНЬЮ, А НЕ ТОЛЬКО ПЕРЕД ЧАСТОТОЙ (`bugs/101`) ────────
    //
    // Оплачено 04.09 10:56: судья вышел кодом 2 (перевзведение отказано) в 10:56:26, а `sweepRange`
    // спрашивает `stopWhen` раз на частоту — и пять ступеней 2700 МГц (seq 857…861, 970 → 890 мВ)
    // прожглись БЕЗ защиты на посту; стоп пришёл «ПЕРЕД 2685 МГц». Правило фазы 5 (`plans/58`) —
    // «прожиг без взведённого предохранителя запрещён» — действует на СТУПЕНИ, потому что ступень
    // и есть то, что тратит карту; сторож, проверяемый на частоте, на ступени отсутствует.
    //
    // Что делается со спуском: прошедшие ступени — настоящие прожиги с вердиктом оракула, они не
    // выбрасываются — частота закрывается `cut-short` на последней прошедшей, ровно той формой, что
    // у остановок пути записи ниже; прошедших нет — спуск не начат, строка не пишется. Полосу
    // останавливает ТОТ ЖЕ крючок перед следующей частотой: решение одно, копий нет (`plans/81` Ш5).
    if (stopWhen) {
      const ext = await stopWhen();
      if (ext) {
        out.externalStop = ext;
        if (lastPass !== null) {
          out.verdict = 'cut-short';
          out.voltageMv = lastPass;
          out.cutShortBy = ext.rescue === true ? 'fuse-rescue' : 'external-stop';
          out.provenBy = `ПОСЛЕДНЯЯ ПРОШЕДШАЯ ступень ${lastPass} мВ — прожиг выдержан на ней самой. `
            + `Спуск ниже прерван ВНЕШНИМ СТОПОМ до ступени ${rung.mv} мВ: ${ext.why}. КРАЙ НЕ НАЙДЕН`;
          out.why = `СПУСК ПРЕРВАН на ${frequencyMhz} МГц ниже ${lastPass} мВ внешним стопом, доказанное сохранено: ${ext.why}`;
        } else {
          out.halted = true;
          out.why = `СПУСК на ${frequencyMhz} МГц ОСТАНОВЛЕН до ступени ${rung.mv} мВ внешним стопом: ${ext.why}`;
        }
        say('external-stop', out.why, { rescue: ext.rescue === true, rungMv: rung.mv });
        return withDelivered();
      }
    }
    ...
    // ─── СПАСЕНИЕ ПРЕДОХРАНИТЕЛЯ ЗАКРЫВАЕТ ЧАСТОТУ — решение владельца (`interviews/023`) ─────────
    //
    // Его слова 2026-08-31 14:03, дословно: *«от этого напряжения берем выше ближайшее прошедшее
    // прожиг + еще один шаг. Фиксируем эту точку в нашу тюнинг кривую… эта частота закрывается как
    // протюненая — и просто дальше по плану тюнинг прогон продолжается со следующей частоты»*.
    //
    // ⚠️ БЕЗ УТОЧНЕНИЯ, И ЭТО ЧАСТЬ ЕГО ЖЕ ПРИКАЗА. Обычный отказ уточняется шагом 5 мВ до
    // применения запаса — но уточнение это НОВЫЕ ПРОЖИГИ на той самой частоте, на которой машина
    // только что чуть не умерла, а он сказал «дальше по плану со СЛЕДУЮЩЕЙ частоты». Поэтому
    // отгружается ПОСЛЕДНЯЯ ПРОШЕДШАЯ + один шаг сетки, одной и той же функцией, что и везде
    // (`shipVoltageFor` — правило живёт в одном месте).
    if (r?.outcome === 'hung' && r?.rescuedByFuse === true) {
      // 🔴 ПОЛЕ СТАВИТСЯ ЗДЕСЬ, А НЕ НИЖЕ, И ЭТО НАХОДКА Ш6, А НЕ ПЕРЕСТАНОВКА СТРОК.
      //
      // `out.hungBy` живёт ниже по телу цикла — и обе ветки спасения выходят РАНЬШЕ него (`return`
      // на пропуске частоты и на закрытии краем). То есть накопитель предупреждений не мог получить
      // от спасения НИ ОДНОЙ записи, сколько бы источников в него ни вписали: читатель ждал поля,
      // которого этот путь не выставляет. Ровно класс `bugs/84`, найденный там же мутацией — и
      // здесь он пойман не чтением, а КРАСНЫМ БЛОКОМ Ш6, который сперва показал пустой накопитель.
      //
      // Глубина от стока едет вместе с именем: без неё правило «два предупреждения У САМОГО СТОКА»
      // не отличит находку о машине от свойства глубокой точки (`bugs/80` Д2).
      out.hungBy = DECIDED_BY_FUSE;
      out.stallDepthMv = r?.stallDepthMv ?? null;
      // ─── СЛУЧАЙ, КОТОРЫЙ ВЛАДЕЛЕЦ ЗАКРЫЛ ОТДЕЛЬНО (его слово 2026-08-31, следом за решением) ───
      //
      // *«если так случиться, то значит, что найдена какая-то уникальная точка, которая дала отказы
      // на первом же шаге от стока — а это значит, что стоковое напряжение УЖЕ БЛИЗКО К КРАЮ.
      // Значит нужно переключаться для точки на минимальный шаг 5 мВ и искать край этим шагом»*.
      //
      // 🔴 ПОЧЕМУ ЭТО ВСЁ ЕЩЁ НЕ ИСПОЛНЯЕТСЯ, И ПОЧЕМУ ПРИЧИНА ТЕПЕРЬ ДРУГАЯ.
      //
      // ⚠️ ПРЕЖНЯЯ ПРИЧИНА УМЕРЛА 2026-08-31 ВМЕСТЕ С Ш5 И ЗАМЕНЕНА, А НЕ ОСТАВЛЕНА. Здесь стояло
      // «судья предохранителя после трипа ВЫХОДИТ, и прогон без предохранителя запрещён». После Ш5
      // защита своё срабатывание ПЕРЕЖИВАЕТ (репетиция 31.08: 9 срабатываний — 9 успешных АПВ),
      // то есть прежний довод стал ЛОЖЬЮ. Оставить его формулировкой значило бы напечатать причину,
      // которую никто не проверял, — ровно класс [[EXP-0206]]: прибор, объясняющий неверно, дороже
      // молчащего, потому что молчание заставляет посмотреть, а объяснение — поверить.
      //
      // ЧЕСТНАЯ ПРИЧИНА СЕГОДНЯ ОДНА: поиск шагом 5 мВ — это НОВАЯ ветка спуска, её кода ещё нет.
      // Не «нельзя», а «не построено», и это разные утверждения (EXP-0169). Ветка открыта шагом Ш5
      // и ждёт своей работы; жечь здесь наугад значило бы исполнить половину приказа владельца
      // выдумкой вместо второй половины.
      if (lastPass === null) {
        out.halted = true;
        out.rescueNeedsFineSearch = true;
        out.why = `СРАБОТАЛА ЗАЩИТА НА ПЕРВОМ ЖЕ ШАГЕ ОТ СТОКА на ${frequencyMhz} МГц: прошедших прожиг ступеней `
          + 'выше нет, и по слову владельца это значит, что СТОК УЖЕ БЛИЗКО К КРАЮ — частоту надо '
          + `переискать шагом ${minStepMv} мВ. Сейчас это НЕ делается по одной причине: ветка поиска `
          + 'минимальным шагом ещё НЕ ПОСТРОЕНА (защита срабатывание переживает — АПВ работает с '
          + '31.08, прежний запрет снят). Частота ПРОПУСКАЕТСЯ, строка в документ НЕ пишется — '
          + 'выдуманного напряжения здесь не будет';
        say('rescue-needs-fine-search', out.why, { frequencyMhz, minStepMv });
        return withDelivered();
      }
      const ship = shipVoltageFor({ voltageGridMv, lastPassMv: lastPass, minStepMv });
      if (ship.shipMv === null) {
        out.halted = true;
        out.why = `СПАСЕНИЕ на ${frequencyMhz} МГц: последняя прошедшая ${lastPass} мВ + запас `
          + `${ship.margin.millivolts} мВ = ${ship.wanted} мВ, а такого напряжения (или выше) в сетке карты `
          + 'нет — просить его не у кого, и выдумывать нельзя';
        return withDelivered();
      }
      out.verdict = 'edge-found';
      out.voltageMv = ship.shipMv;
      out.provenBy = `край УСТАНОВЛЕН СПАСЕНИЕМ ПРЕДОХРАНИТЕЛЯ (машина жива, ступень снята с карты), `
        + `а НЕ вердиктом оракула · отгружается ${ship.shipMv} мВ = ПОСЛЕДНЯЯ ПРОШЕДШАЯ ${lastPass} мВ `
        + `+ запас ${ship.margin.millivolts} мВ (${ship.margin.steps} шаг сетки по ${ship.margin.gridStepMv}) `
        + '· уточнение шагом сетки НЕ проводилось: владелец распорядился идти к следующей частоте';
      out.why = `КРАЙ НАЙДЕН на ${frequencyMhz} МГц ПО СПАСЕНИЮ ПРЕДОХРАНИТЕЛЯ: ${r.why}`;
      say('rescue-closed-edge', out.why, { frequencyMhz, shipMv: ship.shipMv, lastPassMv: lastPass });
      return withDelivered();
    }
...
export function overshootMarkFor(outcome, maxGapMv = config.VOLTAGE_GRID_MAX_GAP_MV) {
  ...
  if (!rung) return none;
  // ⚠️ ВТОРОЙ ПРИЗНАК, И ОН ПРО ПОВТОРЕНИЕ, А НЕ ПРО РАЗМЕР (`bugs/73`, замер по 784 ступеням).
  //
  // Порог выше отвечает на вопрос «это другая запись таблицы?» — и отвечает верно. Но пол карты
  // (`bugs/58`) узнаётся НЕ шириной промаха, а тем, что НЕСКОЛЬКО РАЗНЫХ заказов, идущих вниз,
  // обслужены ОДНИМ И ТЕМ ЖЕ напряжением: карта не округлила, она не пошла ниже. Ровно эту картину
  // владелец видел РИСУНКОМ 1 в `interviews/018` — восемь прожигов подряд, заказ 885, выдача 915.
  //
  // ПОЧЕМУ РАЗМЕР ЗДЕСЬ НЕ РАБОТАЕТ. Рычаг у нас в МЕГАГЕРЦАХ, поэтому набор напряжений, ДОСТИЖИМЫХ
  // для данной частоты, — подмножество сетки (слово владельца 2026-08-16: «ближайшее верхнее из тех,
  // которые карточка ПРЕДОСТАВЛЯЕТ»). Значит одиночный промах в одну ступень неразличим: он и пол, и
  // законная соседняя достижимая ступень. Различает только повторение.
  //
  // ЗАМЕРЕНО ПО БОЕВОМУ ЖУРНАЛУ: плато выдачи (≥2 разных заказа → одна выдача) — 35 штук; из них
  // 14 шире одной ступени сетки, то есть порог их и так метил, а 21 — не шире, и они уходили
  // неотмеченными. Среди этих 21 — ступень seq 792/793 от 2026-08-26 (заказы 895 → 890, выдача 895
  // оба раза), та самая, чей подъём отобрал 45 мВ у пятнадцати частот (`bugs/63`).
  //
  // ЧТО ЭТО НЕ ОТМЕНЯЕТ: правило владельца 2026-08-16 «одна ступень вверх — ПОПАДАНИЕ, мерой
  // становится выданное» в силе целиком. Оно отвечает на вопрос «какое число записать в строку»;
  // метка отвечает на другой — «вправе ли эта строка ТРЕБОВАТЬ столько же от соседей». Одиночное
  // попадание меткой по-прежнему не метится.
  // ⚠️ И СЧИТАЕТСЯ ОНО ДО РАННЕГО ВОЗВРАТА, А НЕ ПОСЛЕ — иначе плато не видно ВООБЩЕ. У плато первый
  // заказ обычно попадает ТОЧНО (895 → 895), а промахивается следующий (890 → 895); `find` берёт
  // первую ступень, промах выходит нулевым, и выход «не наш случай» съедал ровно ту форму, ради
  // которой признак заводится. Найдено собственным блоком П73-AC1 на первом же прогоне.
  const plateauOrders = new Set(
    (outcome?.rungs ?? [])
      .filter((r) => r?.voltageMv === served && Number.isFinite(r?.orderedMv))
      .map((r) => r.orderedMv),
  );
  const floorPlateau = plateauOrders.size >= 2;
  // МЕРОЙ ПРОМАХА У ПЛАТО СЛУЖИТ САМЫЙ ГЛУБОКИЙ ЗАКАЗ: именно до него карта отказалась идти, и
  // именно он говорит, сколько андервольта осталось неизмеренным.
  const orderedMv = floorPlateau ? Math.min(...plateauOrders) : rung.orderedMv;
  const overshootMv = served - orderedMv;
  if (overshootMv <= 0) return none;                 // exact hit, or served BELOW the order — not ours
  return {
    marked: overshootMv > maxGapMv || floorPlateau,  // ширина ИЛИ повторение
    overshootMv,
    orderedMv,
    servedMv: served,
    floorPlateau,                                    // чем именно помечено — читает свидетель
    plateauOrdersMv: floorPlateau ? [...plateauOrders].sort((a, b) => b - a) : null,
  };
}
...
export function overshootWitness(miss) {
  ...
  // ⚠️ ПРИЧИНА МЕТКИ НАЗЫВАЕТСЯ ТА, ПО КОТОРОЙ ОНА ПОСТАВЛЕНА (`bugs/73`). Их две, и они разные:
  // ширина промаха («другая запись таблицы») и ПЛАТО ВЫДАЧИ («карта не пошла ниже»). Печатать
  // первую причину там, где сработала вторая, — это `bugs/42` слово в слово: объяснение, которое
  // ничего не измеряет, живёт неделями, потому что читателю не за что зацепиться.
  if (miss.floorPlateau) {
    return ` · ⚠️ СНЯТО С ПОЛА КАРТЫ: заказы ${miss.plateauOrdersMv.join(' → ')} мВ обслужены ОДНИМ`
      + ` напряжением ${miss.servedMv} мВ — карта ниже не пошла (промах ${miss.overshootMv} мВ).`
      + ' Прожиг доказал выданное и НИЧЕГО не доказал про заказанное';
  }
...
export async function sweepRange({
  ...
  const report = {
    ...
    stalls: [],
    // ⚡ СПАСЕНИЯ — СВОЙ СПИСОК, А НЕ ФИЛЬТР НАД `stalls` (`bugs/90` в редакции владельца).
    //
    // Фильтр на месте вызова («класть в накопитель всё, кроме спасений») был отвергнут заранее и по
    // причине: следующий потребитель `stalls` про этот фильтр забудет, и отменённое поведение
    // вернётся через другую дверь — ровно так `bugs/67` вернулся через конвейер `tee`. Спасение
    // здесь НЕ предупреждение, поэтому у него отдельное место, а не отдельная ветка в чужом.
    //
    // Список ЧАСТОТ, а не срабатываний: этот путь проходит один раз на частоту, и назвать его
    // «число спасений» значило бы повторить ошибку `bugs/90` (шесть срабатываний уместились в две
    // частоты). Настоящее число срабатываний за прогон живёт в `report.fuseTrips` и снимается
    // счётчиком защиты, а не этим циклом.
    rescues: [],
    // СРАБАТЫВАНИЙ ЗАЩИТЫ ЗА ПРОГОН — разность счётчика защиты между началом и концом полосы.
    // `null` значит «счётчика не дали», а не «ноль»: не смочь посмотреть и посмотреть и не найти —
    // разные ответы (правило R4b), и сводка обязана их различать.
    fuseTrips: null,
  ...
  // ⚡ `interviews/027` Q1 = D — УСЛОВИЕ ОСТАНОВА ЧЕРЕДЫ СПАСЕНИЙ. Разбор — `bugs/108`, числа
  // владельца — `config.RESCUE_*`. Счётчик живёт ЗДЕСЬ, в полосе, а не в проводке живого пути:
  // «полоса не может продвинуться» — факт о полосе, и двойник обязан судиться тем же кодом, иначе
  // репетиция смерти доказывала бы другой механизм, чем работает вечером.
  let rescuesInARow = 0;
  let chainPausePending = null;
  let chainStopped = null;
  report.rescueChainLongest = 0;
  // Пауза и останов исполняются МЕЖДУ частотами, а не посреди учёта только что законченной.
  // Первая редакция ломала цикл сразу за спасением — и шестая частота не попадала ни в
  // `report.rescues`, ни в `skipped`: отчёт показывал 5 спасений там, где их было 6. Прогон,
  // потерявший запись о собственной последней частоте, врёт о том, на чём остановился.
  const applyChainStop = () => {
    report.ok = false;
    report.stoppedBy = 'rescue-chain';
    report.why = chainStopped.why;
    say('rescue-chain', chainStopped.why, chainStopped.detail);
    console.log(`  🛑 ${chainStopped.why}`);
  };
  for (const g of groups) {
    ...
    // ⚡ `await` (Ш5): у этого крючка появилось право ЖДАТЬ, а не только отвечать «стоп/не стоп» —
    // полоса обязана дождаться АПВ аварийной защиты, прежде чем начать следующую частоту. Крючок
    // остался ОДИН: заводить второй («подожди») значило бы развести решение по двум местам.
    // Синхронные реализации (все существующие) `await` не замечают — их поведение бит-в-бит прежнее.
    // ⚡ `interviews/027` Q1 = D — ПАУЗА И ОСТАНОВ ЧЕРЕДЫ, оба МЕЖДУ частотами. Порядок дословно по
    // слову владельца: «ещё секунда таймаут — и только потом закрываем прогон», то есть последняя
    // секунда даётся карте ДО объявления конца, а не после.
    if (chainPausePending) {
      const p = chainPausePending;
      chainPausePending = null;
      console.log(`  ⏸ ЧЕРЕДА СПАСЕНИЙ: ${p.rescuesInARow} подряд, и полоса не продвинулась ни на одну частоту — `
        + `пауза ${config.RESCUE_ROUND_PAUSE_MS} мс`
        + (p.last ? ' и прогон закрывается' : ` (раунд ${p.round} из ${config.RESCUE_ROUNDS}: даём карте ещё один шанс)`));
      say('rescue-round', `пауза ${config.RESCUE_ROUND_PAUSE_MS} мс после ${p.rescuesInARow} спасени(я/й) подряд`
        + (p.last ? ' — последняя перед закрытием прогона' : `, раунд ${p.round} из ${config.RESCUE_ROUNDS}`), p);
      await new Promise((r) => { setTimeout(r, config.RESCUE_ROUND_PAUSE_MS); });
    }
    if (chainStopped) { applyChainStop(); break; }
    const externalStop = stopWhen ? await stopWhen() : null;
    if (externalStop) {
    ...
    const outcome = await sweepFrequency({
      ...
      onEvent,
      shapeLadder,
      // ⚡ Тот же крючок, что спрашивается перед частотой выше, — теперь и перед каждой ступенью
      // (`bugs/101` находка 2). Одно решение, один читатель расписки судьи, две точки опроса.
      stopWhen,
      ...
      runRungFn: async (rungArgs) => {
        ...
        const r = await runRung({
          ...
          runStepFn, pulseWindowFn, driverEventsFn, fuseTripsFn, buildVector, chooseShape, demandPin, envelopeMhz,
    ...
    // ═══════════════════════════════════════════════════════════════════════════════════════════
    // ⚡ `interviews/027` Q1 = D — ЧЕРЕДА СПАСЕНИЙ, КОТОРАЯ НИЧЕГО НЕ ЗАКРЫВАЕТ, КОНЧАЕТСЯ САМА
    // ═══════════════════════════════════════════════════════════════════════════════════════════
    //
    // Слово владельца 2026-09-07: три спасения подряд → пауза 1 с → второй раунд из трёх → ещё 1 с
    // → прогон закрывается. Числа — в `config.RESCUE_*`, вместе с его цитатой; здесь механизм.
    //
    // 🔴 ПРИЗНАК СБРОСА — ПОЛОЖИТЕЛЬНЫЙ И ЗАМЕРЕННЫЙ, А НЕ ВЫВЕДЕННЫЙ ИЗ ОТСУТСТВИЯ СПАСЕНИЯ.
    // «Полоса продвинулась» = частота ЗАКРЫЛАСЬ настоящим вердиктом. Спасение не пишет в документ
    // ни строки («2700 МГц ПРОПУЩЕНА» на живом прогоне 07.09), а честный край пишет — вот разница,
    // которую владелец и назвал словами «между ними не сгорело ни одной ступени». Частота без
    // единой ступени (рычаг исчерпан, пол зависания, край уже зажат) продвижением НЕ считается:
    // обнулять счётчик там, где полоса стоит, значило бы гасить сторож ровно в его случае.
    //
    // ⚠️ ПОЧЕМУ НЕ ПО СТУПЕНЯМ, ХОТЯ ПЕРВАЯ РЕДАКЦИЯ СМОТРЕЛА ИМЕННО ТУДА: в `out.rungs` лежит
    // УРЕЗАННАЯ запись — `outcome`/`verdict` есть, а `decidedBy` до неё не доезжает. Признак
    // «ступень решена не защитой» был там всегда истиной, и правило оказалось МЁРТВЫМ КОДОМ:
    // фикстура шести подряд спасённых частот его не покрасила. Найдено отладочной печатью, а не
    // чтением, и записано здесь, чтобы следующий читатель не пошёл в `decidedBy` во второй раз.
    //
    // ⚠️ ЧАСТОТА, КОТОРАЯ ЗАКРЫЛАСЬ И ПОТОМ БЫЛА СПАСЕНА, даёт счёт 1, а не +1 к прежнему:
    // полоса продвинулась, и это спасение — первое в новой череде, а не продолжение старой.
    {
      const rescued = outcome?.hungBy === DECIDED_BY_FUSE;
      const advanced = outcome?.verdict !== null && outcome?.verdict !== undefined && outcome?.halted !== true;
      if (rescued) rescuesInARow = advanced ? 1 : rescuesInARow + 1;
      else if (advanced) rescuesInARow = 0;
      if (rescuesInARow > report.rescueChainLongest) report.rescueChainLongest = rescuesInARow;

      if (rescued && rescuesInARow > 0 && rescuesInARow % config.RESCUE_ROUND_SIZE === 0) {
        const round = rescuesInARow / config.RESCUE_ROUND_SIZE;
        const last = round >= config.RESCUE_ROUNDS;
        chainPausePending = { frequencyMhz: g.topMhz, rescuesInARow, round, rounds: config.RESCUE_ROUNDS, last };
        if (last) {
          // 🔴 ЭТО НЕ ЛИМИТ СПАСЕНИЙ (`interviews/024` = E не тронут) — это конец РАБОТЫ, которая
          // перестала быть работой. Причина называется словами, а не кодом: прогон, честно
          // кончившийся с названной причиной, полезнее прогона, который всю ночь красиво ползёт.
          chainStopped = {
            why: `ПОЛОСА ОСТАНОВЛЕНА: карта не оживает — ${rescuesInARow} спасени(я/й) подряд `
              + `(${config.RESCUE_ROUNDS} раунда по ${config.RESCUE_ROUND_SIZE}), и полоса не закрыла между ними ни одной частоты. `
              + 'Машина возвращена на сток аварийной защитой; всё закрытое до этого уже в документе кривой.',
            detail: { frequencyMhz: g.topMhz, rescuesInARow, rounds: config.RESCUE_ROUNDS },
          };
        }
      }
    }
    ...
    if (outcome.halted || outcome.verdict === null) {
      ...
      const LEDGER_SOURCES = {
        'пульс сэмплера': STALL_KIND.pulse,
        'канал драйвера': STALL_KIND.driver,
        // 🔴 АВАРИЙНОЙ ЗАЩИТЫ ЗДЕСЬ НЕТ — СМ. РАЗБОР У `STALL_KIND`. Ключ `[DECIDED_BY_FUSE]` стоял
        // здесь один день (Ш6 `plans/81`, утро 2026-08-31) и снят решением владельца того же дня.
        // Спасение уходит в `report.rescues` ВЫШЕ по коду и в этот словарь не заглядывает вовсе.
      };
      // ⚡ СПАСЕНИЕ — ОТДЕЛЬНОЙ ВЕТКОЙ И РАНЬШЕ НАКОПИТЕЛЯ, ЧТОБЫ ДВА СЧЁТА НЕ СМЕШИВАЛИСЬ.
      //
      // Ветка стоит ДО накопителя намеренно: `outcome.hungBy` у спасения равен `DECIDED_BY_FUSE`,
      // и пока эта ветка первая, ни одно спасение не может случайно оказаться в `LEDGER_SOURCES`
      // — даже если кто-то однажды впишет туда ключ обратно. Дешевле сторожа и не забывается.
      if (outcome.hungBy === DECIDED_BY_FUSE) {
        report.rescues.push({ frequencyMhz: g.topMhz, depthMv: outcome.stallDepthMv ?? null });
        say('rescue-counted',
          `⚡ СПАСЕНИЕ на ${g.topMhz} МГц: аварийная защита сняла нагрузку и вернула заводское `
          + `напряжение, АПВ прошло — частота закрыта краем, ПОЛОСА ПРОДОЛЖАЕТСЯ. Частот, закрытых `
          + `спасением за этот прогон: ${report.rescues.length}. Остановки по числу спасений НЕТ: `
          + 'владелец разрешил их сколько угодно (interviews/024 = E)',
          { frequencyMhz: g.topMhz, rescues: report.rescues.length });
      } else if (LEDGER_SOURCES[outcome.hungBy]) {
        report.stalls.push({
          frequencyMhz: g.topMhz,
  ...
  // ⚡ `interviews/027` Q1 = D — ЧЕРЕДА, ЗАКРЫВШАЯ ПОЛОСУ НА ЕЁ ПОСЛЕДНЕЙ ЧАСТОТЕ.
  //
  // Цикл кончился САМ, и решение об остановe, принятое на последней частоте, не успело исполниться
  // сверху. Без этой ветки причина молча пропала бы ровно в том случае, ради которого правило и
  // заведено: полоса, где НЕ ПРОДВИНУЛАСЬ НИ ОДНА частота, выглядела бы законченной штатно.
  // Пауза владельца отдаётся и здесь: «ещё секунда таймаут — и только потом закрываем прогон».
  if (chainStopped && report.stoppedBy !== 'rescue-chain') {
    if (chainPausePending) {
      const p = chainPausePending;
      chainPausePending = null;
      console.log(`  ⏸ ЧЕРЕДА СПАСЕНИЙ: ${p.rescuesInARow} подряд, и полоса не продвинулась ни на одну частоту — `
        + `пауза ${config.RESCUE_ROUND_PAUSE_MS} мс и прогон закрывается`);
      say('rescue-round', `пауза ${config.RESCUE_ROUND_PAUSE_MS} мс после ${p.rescuesInARow} спасени(я/й) подряд `
        + '— последняя перед закрытием прогона', p);
      await new Promise((r) => { setTimeout(r, config.RESCUE_ROUND_PAUSE_MS); });
    }
    applyChainStop();
  }
  ...
  // ⚡ СРАБАТЫВАНИЯ ЗАЩИТЫ ЗА ЭТОТ ПРОГОН. Стоит рядом со временем прогона потому, что это величина
  // ТОГО ЖЕ рода: свойство полосы целиком, а не какой-то одной её ступени. И считать спасения по
  // числу пройденных этим циклом частот нельзя — за это заплачено `bugs/90`, где шесть срабатываний
  // уместились в две частоты и я прочитал второе число как первое.
  //
  // 🔴 ОДНО ЧТЕНИЕ, А НЕ РАЗНОСТЬ «КОНЕЦ − НАЧАЛО», И ЭТО РЕШЕНИЕ, А НЕ УПРОЩЕНИЕ ПОД РЕЗУЛЬТАТ.
  // Первая редакция снимала счётчик до первой ступени и вычитала — защита от журнала, который живёт
  // дольше полосы. Мутация «зашить начало нулём» прошла ЗЕЛЁНОЙ, и это оказалось находкой о самой
  // конструкции: журнал защиты СОЗДАЁТСЯ ЕЮ ЖЕ ПРИ СТАРТЕ ПРОГОНА и своего пути (`fuseJournalPath`)
  // ни с кем не делит, поэтому начало ВСЕГДА ноль. Разность защищала от невозможного, а покраснеть
  // не могла ни на одной фикстуре — то есть была лишней сущностью с видом сторожа (`PHILOSOPHY.md`
  // → Оккам; [[EXP-0216]]: слепой сторож хуже отсутствующего). Если журнал когда-нибудь станет
  // общим на несколько полос — вернуть разность вместе с фикстурой, которая её краснит.
  report.fuseTrips = typeof fuseTripsFn === 'function' ? fuseTripsFn() : null;
  // ─── 🗣 ГОЛОС ДРАЙВЕРА ЗА ОКНО ВСЕГО ПРОГОНА (`bugs/96`, ворота B96-AC2) ───────────────────────
  //
  // Ярус 1 читает канал НА ГРАНИЦЕ СТУПЕНИ, и этого оказалось мало: 31.08 пять ошибок `nvlddmkm`
  // легли в 19:33:01 при конце прогона 19:33:35 — то есть либо после последней границы, либо не
  // доехав до журнала ОС к моменту чтения. Ни одна ступень их не видела, вердикт не поднялся, и
  // вечер отчитался чистым, пока у владельца мигал экран. Одно чтение по окну ВСЕГО прогона ловит
  // оба случая сразу, потому что берётся ПОСЛЕ полосы, а не внутри неё. Ступени оно не судит —
  // ступени уже закрыты; оно судит ВЕЧЕР.
  report.driverVoice = driverVoiceForRun({
    driverEventsFn, fromMs: startedMs, toMs: clockMs(), rungs: harvestedRungs,
  });
  return report;
}

/**
 * 🗣 ГОЛОС ДРАЙВЕРА ПРО ВЕСЬ ПРОГОН — `bugs/96`, ворота B96-AC2 и B96-AC3.
 *
 * ─── ЗАЧЕМ ОТДЕЛЬНОЕ ЧТЕНИЕ, ЕСЛИ ЯРУС 1 УЖЕ ЧИТАЕТ КАНАЛ ────────────────────────────────────────
 *
 * Ярус 1 отвечает на вопрос «прекратить ли ЭТУ ступень» и потому читает окно ступени. Вопрос
 * владельца был другой — «чисто?» — и он про ВЕЧЕР. Оба числа нужны, и они разные: 31.08 окна
 * ступеней насчитали НОЛЬ, а окно прогона держало ПЯТЬ событий. Пара печатается вместе именно
 * поэтому — расхождение и есть улика запаздывания журнала ОС (`bugs/76`, гипотеза 1 тикета 96).
 *
 * ─── ТРИ СОСТОЯНИЯ, И «НЕ СМОТРЕЛ» СРЕДИ НИХ ПЕРВОКЛАССНОЕ (R4b) ────────────────────────────────
 *
 *   `looked: false` — шва нет (двойник) либо запрос к журналу Windows не ответил. Число НЕИЗВЕСТНО,
 *                     и `stop` здесь `null`, а не `false`: «не смог посмотреть» — это не «чисто».
 *   `looked: true`  — прочитали; `count` событий за окно прогона, `stop` — перекрыт ли порог серии
 *                     (`DRIVER_VOICE`: 2 события в окне 120 с), `burst` — размер худшей серии.
 *
 * Функция НИЧЕГО не решает про полосу и код возврата: вердикт вечера произносит сводка, а полоса
 * останавливается только тремя признаками, которые оставил владелец («🏗 ЗДАНИЕ ВАЖНЕЕ ЛЕСОВ»).
 *
 * [TESTED: 2026-09-01 · блоки `engine --selftest` на подставленном шве — четыре состояния и
 * расхождение с окнами ступеней; мутации EB/EC краснят свои блоки]
 */
export function driverVoiceForRun({ driverEventsFn = null, fromMs = null, toMs = null, rungs = [] } = {}) {
  // Сколько криков видели ОКНА СТУПЕНЕЙ. `null` значит «ни одна ступень канала не читала», а не
  // «ноль»: ступени без шва кладут `driverCries = null`, и складывать их с нулями нельзя.
  const seen = (Array.isArray(rungs) ? rungs : []).map((r) => r?.driverCries).filter((n) => Number.isFinite(n));
  const inRungs = seen.length ? seen.reduce((a, b) => a + b, 0) : null;
  const blind = (why) => ({ looked: false, why, count: null, burst: null, inRungs, stop: null });
  if (typeof driverEventsFn !== 'function') return blind('шов канала драйвера не подключён к этой развёртке');
  let events = null;
  try {
    events = driverEventsFn({ fromMs, toMs });
  } catch {
    return blind('шов канала драйвера отказал на чтении окна прогона');
  }
  // `null` от шва — это «прочитать не удалось» (`driverEventsInWindow` с 2026-09-01 различает
  // отказ запроса от молчания канала). Пустой массив — законное «чисто».
  if (!Array.isArray(events)) return blind('запрос к журналу Windows не ответил по провайдеру nvlddmkm');
  const v = driverVoiceVerdict(events);
  return {
    looked: true,
    why: null,
    count: events.length,
    ...
    inRungs,
    stop: v.stop,
    windowFromIso: v.windowFromIso,
    windowToIso: v.windowToIso,
...
export function sweepReportLines(report) {
  ...
  // ─── СРАБАТЫВАНИЯ АВАРИЙНОЙ ЗАЩИТЫ — P81-AC6, И ПЕЧАТАЕТСЯ ВСЕГДА, ВКЛЮЧАЯ НОЛЬ ───────────────
  //
  // Владелец назвал зависание НАШИМ провалом, а спасение — успехом защиты, который полосу не
  // останавливает (`interviews/024` = E). Раз остановки нет, показатель обязан быть на виду: иначе
  // вечер, где защита спасала машину двадцать раз, внешне неотличим от вечера, где она молчала.
  // Строка есть замена тормозу, а не украшение рядом с ним.
  //
  // 🔴 ДВА ЧИСЛА, И ОНИ НАЗВАНЫ РАЗНЫМИ СЛОВАМИ НАМЕРЕННО. `bugs/90` оплачен ровно их смешением:
  // 31.08 защита сработала ШЕСТЬ раз, а почастотный канал насчитал ДВЕ записи, и я прочитал вторую
  // как первую. Срабатывания меряет счётчик защиты; частоты считает полоса. Одной строкой их
  // теперь не спутать.
  const rescues = report.rescues ?? [];
  if (report.fuseTrips === null || report.fuseTrips === undefined) {
    lines.push(`СРАБАТЫВАНИЙ АВАРИЙНОЙ ЗАЩИТЫ: счётчик защиты не подключён к этой развёртке — число `
      + `НЕИЗВЕСТНО (не «ноль»). Частот, закрытых спасением: ${rescues.length}`
      + (rescues.length ? ` — ${rescues.map((r) => `${r.frequencyMhz} МГц`).join(', ')}` : ''));
  } else if (report.fuseTrips === 0 && rescues.length === 0) {
    lines.push('СРАБАТЫВАНИЙ АВАРИЙНОЙ ЗАЩИТЫ: 0 — машину спасать не потребовалось ни разу');
  } else {
    lines.push(`СРАБАТЫВАНИЙ АВАРИЙНОЙ ЗАЩИТЫ: ${report.fuseTrips} за прогон; закрыто спасением `
      + `частот: ${rescues.length}`
      + (rescues.length ? ` — ${rescues.map((r) => `${r.frequencyMhz} МГц`).join(', ')}` : '')
      + '. Полоса от этого НЕ останавливается: спасение есть успех защиты и найденный край частоты, '
      + 'а не повод прекращать работу (решение владельца, interviews/024 = E)');
  }
  // ─── 🗣 ГОЛОС ДРАЙВЕРА WINDOWS — ПЕЧАТАЕТСЯ ВСЕГДА, ВКЛЮЧАЯ НОЛЬ И «НЕ СМОТРЕЛ» (B96-AC3) ──────
  //
  // 🔴 СЛОВО ВЛАДЕЛЬЦА 2026-08-31, И ОНО ЖЕ ФОРМУЛИРОВКА ДЕФЕКТА: *«чисто? У меня экран в чёрных
  // мигал»* · *«если бы я не сказал — ты бы и не узнал!»*. Вечер с пятью отказами дисплейного
  // драйвера был внешне неотличим от вечера без них: про драйвер сводка не говорила НИЧЕГО, а
  // единственная строка про него в журнале прогона описывала устройство прибора, а не наблюдение.
  //
  // Барьер здесь тот же, что у строки аварийной защиты выше: показатель, который полосу не
  // останавливает, обязан быть НА ВИДУ. Молчание сводки — это и есть тот механизм, которым улика
  // существует и не доезжает до того, кто выносит вердикт (класс `bugs/93`).
  const dv = report.driverVoice ?? null;
  if (!dv || dv.looked !== true) {
    lines.push('🗣 ГОЛОС ДРАЙВЕРА (журнал Windows, окно всего прогона): НЕ ПРОЧИТАН — число событий '
      + `НЕИЗВЕСТНО (не «ноль»)${dv?.why ? `: ${dv.why}` : ''}. `
      + 'Вечер по этому каналу НЕ ПРОВЕРЕН, и чистым его называть нечем');
  } else if (dv.count === 0) {
    lines.push('🗣 ГОЛОС ДРАЙВЕРА (журнал Windows, окно всего прогона): 0 событий nvlddmkm — '
      + 'канал прочитан и молчит');
  } else {
    // РАСХОЖДЕНИЕ С ОКНАМИ СТУПЕНЕЙ — ЭТО УЛИКА, А НЕ АРИФМЕТИКА. События, которых ярус 1 не
    // видел, пришли либо после последней границы ступени, либо с запозданием журнала ОС; ровно это
    // и произошло 31.08, и ровно этого числа тогда не существовало ни для машины, ни для меня.
    const gap = Number.isFinite(dv.inRungs) && dv.count > dv.inRungs
      ? ` Окна ступеней (ярус 1) видели ${dv.inRungs} — остальные пришли вне окон ступеней либо с `
        + 'запозданием журнала ОС, поэтому ступень их не судила (bugs/96).'
      : '';
    lines.push(`🗣 ГОЛОС ДРАЙВЕРА (журнал Windows, окно всего прогона): ${dv.count} событи(й) nvlddmkm`
      // ПОРОГ ПЕЧАТАЕТСЯ ИЗ ЕДИНСТВЕННОГО ИСТОЧНИКА (`DRIVER_VOICE`), а не из копии рядом: своя
      // константа в этой строке была бы парой «правда↔зеркало» ровно того рода, за который платит
      // реестр канона — правило сменит числа, а сводка продолжит называть прежние.
      + `${dv.stop ? `, худшая серия ${dv.burst} при пороге ${DRIVER_VOICE.minCount} в окне `
        + `${DRIVER_VOICE.windowMs / 1000} с` : ', серии нет (порог не перекрыт)'}.`
      + gap
      + (dv.stop
        ? ' 🔴 ВЕЧЕР НЕ ЧИСТЫЙ: дисплейный драйвер отказывал СЕРИЕЙ, и это отказ, а не фон. По архиву '
          + 'проекта серия предшествовала замиранию машины 6 раз из 6. Разобрать до следующего прогона '
          + '(S4: зависание и отказ драйвера — наш дефект, а не штатный путь)'
        : ' Одиночные ошибки в архиве проекта замиранию не предшествовали ни разу из девяти, но '
          + 'молчанием это не является — вечер прошёл НЕ бесшумно'));
  }
...
export function selfTest() {
  ...
  // =============================================================================================
  // 🔴 `bugs/110` — КАЖДЫЙ ПИШУЩИЙ В КАРТУ ВХОД ОБЪЯВЛЕН: ЛИБО ЗАЩИЩЁН, ЛИБО ОТКАЗЫВАЕТ
  // =============================================================================================
  //
  // Список режимов берётся ИЗ ОБЩЕГО МОДУЛЯ (`lib/entry-points.mjs`), а не из литерала здесь — в
  // этом весь смысл блока. Литерал внутри блока сторожил бы сам себя: появись четвёртый вход, он
  // не попал бы ни в список, ни в ожидание, и блок остался бы зелёным, ничего не проверив. Ровно
  // так третий вход (`--search`) и прожил незамеченным — уборка его знала, защита нет.
  //
  // Блок НЕ утверждает, что защита у всех есть: сегодня её нет у двоих, и это признано открыто
  // (`plans/89` Ш0 — отказ вместо тихой записи). Он утверждает, что НЕОБЪЯВЛЕННЫХ входов нет.
  // По мере исполнения `plans/89` режимы переезжают из одного списка в другой, и ни блок, ни его
  // ожидания от этого не правятся — переезд виден в самих списках.
  ok('bugs/110: пишущих в карту режимов, не объявленных ни защищёнными, ни отказывающими, нет',
    ENGINE_CARD_WRITING_MODES.filter((m) => !GUARDED_ENTRY_POINTS.includes(m) && !UNGUARDED_ENTRY_POINTS.includes(m)), []);
  // И ни один не объявлен ОБОИМИ: пересечение — это состояние «защищён и отказывает», которого не
  // бывает, и молчание про него означало бы, что переезд Ш2 можно сделать наполовину.
  ok('bugs/110: ни один режим не объявлен одновременно защищённым и отказывающим',
    GUARDED_ENTRY_POINTS.filter((m) => UNGUARDED_ENTRY_POINTS.includes(m)), []);
  // Отказ НАЗЫВАЕТ вход по имени и уводит на защищённый путь — иначе он оставляет владельца без
  // работы вместо того, чтобы дать ей безопасную форму.
  ok('bugs/110: отказ называет режим по имени и уводит на защищённый --sweep',
    UNGUARDED_ENTRY_POINTS.map((m) => {
      const lines = unguardedEntryRefusal(m) ?? [];
      return [lines.length > 0, lines.some((l) => l.includes(m)), lines.some((l) => l.includes('--sweep'))];
    }),
    UNGUARDED_ENTRY_POINTS.map(() => [true, true, true]));
  // 🔴 И ЗАГЛУШКА НЕ СМЕЕТ ГАСИТЬ РАБОЧИЙ ПУТЬ. Без этой строки «починка» вида «отказать всем»
  // прошла бы блок целиком, отняв у владельца единственный проверенный на железе режим.
  ok('bugs/110: защищённый вход НЕ отказывает — заглушка не гасит рабочий путь',
    GUARDED_ENTRY_POINTS.map((m) => unguardedEntryRefusal(m)), GUARDED_ENTRY_POINTS.map(() => null));

  // =============================================================================================
  // 🔴 `bugs/112` — ЖИВОЕ ЧТЕНИЕ КАРТЫ ОПУСКАЕТСЯ ФЛАГОМ, А ПО УМОЛЧАНИЮ СТОИТ НА МЕСТЕ
  // =============================================================================================
  //
  // Оба утверждения нужны ОДНОВРЕМЕННО, и второе не менее важно первого: флаг, который случайно
  // включился бы всегда, тихо отнял бы у владельца строку, которую он читает перед прогоном.
  const vecFake = { capLeakMhz: 20, lowestEnforceableCapMhz: 2187 };
  ok('bugs/112: по умолчанию живое чтение НА МЕСТЕ — вывод владельцу не тронут',
    capLeakNote({ shape: 'uniform', vec: vecFake, stable: false }),
    ' · утечка потолка 20 МГц, пол железа 2187 МГц');
  ok('bugs/112: со --stable живого чтения НЕТ — план становится сличаемым с самим собой',
    capLeakNote({ shape: 'uniform', vec: vecFake, stable: true }), '');
  // И у отгружаемой формы этого чтения нет вовсе — молчание тут не экономия, а отсутствие предмета.
  ok('bugs/112: у отгружаемой формы хвоста нет ни с флагом, ни без — предмета не существует',
    [capLeakNote({ shape: 'raise-and-cap', vec: vecFake, stable: false }),
      capLeakNote({ shape: 'raise-and-cap', vec: vecFake, stable: true })], ['', '']);
  ...
  // Обе стороны обязательны: сторож, доказанный только в сторону «запрещает», — стена.
  const names = (arr) => () => arr;
  ok('оба тикета открыты — прогон ЗАПРЕЩЁН, названы оба',
    liveRunBlockers('x', names(['77_step.md', '79_voice.md'])).length, 2);
  ok('один закрыт, второй нет — прогон всё ещё запрещён, назван ОСТАВШИЙСЯ',
    liveRunBlockers('x', names(['77_DONE_step.md', '79_voice.md'])).map((b) => b.id), ['79']);
  ok('ОБА закрыты — запрет снимается САМ, правки кода не нужно',
    liveRunBlockers('x', names(['77_DONE_step.md', '79_DONE_voice.md'])).length, 0);
  ok('тикета нет в каталоге вовсе — считается ОТКРЫТЫМ (отказ в сторону безопасности)',
    liveRunBlockers('x', names(['01_other.md'])).length, 2);
  ok('каталог не читается — считается ОТКРЫТЫМ, сломанный сторож на пути к карте запрещает',
    liveRunBlockers('x', () => { throw new Error('нет каталога'); }).length, 2);
  ok('DONE у ЧУЖОГО тикета не открывает запрет',
    liveRunBlockers('x', names(['77_step.md', '79_voice.md', '80_DONE_other.md'])).length, 2);
  ok('запрет на ЖИВОЙ прогон, а не на двойника: список тикетов непуст и адресен',
    LIVE_RUN_BLOCKERS.map((b) => b.id), ['77', '79']);
  ...
  return (async () => {
    ...
    try {
      ...
      // ─── `bugs/88` — СПАСЕНИЕ ПРЕДОХРАНИТЕЛЯ КАК ВХОД СТУПЕНИ (решение владельца 023) ─────────
      //
      // ЧТО ЭТО ЛЕЧИТ, ИЗМЕРЕННОЕ 2026-08-31 НА ДВОЙНИКЕ: рука 2 обнуляла кривую под живой
      // ступенью, прожиг шёл на ЗАВОДСКОЙ кривой, а ступень закрывалась `PASS` и писала
      // андервольт, которого не было. Ложная доказанная земля — пробитый барьер «в документ идёт
      // только подтверждённое».
      //
      // АДРЕСАТЫ МУТАЦИЙ, НАЗВАННЫЕ ДО ПРОГОНА:
      //   FA. снять вызов `fuseTripsFn` из runRung             → «СПАСЕНИЕ ОТМЕНЯЕТ PASS»
      //   FB. сравнивать `>=` вместо `>` (счёт не менялся)     → «БЕЗ ТРИПА ступень не трогается»
      //   FC. поставить блок ПОСЛЕ пульса                      → «СПАСЕНИЕ РЕШАЕТ РАНЬШЕ ПУЛЬСА»
      {
        const jrnF = openJournal({ dir: join(journalBox, 'fuse-rescue') });
        // Счёт РАСТЁТ между двумя вызовами — ровно то, что видит ступень, под которой сработал
        // судья: до работы трипов не было, после стало один.
        let trips = 0;
        const rescued = await runRung({
          envelopeMhz: 3090, points: tablePoints, clockMhz: 2842, voltageMv: 1000,
          buildVector: vectorCapped, journal: jrnF, seq: 1, now: clock,
          runStepFn: async () => atomPass(1000),
          fuseTripsFn: () => trips++,          // 0 на первом чтении, 1 на втором
        });
        ok('СПАСЕНИЕ ОТМЕНЯЕТ PASS: прожиг после обнуления кривой ничего не доказывает (bugs/88)',
          [rescued.verdict, rescued.outcome, rescued.decidedBy, rescued.rescuedByFuse],
          [config.VERDICT.HUNG, 'hung', DECIDED_BY_FUSE, true]);
        // Та же дорога, что у пульса: исход `hung` кормит пол зависания без единой правки, и это
        // ровно то, чем решение владельца («частота закрывается») исполняется механизмом R18.
        ok('ПОЛ ЗАВИСАНИЯ ВИДИТ СПАСЁННУЮ СТУПЕНЬ — механизм R18 работает без правок',
          hangFloors(readJournal(jrnF).records).get(2842)?.voltageMv ?? 'частоты в полу нет', 1000);
        // 🔴 КОНТРОЛЬ, БЕЗ КОТОРОГО БЛОК ВЫШЕ НИЧЕГО НЕ ЗНАЧИТ: сторож, краснящий и без трипа,
        // закрывал бы КАЖДУЮ частоту отказом — то есть был бы хуже отсутствующего.
        const calm = await runRung({
          envelopeMhz: 3090, points: tablePoints, clockMhz: 2842, voltageMv: 1000,
          buildVector: vectorCapped,
          runStepFn: async () => atomPass(1000),
          fuseTripsFn: () => 7,                // счёт НЕ меняется: трипы были когда-то, но не сейчас
        });
        ok('БЕЗ ТРИПА ступень не трогается — считается РАЗНОСТЬ, а не наличие трипов в журнале',
          [calm.verdict, calm.outcome, calm.rescuedByFuse ?? 'поля нет'],
          [config.VERDICT.PASS, 'passed', 'поля нет']);
        // ПРИОРИТЕТ: спасение отменяет САМ ПРОЖИГ (карта вернулась к стоку), пульс и голос отменяют
        // лишь ВЕРДИКТ прожига. Поэтому спасение решает первым, и порядок блоков — это приоритет.
        let t2 = 0;
        const withPulse = await runRung({
          envelopeMhz: 3090, points: tablePoints, clockMhz: 2842, voltageMv: 1000,
          buildVector: vectorCapped,
          runStepFn: async () => atomPass(1000),
          fuseTripsFn: () => t2++,
        ...
        ok('СПАСЕНИЕ РЕШАЕТ РАНЬШЕ ПУЛЬСА: оно отменяет сам прожиг, а пульс — только его вердикт',
          withPulse.decidedBy, DECIDED_BY_FUSE);
        ok('ШВА НЕТ — СПАСЕНИЯ НЕТ: невзведённый вход не голосует (то же правило R4b)',
          (await runRung({
            envelopeMhz: 3090, points: tablePoints, clockMhz: 2842, voltageMv: 1000,
            buildVector: vectorCapped, runStepFn: async () => atomPass(1000),
          })).rescuedByFuse ?? 'поля нет', 'поля нет');
      }
    ...
    // ─── `bugs/88` — СПАСЕНИЕ ЗАКРЫВАЕТ ЧАСТОТУ КРАЕМ (решение владельца `interviews/023`) ───────
    //
    // Его слова: *«от этого напряжения берем выше ближайшее прошедшее прожиг + еще один шаг…
    // эта частота закрывается как протюненая — и просто дальше по плану… со следующей частоты»*.
    //
    // АДРЕСАТЫ МУТАЦИЙ, НАЗВАННЫЕ ДО ПРОГОНА:
    //   FD. снять ветку `rescuedByFuse` из `sweepFrequency`  → «СПАСЕНИЕ ЗАКРЫВАЕТ ЧАСТОТУ КРАЕМ»
    //   FE. звать `refineEdge` вместо `shipVoltageFor`       → «ПОСЛЕ СПАСЕНИЯ НЕ ЖГЁМ ЗАНОВО»
    //   FF. писать строку и при пустом `lastPass`            → «СПАСЕНИЕ НА ПЕРВОМ ШАГЕ — НЕ СТРОКА»
    {
      // Ступень отвечает как настоящая: выше 995 мВ проходит, на 995 приходит спасение.
      const rescueAt = (mv) => async ({ voltageMv }) => (voltageMv > mv
        ? { outcome: 'passed', verdict: P, why: '' }
        : { outcome: 'hung', verdict: config.VERDICT.HUNG, rescuedByFuse: true, decidedBy: DECIDED_BY_FUSE, why: 'СПАСЕНО ПРЕДОХРАНИТЕЛЕМ' });

      const burned = [];
      const closed = await freqOK({
        runRungFn: async (a) => { burned.push(a.voltageMv); return rescueAt(995)(a); },
      });
      // Последняя прошедшая — 1020 мВ; отгружается она + один минимальный шаг сетки карты.
      // Числа — ЛИТЕРАЛЫ ИЗ ПРОГОНА, а не пересчёт теми же константами, что и код (EXP-0055).
      ok('СПАСЕНИЕ ЗАКРЫВАЕТ ЧАСТОТУ КРАЕМ: отгружается ПОСЛЕДНЯЯ ПРОШЕДШАЯ + один шаг сетки (bugs/88, 023)',
        [closed.verdict, closed.voltageMv, closed.halted ?? false],
        ['edge-found', 1025, false]);
      // 🔴 И ГЛАВНОЕ ПО БЕЗОПАСНОСТИ: после спасения на этой частоте НЕ ЖГЁТСЯ НИ ОДНОГО ПРОЖИГА.
      // Обычный отказ уточняется шагом сетки; здесь уточнение означало бы новые прожиги на частоте,
      // где машина только что чуть не умерла, — а владелец сказал «дальше со СЛЕДУЮЩЕЙ частоты».
      ok('ПОСЛЕ СПАСЕНИЯ НЕ ЖГЁМ ЗАНОВО: спасшая ступень последняя, уточнения нет (слово владельца)',
        burned.at(-1), 995);
      ok('ПОДПИСЬ НЕ ВРЁТ ПРО ПРОИСХОЖДЕНИЕ: строка называет спасение, а не вердикт оракула',
        /СПАСЕНИЕМ ПРЕДОХРАНИТЕЛЯ/u.test(closed.provenBy ?? ''), true);

      // Срабатывание защиты на ПЕРВОЙ ЖЕ ступени: прошедших нет. Слово владельца — это значит, что
      // сток уже близко к краю, и частоту надо переискать шагом 5 мВ; ветка этого поиска ещё не
      // построена, поэтому частота ОТКАЗЫВАЕТ вслух и строку НЕ пишет.
      const first = await freqOK({ runRungFn: rescueAt(1020) });
      ok('СРАБАТЫВАНИЕ НА ПЕРВОМ ШАГЕ — НЕ СТРОКА: прошедших нет, отгружать нечего, и это сказано вслух',
        [first.verdict, first.halted, first.rescueNeedsFineSearch ?? false],
        [null, true, true]);
      // ⚠️ БЛОК СТЕРЕЖЁТ НЕ СЛОВА, А ЧЕСТНОСТЬ ПРИЧИНЫ, и после Ш5 он усилен, а не ослаблен.
      // Прежде он требовал в тексте слово «предохранитель» — и потому остался бы ЗЕЛЁНЫМ на
      // формулировке «судья после трипа вышел», которая с 31.08 стала ложью (защита срабатывание
      // переживает). Теперь блок требует: правило владельца названо · сказано, что ветка НЕ
      // ПОСТРОЕНА · и ПРЯМО ЗАПРЕЩЕНА мёртвая причина. Мутация «вернуть прежний текст» краснит его.
      ok('ОТКАЗ НАЗЫВАЕТ ПРАВИЛО ВЛАДЕЛЬЦА И НАСТОЯЩЕЕ НЕДОСТАЮЩЕЕ УСЛОВИЕ, а не мёртвую причину',
        [/СТОК УЖЕ БЛИЗКО К КРАЮ/u.test(first.why ?? ''),
          /НЕ ПОСТРОЕНА/u.test(first.why ?? ''),
          /вышел|выходит/u.test(first.why ?? '')],
        [true, true, false]);
    }
    ...
    // =============================================================================================
    {
      ...
      // ═══════════════════════════════════════════════════════════════════════════════════════════
      // ⚡ Ш5 `plans/81` — ПОЛОСА ЖДЁТ АПВ И НЕ ЖЖЁТ ПОД НЕВЗВЕДЁННОЙ ЗАЩИТОЙ (P81-AC4)
      // ═══════════════════════════════════════════════════════════════════════════════════════════
      //
      // Мутируется ПРОВОДКА, а не судья ([[EXP-0205]]): блок ломает не решение защиты, а право
      // полосы ждать. Крючок отвечает «ещё не взведена» ровно один раз, затем «взведена» — и
      // проверяется ДВА факта сразу: полоса дождалась (не встала) И не сожгла ничего за время
      // ожидания. Второй факт — тот самый, ради которого ожидание вообще существует.
      {
        const burned = [];
        let burnsAtWaitStart = null;
        let burnsAtWaitEnd = null;
        const waited = await sweepRange({
          envelopeMhz: 3090,
          curveDoc: sweepDoc(bandRows), points: sweepPoints,
          runStepFn: async (args) => { burned.push(args.pinMhz ?? args.capMhz); return sweepAtom(980)(args); },
          buildVector: vectorPinned,
          saveFn: async () => ({ ok: true }),
          // Крючок ЖДЁТ — ровно так же, как ждёт настоящий, пока защита возвращается на пост.
          // Ждём один раз, после первых прожигов: это и есть момент «защита спасает машину».
          stopWhen: async () => {
            if (burnsAtWaitStart === null && burned.length > 0) {
              burnsAtWaitStart = burned.length;
              await new Promise((r) => { setTimeout(r, 20); });
              burnsAtWaitEnd = burned.length;
            }
            return null;
          },
          now: () => '2026-08-16T02:00:00+03:00',
        ...
        // Без `await` у крючка `externalStop` стал бы промисом — истинным значением, — и полоса
        // встала бы с подделанной причиной `external-stop`. Мутация «снять await» краснит именно
        // эту строку, и краснит по существу: она проверяет, что полоса ДОШЛА, а не что ей повезло.
        ok('Ш5: полоса пережила ожидание АПВ и дошла до конца — срабатывание больше не кончает вечер',
          [waited.stoppedBy ?? null, waited.closed > 0], [null, true]);
        ok('Ш5: пока полоса ждёт АПВ, не начат НИ ОДИН прожиг (прожиг без защиты запрещён)',
          burnsAtWaitEnd, burnsAtWaitStart);
      }

      // ═══════════════════════════════════════════════════════════════════════════════════════
      // ⚡ СУДЬЯ ВЫШЕЛ ПОСРЕДИ ЧАСТОТИ — СЛЕДУЮЩАЯ СТУПЕНЬ НЕ НАЧИНАЕТСЯ (`bugs/101`, находка 2)
      // ═══════════════════════════════════════════════════════════════════════════════════════
      //
      // 04.09 10:56 судья вышел кодом 2 после второй ступени 2700 МГц, а полоса сожгла ещё пять:
      // крючок спрашивался раз на частоту. Здесь судья «выходит» после ВТОРОГО прожига, атом
      // всегда PASS — без опроса на ступени первая частота прожглась бы всей лестницей.
      //   АДРЕСАТ МУТАЦИИ: снять опрос `stopWhen` в цикле ступеней `sweepFrequency` → «прожигов ровно 2».
      {
        const burnedMid = [];
        const midStop = await sweepRange({
          envelopeMhz: 3090,
          curveDoc: sweepDoc(bandRows), points: sweepPoints,
          runStepFn: async (args) => { burnedMid.push(args.pinMhz ?? args.capMhz); return sweepAtom(-1)(args); },
          buildVector: vectorPinned,
          saveFn: async () => ({ ok: true }),
          stopWhen: () => (burnedMid.length >= 2
            ? { why: 'судья вышел посреди частоты (код 2 — перевзведение отказано)', rescue: true } : null),
          now: () => '2026-08-16T02:00:00+03:00',
          clockMs: (() => { let t = 0; return () => (t += 1000); })(),
        });
        ok('bugs/101: судья вышел ПОСРЕДИ частоты — следующая СТУПЕНЬ не начата: прожигов ровно 2',
          burnedMid.length, 2);
        ok('bugs/101: прерванная частота закрыта на последней прошедшей (cut-short), полоса встала fuse-rescue',
      ...
      // ═══════════════════════════════════════════════════════════════════════════════════════════
      //
      // 🔴 ЭТОТ БЛОК ПЕРЕВЁРНУТ, И ПЕРЕВЁРНУТ ПО СЛОВУ ВЛАДЕЛЬЦА, А НЕ ПОД РЕЗУЛЬТАТ. Прежде он
      // утверждал «череда срабатываний ОСТАНАВЛИВАЕТ полосу — тормоз против бесконечного спасения»
      // и был зелёным: я построил этот тормоз утром 2026-08-31 (Ш6 `plans/81`). Днём того же дня
      // владелец его отменил дословно: *«Сколько угодно спасений разрешаю. KAGO — это
      // автоматизированный движок тюнинга видеокарты. В идеале он должен работать по принципу
      // "Запустил и забыл — оно само все сделало"»*. Слово владельца старше кода и старше блока,
      // который код утверждал (`BUG_FIXING_FRAMEWORK.md` → порядок старшинства у ворот намерения).
      //
      // ⚠️ И ФИКСТУРА ПЕРЕПИСАНА ВМЕСТЕ С УТВЕРЖДЕНИЕМ, ПОТОМУ ЧТО СТАРАЯ ЗАДАВАЛА ТО, ЧТО ОБЯЗАНА
      // БЫЛА МЕРИТЬ (`bugs/90` B90-AC2). Стоял счётчик `fuseTripsFn: () => trips++` — чтение с
      // побочным действием: каждый ВЗГЛЯД на счётчик двигал его, поэтому «сработала защита» было
      // свойством числа вызовов, а не полосы, и сводка посчитала бы собственные чтения. Теперь
      // счётчик растёт ОТ ПРОЖИГА, а читается чисто — раскладку срабатываний по частотам определяет
      // полоса, и её же можно измерить. Это тот класс, за который заплачено дважды: [[EXP-0218]]
      // (диагностика, которая пишет) и [[EXP-0217]] (фикстура задала то, что должна была мерить).
      //
      // Полоса нарочно шире порога накопителя: ШЕСТЬ ступеней против `runLimit` 3, и 18 срабатываний
      // на них — то есть и по частотам, и по срабатываниям порог перейдён с запасом. Если бы правка
      // была фильтром над `stalls`, а не своим списком, этот блок бы её не отличил — а мутация
      // «вернуть спасение в накопитель» краснит здесь четыре строки сразу (проверено 2026-08-31).
      {
        let trips = 0;
        // Шесть РАЗНЫХ стоковых напряжений — шесть ступеней по одной частоте (группировка идёт по
        // стоку, это проверено блоком выше). Ровно шесть срабатываний живого вечера 2026-08-31.
        const sixRungs = [
          sweepRow(2842, 1045), sweepRow(2835, 1040), sweepRow(2828, 1035),
          sweepRow(2820, 1030), sweepRow(2813, 1025), sweepRow(2806, 1020),
        ];
        const rescued = await sweepRange({
          envelopeMhz: 3090,
          curveDoc: sweepDoc(sixRungs), points: sweepPoints,
          // Счёт срабатываний двигает ПРОЖИГ, а не взгляд на счётчик — и двигает НА ТРИ, а не на
          // один. Это не украшение фикстуры, а её главное свойство: с шагом «одно срабатывание на
          // ступень» числа срабатываний и частот СОВПАЛИ БЫ (6 и 6), и блок, который обязан их
          // различать, не отличил бы одно от другого — вырожденный контроль ([[EXP-0213]]). Живой
          // вечер 31.08 дал ровно эту несоразмерность: ШЕСТЬ срабатываний уместились в ДВЕ частоты,
          // и `bugs/90` оплачен тем, что я прочитал второе число как первое. Здесь 18 против 6.
          runStepFn: async (args) => { trips += 3; return sweepAtom(980)(args); },
          fuseTripsFn: () => trips,
          buildVector: vectorPinned,
          saveFn: async () => ({ ok: true }),
          now: () => '2026-08-16T02:00:00+03:00',
        ...
        ok('спасения НЕ ДОЕЗЖАЮТ до накопителя предупреждений — они не предупреждения, а успех защиты',
          rescued.stalls, []);
        // ✏️ ПЕРЕПИСАНО 2026-09-07 ПО РЕШЕНИЮ ВЛАДЕЛЬЦА (`interviews/027` Q1 = D), А НЕ ПОДОГНАНО.
        //
        // Здесь стояло «ЧЕРЕДА СРАБАТЫВАНИЙ НЕ ОСТАНАВЛИВАЕТ ПОЛОСУ… полоса прошла все шесть», и до
        // сегодня это был весь закон (`interviews/024` = E). Владелец ДОПОЛНИЛ его: череда, в
        // которой полоса не закрыла НИ ОДНОЙ частоты, кончается — три спасения, пауза, ещё три,
        // пауза, конец. В этой фикстуре все шесть частот спасены и ни одна не закрыта, то есть она
        // ровно тот случай, который он описал.
        //
        // 🔴 ПОЧЕМУ СТРОКУ ПРИШЛОСЬ ТРОГАТЬ, ХОТЯ ОНА ОСТАВАЛАСЬ ЗЕЛЁНОЙ: `haltedBy` и
        // `skipped.length` после правки не изменились (полоса и правда прошла все шесть, а потом
        // встала), и блок продолжал гореть зелёным, ДОКАЗЫВАЯ УЖЕ НЕ ТО, ЧТО ЗАЯВЛЯЕТ. Зелёный,
        // разошедшийся со своим именем, — тот же оплаченный класс, что `bugs/106`. Поэтому имя
        // приведено к правде и добавлено ТРЕТЬЕ поле, которое отличает старый закон от нового.
        ok('interviews/027 = D: шесть спасений подряд БЕЗ единой закрытой частоты — полоса проходит все шесть и ЗАКРЫВАЕТСЯ с названной причиной',
          [rescued.haltedBy ?? null, rescued.skipped.length, rescued.stoppedBy ?? null, rescued.rescueChainLongest],
          [null, 6, 'rescue-chain', 6]);
        ok('спасения ВИДНЫ поимённо и сосчитаны своим счётчиком (P81-AC6, число ЗАМЕРЕНО фикстурой)',
          [rescued.rescues.map((r) => r.frequencyMhz), rescued.fuseTrips],
          [[2842, 2835, 2828, 2820, 2813, 2806], 18]);
        // Сводка — это то, что владелец ВИДИТ. Показатель, который есть только в объекте отчёта,
        // для него не существует; P81-AC6 требует именно СТРОКУ, и здесь она проверяется как строка.
        const rescueLine = sweepReportLines(rescued).filter((l) => l.startsWith('СРАБАТЫВАНИЙ АВАРИЙНОЙ ЗАЩИТЫ:'));
        ok('P81-AC6: сводка несёт РОВНО ОДНУ строку о срабатываниях, и в ней настоящее число',
          [rescueLine.length, /^СРАБАТЫВАНИЙ АВАРИЙНОЙ ЗАЩИТЫ: 18 за прогон; закрыто спасением частот: 6 — /u.test(rescueLine[0] ?? '')],
          [1, true]);
        // ═══════════════════════════════════════════════════════════════════════════════════════
        // 🔴 ПАРА К ПРЕДЫДУЩЕМУ БЛОКУ, БЕЗ КОТОРОЙ ОН ДОКАЗЫВАЕТ ПОЛОВИНУ ЗАКОНА
        // ═══════════════════════════════════════════════════════════════════════════════════════
        //
        // Блок выше показал, что череда КОНЧАЕТСЯ. Сам по себе он не отличает нового закона от
        // «спасений теперь можно не больше шести» — а это прямо противоречило бы `interviews/024`
        // = E («сколько угодно спасений разрешаю»). Отличает их ровно одно: ПРОДВИНУЛАСЬ ЛИ полоса
        // между спасениями. Здесь спасения чередуются с закрытыми частотами — и полоса обязана
        // дойти до конца, сколько бы спасений ни случилось.
        //
        // Фикстура задаёт не число, а ЧЕРЕДОВАНИЕ: счётчик обнуляется каждой закрытой частотой,
        // поэтому длиннейшая череда здесь структурно равна единице, и это утверждение a priori, а
        // не списанное с прогона ([[EXP-0217]]: фикстура не смеет спрашивать то, что должна мерить).
        {
          let tripsAlt = 0;
          const rescueAt = new Set([2842, 2828, 2813]);   // 1-я, 3-я, 5-я частоты полосы
          const alternating = await sweepRange({
            envelopeMhz: 3090,
            curveDoc: sweepDoc(sixRungs), points: sweepPoints,
            runStepFn: async (args) => {
              if (rescueAt.has(args.pinMhz ?? args.capMhz)) tripsAlt += 3;
              return sweepAtom(980)(args);
            },
            fuseTripsFn: () => tripsAlt,
            buildVector: vectorPinned,
            saveFn: async () => ({ ok: true }),
            now: () => '2026-08-16T02:00:00+03:00',
            clockMs: (() => { let t = 0; return () => (t += 1000); })(),
          });
          ok('interviews/024 = E НЕ ОТМЕНЁН: спасения, между которыми полоса продвигается, её не останавливают',
            [alternating.stoppedBy ?? null, alternating.rescueChainLongest, alternating.closed > 0],
            [null, 1, true]);
          // И спасения при этом были — иначе блок доказывал бы только «полоса без спасений доходит».
          ok('пара измерима: спасения в этом прогоне ДЕЙСТВИТЕЛЬНО случались',
            [alternating.rescues.length > 0, alternating.fuseTrips > 0], [true, true]);
        }

        // ─── ВТОРАЯ ПОЛОВИНА, БЕЗ КОТОРОЙ СТРОКА НЕДОКАЗУЕМА: НОЛЬ ТОЖЕ ПЕЧАТАЕТСЯ ────────────────
        // Строка, появляющаяся только при спасениях, не говорит читателю НИЧЕГО о вечерах, где их
        // не было, — а «защита ни разу не понадобилась» это такой же результат прогона, как список.
        const calm = await sweepRange({
          envelopeMhz: 3090,
          curveDoc: sweepDoc(bandRows), points: sweepPoints,
          runStepFn: async (args) => sweepAtom(-1)(args),
          fuseTripsFn: () => 0,
          buildVector: vectorPinned,
          saveFn: async () => ({ ok: true }),
          now: () => '2026-08-16T02:00:00+03:00',
        ...
        ok('спокойный прогон: строка о срабатываниях всё равно печатается и говорит НОЛЬ',
        ...
        ok('без счётчика защиты сводка говорит НЕИЗВЕСТНО, а не «ноль» (R4b: не смотрел ≠ не нашёл)',
          [blind.fuseTrips, sweepReportLines(blind).some((l) => /^СРАБАТЫВАНИЙ АВАРИЙНОЙ ЗАЩИТЫ: счётчик защиты не подключён/u.test(l))],
          [null, true]);

        // ─── 🗣 ГОЛОС ДРАЙВЕРА ПРО ВЕСЬ ПРОГОН — `bugs/96`, ворота B96-AC2 и B96-AC3 ──────────────
        //
        // Барьер здесь ТОТ ЖЕ, что у трёх блоков выше, и стоит он рядом намеренно: показатель,
        // который полосу не останавливает, доказывается двумя половинами — «печатается, когда есть»
        // и «печатается, когда нет», — плюс третьей, которую проект знает как R4b: «не смог
        // посмотреть» обязано отличаться от «посмотрел и ноль».
        //
        // ⚠️ ПРАВИЛО ПРОВЕРЯЕТСЯ НА ФУНКЦИИ, А ПРОВОДКА — НА ПОЛОСЕ, И ЭТО РАЗНЫЕ БЛОКИ ([[EXP-0205]]:
        // мутируй проводку, а не судью). Кормить полосу шовом, который отдаёт события, значило бы
        // измерять ЯРУС 1 (он прекратил бы каждую ступень голосом драйвера) — то есть фикстура
        // задала бы то, что должна была мерить ([[EXP-0217]]).
        {
          const cry = (ms) => ({ atMs: ms, provider: 'nvlddmkm' });
          const noSeam = driverVoiceForRun({ rungs: [{ driverCries: 0 }, { driverCries: 0 }] });
          ok('B96: шва нет — число НЕИЗВЕСТНО, и `stop` это null, а не false («не смотрел» ≠ «чисто»)',
            [noSeam.looked, noSeam.count, noSeam.stop], [false, null, null]);
          ok('B96: запрос не ответил (null от шва) — тоже «не прочитан», а не ноль',
            (() => { const r = driverVoiceForRun({ driverEventsFn: () => null }); return [r.looked, r.count]; })(),
            [false, null]);
          ok('B96: шов БРОСИЛ — прогон не падает, но сводка узнаёт, что канал не прочитан',
            (() => { const r = driverVoiceForRun({ driverEventsFn: () => { throw new Error('powershell died'); } }); return [r.looked, r.count]; })(),
            [false, null]);
          const silent = driverVoiceForRun({ driverEventsFn: () => [], rungs: [{ driverCries: 0 }] });
          ok('B96: прочитали и канал молчит — 0 это ЗАКОННЫЙ ответ, отличимый от «не прочитали»',
            [silent.looked, silent.count, silent.stop], [true, 0, false]);
          ok('B96: одиночная ошибка сосчитана, но серии нет — порог не перекрыт',
            (() => { const r = driverVoiceForRun({ driverEventsFn: () => [cry(0)], rungs: [{ driverCries: 1 }] }); return [r.count, r.stop, r.inRungs]; })(),
            [1, false, 1]);
          // ВЕЧЕР 31.08 ЧИСЛАМИ, СНЯТЫМИ С ЖУРНАЛА WINDOWS: пять событий за 4 мс, окна ступеней
          // видели НОЛЬ. Ровно этой пары в тот вечер не существовало ни для машины, ни для меня.
          const evening = driverVoiceForRun({
            driverEventsFn: () => [cry(0), cry(1), cry(2), cry(3), cry(4)],
            rungs: [{ driverCries: 0 }, { driverCries: 0 }],
          });
          ok('B96 ВЕЧЕР 31.08: пять событий окна ПРОГОНА против НУЛЯ у окон СТУПЕНЕЙ — серия найдена',
            [evening.count, evening.burst, evening.stop, evening.inRungs], [5, 5, true, 0]);
          ok('B96: ступени без шва дают inRungs = null, а не ноль — складывать «не смотрел» с нулём нельзя',
            driverVoiceForRun({ driverEventsFn: () => [], rungs: [{}, {}] }).inRungs, null);

          // ─── СТРОКА — ЭТО ТО, ЧТО ВИДИТ ВЛАДЕЛЕЦ. Показатель, живущий только в объекте отчёта,
          // для него не существует: тикет требует СТРОКУ, и здесь она проверяется как строка.
          const dvLine = (r) => sweepReportLines(r).filter((l) => l.startsWith('🗣 ГОЛОС ДРАЙВЕРА'));
          // ⚠️ ПОЛЕ СПРАШИВАЕТСЯ ЧЕРЕЗ `?.`, И ЭТО НЕ ОСТОРОЖНОСТЬ, А ТРЕБОВАНИЕ К БЛОКУ. Первая
          // редакция читала `calm.driverVoice.looked` напрямую — и мутация «снять вызов
          // `driverVoiceForRun` из `sweepRange`» УБИЛА набор броском вместо того, чтобы покрасить
          // этот блок. Набор, умерший на исключении, провалов не печатает: адресат мутации обязан
          // получить КРАСНЫЙ, а не отсутствие отчёта (та же причина, по которой `journal --selftest`
          // прогоняется через `runSelfTest`). Ждём `false`, а `null` от снятой проводки — красный.
          ok('B96-AC3: строка про драйвер РОВНО ОДНА, и на прогоне без шва она говорит НЕ ПРОЧИТАН',
            [dvLine(calm).length, calm.driverVoice?.looked ?? null, /НЕ ПРОЧИТАН/u.test(dvLine(calm)[0] ?? '')],
            [1, false, true]);
          ok('B96: прочитанный молчащий канал говорит НОЛЬ и не притворяется отказом',
            /0 событий nvlddmkm — канал прочитан и молчит/u.test(dvLine({ ...calm, driverVoice: silent })[0] ?? ''), true);
          ok('B96-AC4: вечер с СЕРИЕЙ прямо назван НЕ ЧИСТЫМ, и расхождение с ярусом 1 стоит в той же строке',
            (() => {
              const l = dvLine({ ...calm, driverVoice: evening })[0] ?? '';
              return [/ВЕЧЕР НЕ ЧИСТЫЙ/u.test(l), /Окна ступеней \(ярус 1\) видели 0/u.test(l),
                new RegExp(`пороге ${DRIVER_VOICE.minCount} в окне ${DRIVER_VOICE.windowMs / 1000} с`, 'u').test(l)];
            })(), [true, true, true]);
          // ПРОВОДКА: развёртка обязана САМА позвать шов за окно всего прогона. Мутация «снять вызов
          // `driverVoiceForRun` из `sweepRange`» краснит этот блок, а не блоки правила выше.
          const watched = await sweepRange({
            envelopeMhz: 3090,
            curveDoc: sweepDoc(bandRows), points: sweepPoints,
            runStepFn: async (args) => sweepAtom(-1)(args),
            driverEventsFn: () => [],
          ...
          ok('B96-AC2: развёртка ЗОВЁТ шов канала за окно всего прогона — поле отчёта заполнено чтением',
            [watched.driverVoice?.looked, watched.driverVoice?.count], [true, 0]);
        }
      }
    ...
    // ═══ `bugs/73` — ПОЛ КАРТЫ УЗНАЁТСЯ ПОВТОРЕНИЕМ, А НЕ ШИРИНОЙ ПРОМАХА ══════════════════════════
    //
    // Порог выше отвечает на вопрос «другая ли это запись таблицы» и отвечает верно. Пол карты
    // (`bugs/58`) он не ловит: рычаг в МЕГАГЕРЦАХ, достижимые напряжения частоты — подмножество
    // сетки, поэтому одиночный промах в одну ступень неразличим (пол или соседняя достижимая
    // ступень). Различает только ПЛАТО: несколько РАЗНЫХ заказов вниз — одна выдача.
    //
    // ЗАМЕРЕНО по боевому журналу (784 ступени): плато выдачи 35; из них 14 шире ступени сетки —
    // их порог метил и раньше, а 21 не шире, и они уходили чистым замером. Среди этих 21 —
    // seq 792/793 от 2026-08-26, чей подъём отобрал 45 мВ у пятнадцати частот (`bugs/63`).
    //
    // АДРЕСАТЫ МУТАЦИЙ, НАЗВАННЫЕ ДО ПРОГОНА:
    //   EL. убрать `|| floorPlateau` из `marked`        → П73-AC1 и П73-AC2
    //   EM. считать плато от ОДНОГО заказа (size >= 1)  → F2-AC2 и F2-AC3 (метка на пол-документа)
    //   EN. подпись плато снова говорит «другая запись» → П73-AC4
    const plateauOutcome = (servedMv, orders) => ({
      voltageMv: servedMv, rungs: orders.map((orderedMv) => ({ voltageMv: servedMv, orderedMv })),
    });
    // П73-AC1. Промах РОВНО в одну ступень, но заказов было два — карта не пошла ниже.
    ok('П73-AC1: ПЛАТО ВЫДАЧИ помечается даже при промахе в ОДНУ ступень — это пол, а не округление',
      (() => { const m = overshootMarkFor(plateauOutcome(895, [895, 890]), 10);
        return [m.marked, m.floorPlateau, m.overshootMv, m.plateauOrdersMv]; })(),
      [true, true, 5, [895, 890]]);
    // П73-AC2. ФОРМА ВЕЧЕРА 26 АВГУСТА ЦЕЛИКОМ: спуск 970 → 945 → 920 → 895 → 890, выдача строки 895,
    // и 895 обслужило ДВА разных заказа. Это ступень, поднявшая пятнадцать частот до 895.
    ok('П73-AC2: ступень 26.08 (seq 792/793) теперь помечена — та, что отобрала 45 мВ',
      (() => { const m = overshootMarkFor({
        voltageMv: 895,
        rungs: [
          { voltageMv: 970, orderedMv: 970 }, { voltageMv: 950, orderedMv: 945 },
          { voltageMv: 925, orderedMv: 920 }, { voltageMv: 895, orderedMv: 895 },
          { voltageMv: 895, orderedMv: 890 },
        ],
      }, 10); return [m.marked, m.floorPlateau]; })(), [true, true]);
    // П73-AC3. ОБРАТНАЯ СТОРОНА, БЕЗ КОТОРОЙ МЕТКА НАКРЫЛА БЫ ПОЛДОКУМЕНТА: правило владельца
    // 2026-08-16 («одна ступень вверх — ПОПАДАНИЕ») цело. Один заказ — плато нет.
    ok('П73-AC3: ОДИНОЧНОЕ попадание в одну ступень плато НЕ образует — правило владельца 16.08 цело',
      (() => { const m = overshootMarkFor(plateauOutcome(895, [890]), 10);
        return [m.marked, m.floorPlateau]; })(), [false, false]);
    // П73-AC4. ПРИЧИНА МЕТКИ НАЗЫВАЕТСЯ ТА, ПО КОТОРОЙ ОНА ПОСТАВЛЕНА (`bugs/42`: объяснение, которое
    // ничего не измеряет, живёт неделями).
    ok('П73-AC4: подпись плато говорит про ПОЛ и перечисляет заказы, а не про «другую запись таблицы»',
      (() => {
        const w = overshootWitness(overshootMarkFor(plateauOutcome(895, [895, 890]), 10));
        return [w.includes('С ПОЛА КАРТЫ'), w.includes('895 → 890'), w.includes('ДРУГАЯ запись')];
      })(), [true, true, false]);
    ...
    // ═══════════════════════════════════════════════════════════════════════════════════════════
    // 📓 СВОЙ ЖУРНАЛ У ПРОГОНА — `bugs/93`, ворота B93-AC1 и B93-AC2
    // ═══════════════════════════════════════════════════════════════════════════════════════════
    //
    // Дефект, который эти блоки сторожат, НЕ в движке: он в командной строке. Оператору нужен файл,
    // файла нет — оператор ставит `| tee`, и код возврата становится кодом `tee`. Так 2026-08-31
    // оба живых прогона отчитались нулём, хотя второй был остановлен спасением.
    //
    // ⚠️ ДИСК ЗДЕСЬ НЕ ТРОГАЕТСЯ ВОВСЕ: файловые вызовы подставлены. Набор, который ради проверки
    // журнала пишет на диск, — это `bugs/08` в ожидании; и он же не смог бы показать РЕЖИМ
    // открытия файла, а именно режим отличает накопление улик от их затирания (`bugs/94`).
    {
      const calls = [];
      const fake = (over = {}) => ({
        mkdirSync: (p, o) => calls.push(['mkdir', p, o?.recursive === true]),
        openSync: (p, flags) => { calls.push(['open', p, flags]); return 77; },
        writeSync: (fd, buf) => { calls.push(['write', fd, String(buf)]); return 0; },
        closeSync: (fd) => calls.push(['close', fd]),
        ...over,
      });

      // (1) БЕЗ ФЛАГА ЗЕРКАЛО НЕ СТАВИТСЯ ВОВСЕ. Это половина, без которой вторая ничего не значит:
      // сторож, который «работает» и когда его не просили, — не сторож, а всегда включённый шум.
      const before = process.stdout.write;
      const off = startRunLog(['--sweep', '--from', '2842'], fake());
      ok('bugs/93: без `--log` журнал не поднимается и поток НЕ подменён',
        [off.path, process.stdout.write === before, calls.length], [null, true, 0]);
      off.stop(0);

      // (2) ИМЯ ФАЙЛА СТАВИТ МАШИНА, А НЕ ОПЕРАТОР — решение владельца 2026-08-31: *«есть дата и
      // время на машине — так логи и называть»*. Оператор даёт СМЫСЛ, и смысл становится
      // поддиректорией внутри одной общей директории журналов. Проверяется обе половины: что штамп
      // пришёл из часов машины и что смысл лёг папкой, а не в имя файла.
      calls.length = 0;
      const named = startRunLog(['--sweep', '--log', 'peremer-2872'],
        fake({ now: () => '2026-08-31T20:15:07+03:00' }));
      named.stop(null);
      ok('bugs/93: имя журнала — ДАТА И ВРЕМЯ МАШИНЫ, смысл прогона — поддиректория',
        [named.path === join(RUN_LOGS_DIR, 'peremer-2872', '2026-08-31T20-15-07+03-00.log'),
          named.path?.startsWith(RUN_LOGS_DIR)],
        [true, true]);

      // (3) СМЫСЛ НЕ НАЗВАН — БЕРЁТСЯ ИЗ РЕЖИМА, А НЕ ВЫДУМЫВАЕТСЯ. И `--log --dry-run` при этом
      // НЕ создаёт папку с именем «--dry-run»: назвать файл нечем, значит и промахнуться нечем.
      calls.length = 0;
      const bare = startRunLog(['--sweep', '--log', '--dashboard'],
        fake({ now: () => '2026-08-31T20:15:07+03:00' }));
      bare.stop(null);
      const dry = startRunLog(['--sweep', '--dry-run', '--log'],
        fake({ now: () => '2026-08-31T20:15:07+03:00' }));
      dry.stop(null);
      ok('bugs/93: смысл по умолчанию берётся из РЕЖИМА, флаг следом за `--log` именем не станет',
        [dirname(bare.path), dirname(dry.path)],
        [join(RUN_LOGS_DIR, 'sweep'), join(RUN_LOGS_DIR, 'dry-run')]);

      // (4) ФАЙЛ ОТКРЫВАЕТСЯ НА ДОПИСЫВАНИЕ, А НЕ НА ПЕРЕЗАПИСЬ. Решение владельца по `bugs/94`
      // дословно: *«можем накопить случаи, да, это более научный метод»*. Журнал прогона — улика
      // того же рода: два прогона в одну секунду обязаны сложиться, а не затереть друг друга.
      calls.length = 0;
      const log = startRunLog(['--sweep', '--log'], fake());
      const opened = calls.find((c) => c[0] === 'open');
      // 🔴 `stop` ЗДЕСЬ ОБЯЗАТЕЛЕН, И ЭТО НЕ ОПРЯТНОСТЬ. Зеркало ставится на ГЛОБАЛЬНЫЙ поток
      // процесса: не сняв его, этот блок оставил бы подмену на всю остальную батарею. Найдено
      // мутацией М2 — набор погиб целиком, не напечатав ни строки, вместо того чтобы покраснеть
      // одним блоком. Прибор, переживший то, ради чего его завели, — класс, за который в этом
      // проекте уже заплачено (сэмплер на десять часов, `stopSideCar` в `finally`).
      log.stop(null);
      ok('bugs/93: журнал открыт на ДОПИСЫВАНИЕ (улика копится, а не затирается)',
        [log.path !== null, opened?.[2], calls.some((c) => c[0] === 'mkdir' && c[2] === true)],
        [true, 'a', true]);

      // (4) ЗЕРКАЛО ЗЕРКАЛИТ, А НЕ ПОДМЕНЯЕТ: строка обязана уйти И в файл, И в исходный поток.
      // Мутация «вернуть только запись в файл» краснит здесь — и краснит по существу: прогон, чей
      // вывод ушёл только в файл, оставляет владельца у машины перед тёмным экраном.
      // 🔴 «ПОТОК ВОЗВРАЩЁН» ПРОВЕРЯЕТСЯ ПОВЕДЕНИЕМ, А НЕ ТОЖДЕСТВОМ ФУНКЦИИ, И ЭТО ИСПРАВЛЕНИЕ
      // МОЕГО ЖЕ БЛОКА. Первая редакция спрашивала «поток определён?» — и мутация «не возвращать
      // поток вовсе» прошла ЗЕЛЁНОЙ: подменённый поток тоже определён. Сравнить с исходной функцией
      // напрямую тоже нельзя — `startRunLog` держит её `bind`-копию, а копия никогда не равна
      // оригиналу. Поэтому спрашиваем то, что и важно на деле: после закрытия строка обязана уйти
      // в терминал и НЕ уйти в закрытый журнал. ([[EXP-0216]]: сторож, доказанный в одну сторону.)
      const seen = [];
      const prevOut = process.stdout.write;
      process.stdout.write = (chunk) => { seen.push(String(chunk)); return true; };
      const log2 = startRunLog(['--log', 'zerkalo'], fake());
      process.stdout.write('строка прогона\n');
      log2.stop(7);
      process.stdout.write('строка ПОСЛЕ закрытия\n');
      process.stdout.write = prevOut;
      const inFile = (s) => calls.some((c) => c[0] === 'write' && c[2] === s);
      ok('bugs/93: вывод уходит И в файл, И в терминал; после `stop` журнал больше не пишется',
        [seen.includes('строка прогона\n'), inFile('строка прогона\n'),
          seen.includes('строка ПОСЛЕ закрытия\n'), inFile('строка ПОСЛЕ закрытия\n')],
        [true, true, true, false]);

      // (5) КОД ВОЗВРАТА ДОПИСЫВАЕТСЯ В САМ ЖУРНАЛ. Это вторая половина починки: файл, по которому
      // разбирают вечер, обязан САМ отвечать «прогон удался?». Иначе улику опять надо где-то
      // перехватывать — а `bugs/93` ровно про улику, живущую только пока её кто-то ловит.
      ok('bugs/93: журнал сам несёт исход прогона последней строкой, и закрывается',
        [calls.some((c) => c[0] === 'write' && c[2] === 'КОД ВОЗВРАТА: 7\n'),
          calls.some((c) => c[0] === 'close' && c[1] === 77)],
        [true, true]);

      // (6) ОТКАЗ ЗАПИСИ В ЖУРНАЛ НЕ ИМЕЕТ ПРАВА УБИТЬ ПРОГОН, который в этот момент держит карту.
      // Диск кончился — это причина потерять журнал, а не причина бросить видеокарту в изменённом
      // состоянии. Барьер тот же, что у сэмплера: прибор не старше того, за чем он смотрит.
      const prevOut2 = process.stdout.write;
      process.stdout.write = () => true;
      let survived = true;
      try {
        const boom = startRunLog(['--log', 'otkaz-diska'],
          fake({ writeSync: () => { throw new Error('на диске нет места'); } }));
        process.stdout.write('строка под отказавшим журналом\n');
        boom.stop(0);
      } catch { survived = false; }
      process.stdout.write = prevOut2;
      ok('bugs/93: отказ записи в журнал прогон НЕ роняет — прибор не старше того, за чем смотрит',
        survived, true);
    }
 ...
 */
/**
 * ⚡ `bugs/110` Ш0 — ВХОДЫ, КОТОРЫЕ ПИШУТ В КАРТУ, НО ЗАЩИТУ НЕ ПОДНИМАЮТ, ОТКАЗЫВАЮТ НА ЖИВОМ ПУТИ.
 *
 * Найдено живым прогоном 2026-09-07 при владельце у машины: пролог защиты (ворота окна · судья ·
 * проба · вход 2 · сейлок) лежит ЦЕЛИКОМ внутри `mainSweep`. `mainBand` и ветка `--search` пишут в
 * карту теми же `searchEdge` + `vf.runStep` и не поднимают ничего — то есть прогон идёт без
 * снятия нагрузки при зависании, без возврата стока и без окна наблюдения, запрещённого владельцем
 * («прогоны без визуализатора ПОД СТРОГИМ ЗАПРЕТОМ», 2026-08-16).
 *
 * 🔴 ЭТО ЗАГЛУШКА, А НЕ ПОЧИНКА, и названа так намеренно. Настоящая починка — `plans/89`: вынести
 * пролог в ОДНО место и позвать его отовсюду. Заглушка стоит здесь потому, что безопасное состояние
 * достижимо за минуты, а извлечение — часы аккуратной хирургии в самом безопасно-критичном месте
 * проекта, и оставлять дыру открытой на это время незачем: нижняя полоса `--band` целиком
 * достижима через `--sweep` (проверено живым прогоном 2370→2167 в тот же день).
 *
 * ЧИСТАЯ ФУНКЦИЯ — возвращает строки, а не печатает: решение проверяется блоком без запуска движка.
 * Снятие входа из списка ОБЯЗАНО идти вместе с проводкой пролога в него, и за этим следит блок.
 *
 * @param {string} mode имя режима, как его пишет владелец в командной строке
 * @returns {string[]|null} строки отказа, либо `null` — вход защищён и работать ему можно
 */
/**
 * Входы, у которых пролог защиты ЕСТЬ. Разделение на два списка — не украшение: по мере исполнения
 * `plans/89` режимы ПЕРЕЕЗЖАЮТ отсюда-туда, и блок покрытия проверяет объединение, а не каждый
 * список по отдельности. Так шаг Ш2 не потребует править ни блок, ни его ожидания.
 */
/**
 * ⚡ `bugs/112` — ХВОСТ СТРОКИ ФОРМЫ С ЖИВЫМИ ЧТЕНИЯМИ КАРТЫ, И ОН ЧИСТАЯ ФУНКЦИЯ НАМЕРЕННО.
 *
 * «Утечка потолка» и «пол железа» читаются с ЖИВОЙ карты и едут с её текущей частотой: два сухих
 * прогона ПОДРЯД на одном коде расходятся ровно на этой строке (замер 07.09: 5 МГц / 2172 против
 * 20 / 2187). Пока они стоят вперемешку с выведенной из политики ФОРМОЙ, сухой прогон нельзя
 * сличать с самим собой — то есть он не годится в эталон, чего от него требует `plans/89` AC1.
 *
 * 🔴 ПО УМОЛЧАНИЮ ВЫВОД НЕ ТРОНУТ НИ НА БАЙТ: эту строку читает ГЛАЗ ВЛАДЕЛЬЦА перед прогоном, а
 * на том, что он видит, агент не экономит (граница молитвы). `stable` нужен МАШИНЕ — сличению
 * эталонов, — и включается только флагом `--stable`.
 *
 * Вынесено в функцию, чтобы у флага был БЛОК: решение, живущее строкой внутри `console.log`,
 * проверяется только глазами, а флаг без теста в этом проекте не считается сделанным.
 *
 * @param {{shape:string, vec:{capLeakMhz:*, lowestEnforceableCapMhz:*}, stable:boolean}} a
 * @returns {string} хвост строки — пустой, если форма отгружаемая или запрошен стабильный вывод
 */
export function capLeakNote({ shape, vec, stable = false } = {}) {
  // Отгружаемая форма потолок не «протекает» — у неё этого чтения нет вовсе, и молчание тут не
  // экономия, а отсутствие предмета.
  if (shape === 'raise-and-cap') return '';
  if (stable) return '';
  return ` · утечка потолка ${vec?.capLeakMhz} МГц, пол железа ${vec?.lowestEnforceableCapMhz} МГц`;
}

export const GUARDED_ENTRY_POINTS = Object.freeze(['--sweep']);

export const UNGUARDED_ENTRY_POINTS = Object.freeze(['--band', '--search']);

export function unguardedEntryRefusal(mode) {
  if (!UNGUARDED_ENTRY_POINTS.includes(mode)) return null;
  return [
    `ОТКАЗ: РЕЖИМ ${mode} ПИШЕТ В КАРТУ, НО АВАРИЙНУЮ ЗАЩИТУ НЕ ПОДНИМАЕТ (bugs/110).`,
    '',
    '       Судья, проба живости, вход 2 по прогрессу прожига и ворота окна наблюдения заведены',
    '       только в `--sweep`. Этот режим пишет в карту теми же руками и не поднимает ничего:',
    '       зависнет карта — нагрузку никто не снимет и сток никто не вернёт.',
    '',
    '       Найдено живым прогоном 2026-09-07. Починка — `plans/89` (один пролог на все входы).',
    '',
    `       ЧТО СДЕЛАТЬ СЕЙЧАС: то же самое через \`--sweep\`, он защищён и проверен на железе.`,
    '       Диапазоном: `npm run engine -- --sweep --from <МГц> --to <МГц>`.',
    '',
    `       План читается без защиты и карту не трогает: добавьте \`--dry-run\`.`,
  ];
}
  ...
  for (const pin of pins) {
    ...
    if (serving) {
      ...
      console.log(`          ФОРМА: ${shapeChoice.shape === 'raise-and-cap' ? 'ОТГРУЖАЕМАЯ (подъём с потолком)' : 'равномерный подъём'}`
        ...
        + capLeakNote({ shape: shapeChoice.shape, vec, stable: stableOut }));
    }
  }
  console.log('');
  if (dryRun) { console.log('СУХОЙ ПРОГОН: ни одной записи в карту не сделано.'); return 0; }

  // ⚡ `bugs/110` Ш0 — СТОИТ ПОСЛЕ возврата по `--dry-run` намеренно: план читается без защиты и
  // карту не трогает, а отказывать плану значило бы отнять у владельца чтение вместе с записью.
  {
    const refusal = unguardedEntryRefusal('--band');
    if (refusal) { for (const line of refusal) console.error(line); return 2; }
  }
...
async function mainSweep(argv, arg) {
  ...
  // ─── ЗАПРЕТ ВЛАДЕЛЬЦА: ПРОГОН НЕДОПУСТИМ, ПОКА ОТКРЫТЫ НАЗВАННЫЕ ИМ ТИКЕТЫ ──────────────────────
  //
  // Слово владельца 2026-08-31, дословно: *«bugs/79 открыт: драйвер кричал 110 секунд, и у канала
  // до сих пор нет голоса. bugs/77 открыт: шаг меряется не от стока. — чиним это в новой сессии.
  // Без них прогон НЕДОПУСТИМ»*.
  //
  // 🔴 ПОЧЕМУ ЭТО КОД, А НЕ СТРОКА В ЭСТАФЕТЕ. За одну ночь 30→31 августа проект дважды заплатил
  // за правило, жившее прозой: канон требовал объявлять угрозу сторожа — не работало, пока не стало
  // линтером (`plans/75`); канон разрешал слать баги KAIF наверх — не работало, потому что общее
  // правило рядом читалось чаще (`bugs/KAIF/16` → origin #37). Запрет, который держится на том, что
  // следующая сессия дочитает STATUS до нужного абзаца, — это не запрет.
  //
  // САМОСНИМАЮЩИЙСЯ ПО ПОСТРОЕНИЮ: гейт смотрит на имена файлов в `bugs/`. Тикет получил тег `DONE`
  // по общей конвенции проекта — гейт открылся сам, править код не нужно и забыть нечего.
  if (!twin) {
    const blocking = liveRunBlockers();
    if (blocking.length) {
      console.error('ОТКАЗ: ЖИВОЙ ПРОГОН ЗАПРЕЩЁН ВЛАДЕЛЬЦЕМ, ПОКА ОТКРЫТЫ ЭТИ ТИКЕТЫ.');
      for (const b of blocking) console.error(`       🔴 bugs/${b.id} — ${b.why}`);
      console.error('');
      console.error('       Слово владельца 2026-08-31: «Без них прогон НЕДОПУСТИМ».');
      console.error('       Оба тикета — про то, что 30 августа не остановило смерть машины:');
      console.error('       у канала драйвера нет голоса, и шаг спуска меряется не от стока.');
      console.error('');
      console.error('       Гейт снимается САМ, когда тикет получает тег DONE по конвенции проекта');
      console.error('       (git mv bugs/NN_x.md bugs/NN_DONE_x.md). Снимать его правкой кода —');
      console.error('       значит отменять слово владельца молча.');
      return 2;
    }
    console.log('ЗАПРЕТ ВЛАДЕЛЬЦА (bugs/77, bugs/79): снят — оба тикета закрыты');
  }
  ...
  // ⚡ ПУТЬ ЖУРНАЛА СУДЬИ НАЗЫВАЕТ ВЫЗЫВАЮЩИЙ, А НЕ САМ СУДЬЯ (`bugs/88`, Ш4 `plans/81`).
  //
  // До этого живой судья выбирал имя сам (`<штамп>-fuse.jsonl`), и развёртка своего же журнала не
  ...
  // ответили бы разное на один вопрос. Нечитаемо и пусто здесь ОДНО И ТО ЖЕ состояние: файл
  // создаёт сама защита при старте, поэтому «файла нет» честно означает «срабатываний не было».
  const fuseLines = () => {
    try { return readFileSync(fuseJournalPath, 'utf8').split(/\r?\n/u); } catch { return []; }
  };
  // Печатать «жду АПВ» на каждом опросе (4 раза в секунду) значило бы залить экран владельца во
  // время и без того тревожной минуты. Одна строка на вход в ожидание, одна на выход.
  ...
  try {
    ...
    // ⚡ Ш5 (`plans/81`): ПОЛОСА ЖДЁТ АПВ — АВТОМАТИЧЕСКОГО ПОВТОРНОГО ВКЛЮЧЕНИЯ ЗАЩИТЫ.
    //
    // До Ш5 срабатывание САЗ кончало вечер: защита выходила, полоса вставала, 31.08 закрыто 0 из 20
    // частот. Теперь САЗ переживает своё срабатывание — но между ним и АПВ есть окно (измерено:
    // 1,9 с на здоровой карте, 58,4 с на больной), и в этом окне полоса ЖЕЧЬ НЕ СМЕЕТ: прожиг без
    // взведённой защиты запрещён фазой 5 (`plans/58`) и не перестал быть запрещённым оттого, что
    // защита жива. Поэтому здесь ОЖИДАНИЕ, а не пропуск.
    //
    // 🔴 ГРАНИЦА ОЖИДАНИЯ — САМА ЗАЩИТА, А НЕ ВЫДУМАННОЕ ВРЕМЯ. Решение «можно ли включаться
    // повторно» принимает ОДИН узел — САЗ, по расписке ВЗН; полоса его РЕШЕНИЕ ЧИТАЕТ, а не
    // повторяет. Повторить значило бы завести пару «правда ↔ зеркало»: разойдясь, две копии не
    // покраснели бы, а тихо разрешили прожиг без защиты. Не смогла включиться — САЗ выходит, и
    // цикл кончается сам, следующей же строкой.
    //
    // [NOT-TESTED: живой путь. Ветки доказаны на двойнике и блоками; живой свидетель — вечерний
    //  прогон при владельце (ворота 6a): в протоколе обязана появиться строка АПВ]
    stopWhen: async () => {
      while (fuseJudge.exitCode === null) {
        const lines = fuseLines();
        // Срабатываний больше, чем повторных включений → защита сейчас спасает машину и на пост
        // ещё не вернулась. Это и есть прибор критерия P81-AC4.
        if (fuseMod.tripCount(lines) <= fuseMod.rearmCount(lines)) break;
        if (!waitingForRearm) {
          waitingForRearm = true;
          console.log('  ⏳ ЖДУ АПВ: аварийная защита спасает машину; следующая ступень не начнётся, пока защита не вернётся на пост');
        }
        await new Promise((r) => { setTimeout(r, fuseMod.REARM_POLL_MS); });
      }
      if (waitingForRearm && fuseJudge.exitCode === null) {
        waitingForRearm = false;
        console.log(`  ✅ АПВ УСПЕШНО: защита снова на посту (срабатываний за прогон: ${fuseMod.tripCount(fuseLines())}) — тюнинг продолжается`);
      }
      return fuseJudge.exitCode === null ? null
        : (fuseJudge.exitCode === 2
          ? { why: `АПВ НЕУСПЕШНО — аварийная защита не смогла вернуться на пост: читай протокол в ${fuseJournalHint}`, rescue: true }
          : { why: `аварийная защита погибла сама (код ${fuseJudge.exitCode}) — прожиг без защиты запрещён`, rescue: false });
    },
    ...
    // ⚡ СПАСЕНИЕ ПРЕДОХРАНИТЕЛЯ — ВХОД СТУПЕНИ (`bugs/88`, решение владельца `interviews/023`).
    // Читается ФАЙЛ, который судья `fsync`-ит ПЕРВЫМ делом трипа, — то есть строка существует даже
    // у спасения, не дожившего до конца своих рук. Считается ЧИСЛО намерений, не время: часы у
    // судьи и у движка разные, и сравнивать их концы — оплаченная ловушка ([[EXP-0207]]).
    // Нет файла (трипов не было ни одного) — честный ноль, а не молчание: файл создаёт сам судья
    // при старте, поэтому «нечитаемо» и «пусто» здесь одно и то же состояние.
    //
    // [TESTED: 2026-08-31 · ЛОГИКА входа — 5 блоков `engine --selftest` и 5 мутаций (FA·FB·FD·FE·FF),
    //  плюс сквозной блок на двойнике `twin --selftest` → «ЛЕЧЕНИЕ НА ДВОЙНИКЕ»]
    // [NOT-TESTED: ЭТА ПРОВОДКА на ЖИВОМ пути — настоящий судья, настоящий трип, настоящий файл —
    //  не бегала ни разу. Доказано, что ступень правильно РЕАГИРУЕТ на растущий счёт; не доказано,
...
/**
 * ОДНА ДИРЕКТОРИЯ НА ВСЕ ЖУРНАЛЫ ПРОГОНОВ — слово владельца 2026-08-31: *«в одну директорию
 * складывать, внутри можно делать поддиректории с названием смысла прогонов»*.
 */
export const RUN_LOGS_DIR = join(ROOT, 'runs', 'logs');

/**
 * СМЫСЛ ПРОГОНА ПО УМОЛЧАНИЮ — имя поддиректории, когда оператор его не назвал.
 *
 * Берётся из РЕЖИМА, а не из настроения: режим — это и есть смысл в самом грубом приближении, и он
 * уже стоит в командной строке. Выдумывать здесь ничего не надо, а «прочее» честнее, чем красивое
 * имя для того, чего мы не распознали.
 */
export function runMeaningOf(argv) {
  if (argv.includes('--dry-run')) return 'dry-run';
  if (argv.includes('--sweep')) return 'sweep';
  if (argv.includes('--band')) return 'band';
  if (argv.includes('--search')) return 'search';
  return 'run';
}

/**
 * СВОЙ ЖУРНАЛ У ПРОГОНА (`--log [смысл]`) — ПОЧИНКА `bugs/93`, И ОНА ЛЕЧИТ ПРИЧИНУ, А НЕ ПРИВЫЧКУ.
 *
 * Вечером 2026-08-31 оба живых прогона отдали КОД ВЫХОДА 0, хотя второй был остановлен спасением:
 * запускались они через `| tee runs/….log`, и код принадлежал `tee`, а не движку. Различитель,
 * ради которого три недели строился `bugs/67` («остановка обязана быть видна МАШИНЕ, а не только
 * человеку в терминале»), был стёрт одним символом в командной строке.
 *
 * 🔴 ПОЧЕМУ ЭТО НЕ ЛЕЧИТСЯ СТОРОЖЕМ НА «НЕ СТАВЬ КОНВЕЙЕР». Сторожу неоткуда узнать, как его
 * запустили: процесс не видит своего конвейера. И дело не в аккуратности оператора — оператору
 * нужен ФАЙЛ (разбор, эстафета, форензика). Файла нет — появляется `tee`, и так поступит любая
 * следующая сессия, у меня это вышло дважды за один вечер. Причина — отсутствующая возможность;
 * даём файл, и конвейер становится не нужен.
 *
 * 🔴 ПИШЕТСЯ СИНХРОННО, И ЭТО НЕ ПЕДАНТИЗМ. Поток (`createWriteStream`) буферизует, а `process.exit`
 * обрывает процесс, не дожидаясь слива — то есть последние строки прогона, ровно те, где написана
 * причина остановки, потерялись бы именно в аварийном случае. Это класс `bugs/37`: улика гибнет
 * тогда же, когда становится нужна. `writeSync` доводит каждую строку до дескриптора немедленно;
 * потерять можно лишь то, что не успело в него войти при смерти машины.
 *
 * Открывается на ДОПИСЫВАНИЕ (`'a'`), а не на перезапись: файл журнала — улика, и второй запуск с
 * тем же именем обязан копить, а не затирать (то же решение владельца, что и в `bugs/94`:
 * *«можем накопить случаи, да, это более научный метод»*).
 *
 * @returns {{path: string|null, note: (text: string) => void, stop: (code: number|null) => void}}
 */
export function startRunLog(argv, io = {}) {
  const fs = {
    openSync, writeSync, closeSync, mkdirSync, now: localIso, ...io,
  };
  const i = argv.indexOf('--log');
  if (i < 0) return { path: null, note: () => {}, stop: () => {} };
  // ⚡ ИМЯ ЖУРНАЛА СТАВИТ МАШИНА, А НЕ ОПЕРАТОР — РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-08-31, ДОСЛОВНО:
  // *«не надо называть vecher · есть дата и время на машине — так логи и называть, в одну
  // директорию складывать, внутри можно делать поддиректории с названием смысла прогонов»*.
  //
  // Поэтому значение флага — это СМЫСЛ прогона (поддиректория), а НЕ имя файла. Имя файла всегда
  // дата и время: имена, придуманные под настроение («vecher», «test2», «final-final»), через
  // неделю не отвечают на вопрос «какой это был прогон», а штамп отвечает всегда и сам себя
  // сортирует. Побочно это убирает целый класс опечаток: назвать файл нечем, значит и промахнуться
  // именем нельзя — `--log --dry-run` просто ложится в смысл по умолчанию.
  const raw = argv[i + 1];
  const meaning = raw && !raw.startsWith('--') ? raw : runMeaningOf(argv);
  // Разделители в смысле резались бы на поддиректории неожиданно для оператора: смысл — ОДНО имя.
  const safe = String(meaning).replace(/[^\p{L}\p{N}_-]+/gu, '-').replace(/^-+|-+$/g, '') || 'run';
  // Двоеточия часового пояса и времени в имени файла Windows не примет — режем их, но СОДЕРЖАНИЕ
  // штампа не трогаем: он остаётся читаемым и сортируемым как строка.
  const stamp = fs.now().replace(/:/g, '-');
  const dir = join(RUN_LOGS_DIR, safe);
  const path = join(dir, `${stamp}.log`);
  fs.mkdirSync(dir, { recursive: true });
  const fd = fs.openSync(path, 'a');
  const outWrite = process.stdout.write.bind(process.stdout);
  const errWrite = process.stderr.write.bind(process.stderr);
  // Отказ ЗАПИСИ В ЖУРНАЛ НЕ ИМЕЕТ ПРАВА УБИТЬ ПРОГОН, который в этот момент держит карту: диск
  // может кончиться, дескриптор — закрыться. Зеркало молчит, оригинал идёт дальше.
  const mirror = (chunk) => {
    try { fs.writeSync(fd, typeof chunk === 'string' ? chunk : Buffer.from(chunk)); } catch { /* журнал не старше прогона */ }
  };
  process.stdout.write = (chunk, enc, cb) => { mirror(chunk); return outWrite(chunk, enc, cb); };
  process.stderr.write = (chunk, enc, cb) => { mirror(chunk); return errWrite(chunk, enc, cb); };
  const note = (text) => { mirror(`${text}\n`); };
  return {
    path,
    note,
    // КОД ВОЗВРАТА ДОПИСЫВАЕТСЯ В САМ ЖУРНАЛ, и это вторая половина починки. Файл, по которому
    // разбирают вечер, обязан сам отвечать на вопрос «прогон удался?» — иначе улику снова придётся
    // где-то перехватывать, а `bugs/93` ровно про это. В терминале строки нет: печать в stdout
    // изменила бы существующий вывод, а вопрос читателя журнала и вопрос зрителя за машиной разные.
    stop: (code) => {
      if (code !== null && code !== undefined) note(`КОД ВОЗВРАТА: ${code}`);
      process.stdout.write = outWrite;
      process.stderr.write = errWrite;
      try { fs.closeSync(fd); } catch { /* закрывать нечего — прогон это переживает */ }
    },
  };
}

async function main(argv) {
  // ⚡ `bugs/118`: СПРАВКА ПЕРВОЙ ВЕТКОЙ, ДО ЛЮБОГО РАЗБОРА РЕЖИМА. До 2026-09-08 `engine.mjs --help`
  // печатал «ОШИБКА: нужен один из режимов» и уходил ненулевым кодом — то есть попытка узнать о
  // главном приборе проекта выглядела как отказ. Прибор, пишущий в карту, обязан уметь рассказать
  // о себе, ничего не сделав.
  if (argv.includes('--help') || argv.includes('-h')) {
    console.log('Использование: node automation-engine/engine.mjs <режим> [флаги]\n'
      + '\nРЕЖИМЫ (ровно один обязателен):\n'
      + '  --sweep --from <МГц> --to <МГц>   развёртка полосы сверху вниз\n'
      + '  --band <частоты через запятую>    отдельные частоты\n'
      + '  --search --cap <МГц>              поиск края под потолком\n'
      + '  --selftest                        батарея движка (карта не нужна)\n'
      + '\nЧАСТЫЕ ФЛАГИ:\n'
      + '  --card virtual        прогон на ДВОЙНИКЕ: карта владельца не трогается\n'
      + '  --max-depth <мВ>      потолок глубины спуска от стока; без него спуск идёт до рычага\n'
      + '  --dashboard           писать в окно наблюдения (порт 7311 поднимается отдельно)\n'
      + '  --log <метка>         свой журнал прогона в runs/logs/<метка>/\n'
      + '  --dry-run             напечатать план, ничего не жечь\n'
      + '\n⚠️ ЖИВОЙ ПРОГОН ПИШЕТ В ВИДЕОКАРТУ. Условия — открытое окно наблюдения и владелец у машины.');
    return 0;
  }
  ...
  // ⚡ `bugs/110` Ш0 — та же заглушка, тот же довод: после сухого прогона, до первой записи.
  {
    const refusal = unguardedEntryRefusal('--search');
    if (refusal) { for (const line of refusal) console.error(line); return 2; }
  }
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  ...
  // ⚡ ЖУРНАЛ ПОДНИМАЕТСЯ ЗДЕСЬ, А НЕ ВНУТРИ `main`, И ЭТО НЕ ВКУС (`bugs/93`).
  //
  // У `main` два десятка точек выхода (`return 2` на каждом разборе аргументов), и обернуть её тело
  // в `try/finally` значило бы двадцать шансов промахнуться мимо одного. Здесь выход РОВНО ОДИН —
  // и обе его двери, удачная и через исключение, закрывают журнал. Отказ, ушедший в лог без кода
  // возврата, — это и есть дефект, который чинится, только в новой обёртке.
  const runLog = startRunLog(argv);
  if (runLog.path) {
    runLog.note(`\n=== ПРОГОН ${localIso()} · аргументы: ${argv.join(' ')}`);
    console.log(`ЖУРНАЛ ПРОГОНА: ${runLog.path} — конвейер не нужен, код возврата дойдёт до вызывающего`);
  }
  main(argv).then(
    (code) => { runLog.stop(code); process.exit(code); },
    (e) => {
      // Исключение — тоже исход прогона, и журнал обязан его унести. Печатаем ДО `stop`, пока
      // зеркало ещё стоит, иначе разбор упадёт на файл, кончающийся на полуслове.
      console.error(`ОШИБКА ПРОГОНА: ${e?.stack ?? e}`);
      runLog.stop(1);
      process.exit(1);
    },
  );
