...
export async function sweepRange({
  ...
  // ─── И УРОЖАЙ ЕДЕТ В ДОКУМЕНТ (`plans/41` фаза 3, решение владельца `interviews/014` Q1 = A) ───
  //
  // Пишется ПОСЛЕ всей полосы, а не по ходу: строки спуска ложатся первыми, и урожай заполняет
  // только то, до чего спуск не дошёл. Прерванная полоса сюда всё равно попадает (`break` выходит
  // из цикла, а не из функции); теряет урожай лишь смерть машины, и тогда его отдаёт журнал.
  ...
  if (harvested.written.length) {
    ...
    if (saveFn) {
      const saved = await saveFn(doc);
      if (saved && saved.ok === false) {
        // ⚠️ НЕ РОНЯЕТ ПРОГОН. Края уже сохранены выше своим `saveFn`; урожай — побочный продукт, и
        // терять из-за него найденный край было бы ровно тем «строгим стендом», который владелец
        // отменил («здание важнее лесов»). Отказ НАЗЫВАЕТСЯ и едет в сводку.
        report.harvestRefusals = [...report.harvestRefusals,
          `урожай не сохранён: ${saved.why ?? 'без причины'} — края прогона при этом на диске`];
        report.harvestedRows = [];
      }
    }
    say('harvest', `урожай в документ: строк ${harvested.written.length} `
      + `(${harvested.written.slice(0, 6).map((w) => `${w.mhz} МГц ← ${w.voltageMv} мВ`).join(', ')}`
      + `${harvested.written.length > 6 ? ' …' : ''})`, { rows: harvested.written.length });
  }
  report.doc = doc;
