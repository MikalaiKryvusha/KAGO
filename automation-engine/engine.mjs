...
export const STALL_KIND = Object.freeze({
  ...
  // ⚡ ТРЕТИЙ РОД (Ш6 `plans/81`): СРАБАТЫВАНИЕ АВАРИЙНОЙ ЗАЩИТЫ.
  //
  // Он появился НЕ для полноты, а по замеру: репетиция смерти на двойнике 2026-08-31 после Ш5 дала
  // **девять срабатываний на полосе из трёх частот** — защита исправно спасала машину и исправно
  // возвращалась на пост, а полоса шла дальше и не жгла НИЧЕГО. Пережитое срабатывание перестало
  // останавливать вечер (это и была цель Ш5) — и ровно поэтому обязано КОПИТЬСЯ: иначе на 389
  // частотах стенд отработает всю ночь, спасая машину от самого себя. Это риск (б) плана 81,
  // названный заранее и теперь измеренный.
  //
  // Род ОТДЕЛЬНЫЙ, а не «подстой пульса», потому что довод другой: подстой — наблюдение прибора о
  // ступени, срабатывание — факт, что машину пришлось спасать. Складывать их в один счёт можно
  // (порог общий), путать их имена нельзя — причина остановки не должна врать про свой состав.
  rescue: 'срабатывание аварийной защиты',
});
  ...
  for (const g of groups) {
    ...
    if (outcome.halted || outcome.verdict === null) {
      ...
      const LEDGER_SOURCES = {
        ...
        // ⚡ Ш6: ТРЕТИЙ ИСТОЧНИК — САМА АВАРИЙНАЯ ЗАЩИТА. Ключ берётся из константы, а не из
        // литерала: разойдясь с тем, что пишет ступень, эта строка не покраснела бы, а молча
        // перестала считать — ровно дефект `bugs/84`, где накопитель за всю жизнь не получил ни
        // одной записи, потому что читал поле, которого никто не выставляет.
        [DECIDED_BY_FUSE]: STALL_KIND.rescue,
      };
...
export function selfTest() {
  ...
  return (async () => {
    ...
    // =============================================================================================
    {
      ...
      // ═══════════════════════════════════════════════════════════════════════════════════════════
      // ⚡ Ш6 `plans/81` — ЧЕРЕДА СРАБАТЫВАНИЙ УПИРАЕТСЯ В НАКОПИТЕЛЬ (риск (б), ИЗМЕРЕННЫЙ)
      // ═══════════════════════════════════════════════════════════════════════════════════════════
      //
      // Замер, породивший этот блок: репетиция смерти на двойнике 2026-08-31 ПОСЛЕ Ш5 дала
      // 9 срабатываний на полосе из ТРЁХ частот — защита исправно спасала, полоса исправно шла
      // дальше, и не сожгла ничего. Пережитое срабатывание перестало останавливать вечер, значит
      // обязано КОПИТЬСЯ, иначе на 389 частотах стенд отработает всю ночь впустую.
      {
        let trips = 0;
        const halted = await sweepRange({
          envelopeMhz: 3090,
          curveDoc: sweepDoc(bandRows), points: sweepPoints,
          // Каждая ступень видит рост счёта срабатываний — то есть под ней сработала защита.
          runStepFn: async (args) => sweepAtom(980)(args),
          fuseTripsFn: () => trips++,
          buildVector: vectorPinned,
          saveFn: async () => ({ ok: true }),
        ...
        ok('Ш6: срабатывания защиты ДОЕЗЖАЮТ до накопителя СВОИМ родом (не «подстой пульса»)',
          [...new Set(halted.stalls.map((x) => x.kind))], [STALL_KIND.rescue]);
        ok('Ш6: череда срабатываний ОСТАНАВЛИВАЕТ полосу — тормоз против бесконечного спасения',
          halted.haltedBy, 'накопитель предупреждений');
      }
