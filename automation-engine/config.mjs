...
/**
 * ПОРОГ СТУПОРА СЭМПЛЕРА — ВЫВЕДЕН, НЕ НАЗНАЧЕН (`bugs/61` шаг 2, замер 2026-08-29).
 *
 * Правило вывода: **2 × максимум фонового зазора** между пробами сэмплера телеметрии.
 * По ПОЛНОМУ архиву `runs/telemetry/` (20 прогонов): фон — 18 прогонов без смерти машины,
 * max зазор 1022…1065 мс; оба смертельных прогона — 3042 мс (2775 МГц, 26.08) и 3366 мс
 * (2797 МГц, 23.08). Между 1065 и 3042 — ни одной точки. Порог 2 × 1065 = 2130 мс красен на
 * обоих смертельных (запас 43 %) и молчит на всём фоне (запас 100 %).
 *
 * Правило одностороннее — от ФОНА, не от предвестников: его нельзя обвинить в подгонке под две
 * смерти, и на другой машине оно перевыводится из её же фона (та же форма, что
 * `MARGIN_STEPS_ABOVE_LAST_STABLE` и `DERIVED_ARM_N_MS`). Слово владельца, на котором стоит сам
 * вердикт: «лаг телеметрии — это отказ» (`GOAL.md`), «Это даёт вердикт о том, что край найден».
 */
export const PULSE_STALL_MS = 2130;
