...
4. **Защитная машинерия съела пропускную способность и отравила данные.** 5,4 часа реального
   прожига за весь проект против ~100 сессий; 10 из 48 «зависаний» записаны на глубине 25 мВ от
   стока, где сбоить нечему, — это срабатывания самой защиты, ставшие «стенами» в документе кривой.
5. **Канон перерос модели, которые его читают.** Ядро обязательного перечитывания — **809 КБ**,
   порядка 300 тысяч токенов: больше всего рабочего окна модели на 200 тысяч. Сессия физически не
...
| **Реальный прожиг за весь проект** | **≈ 5,4 часа**; вердиктов 965 на 14 датах | `runs/sweep/journal.jsonl` |
...
| Ядро перечитывания (9 документов) | **809 КБ ≈ 300 тыс. токенов** (≈ 2,4 байта на токен у этого текста) | `wc -c` |
...
flowchart TD
    ...
    E --> F["Ядро канона больше окна модели<br/>(809 КБ ≈ 300 тыс. токенов)"]
...
- **Ложные стены.** 10 из 48 записанных отказов — на глубине ≤ 25 мВ от стока (08.09: 1090–1135 мВ
  на 2977–3052 МГц; 30.08: 1015–1035 мВ). Все десять — срабатывания защиты или пульса при живой
  машине. Сам проект нашёл, что **7 стен из 20 противоречат физике** (`plans/92` Ш2, 08.09), а его
  же сторож в `npm run check` сегодня печатает **13 из 28** — но вывод на метод не распространён. А храповик разносит такие стены вверх: строку 2887 МГц он поднял с 875 до
  910 мВ, потому что «более низкая 2692 МГц потребовала больше» — по отказу, записанному зависанием
  (`curves/measured.json`).
...
сильные». Я не считаю, что более сильная модель спасла бы проект в этой системе: 300 тысяч токенов
