...
Владелец ответил 14:15:22 через контур, дословно: *«Не понимаю проблемы и вопроса. Нужно проще
пояснить»* (`choice: null`). **Второй раз за день — и вина не в изложении, а в самом факте вопроса:
это инженерный вопрос, переодетый в развилку владельца.** Разбор ниже, в разделе «Почему этот вопрос
...
**Ответ:** Не понимаю проблемы и вопроса. Нужно проще пояснить
<!-- owner-review:answer at="2026-08-30T14:15:22+03:00" by="Mikalai Kryvusha" -->
...
## ❌ Почему этот вопрос не следовало задавать — разбор агента, 2026-08-30 14:5x

**Что владелец решил 30 августа (`interviews/018` = A):** замер, где карта подставила своё
напряжение вместо заказанного, не поднимает чужие строки. Это ПОЛИТИКА, и она принята.

**Что я вынес ему сюда:** как ОПОЗНАВАТЬ такой замер — по ширине промаха или по повторению заказов.
Это не политика, это измерение, и оно целиком моё. Признак «> 10 мВ» нигде в его словах не стоит —
его выбрал агент как приближение. Заменить приближение на прямой признак — исполнение его решения,
а не новое решение.

**Столкновения с его правилом 2026-08-16 («одна ступень вверх — ПОПАДАНИЕ») не было.** Я его
придумал. То правило отвечает на вопрос «какое число записать в строку»; здесь речь о праве строки
требовать столько же от соседей. Два разных вопроса, и они не спорят.

**Цена ошибки:** два подъёма контура за день на один и тот же предмет, оба вернулись «не понимаю».
Владелец заплатил вниманием дважды за то, что должен был решить я. Урок — **EXP-0196**.

## Что сделано вместо ответа

1. **Признак оставлен включённым** (`bugs/73`): пол карты опознаётся повторением заказов. Это
   исполнение решения от 30.08, доказано мутациями EL · EM · EN, батарея зелёная. Отменяется одной
   строкой, если владелец решит иначе.
2. **Ремонт 11 строк НЕ СДЕЛАН, и причина измерена, а не выбрана.** Возврат «того, что храповик
   забрал» **не даёт валидного документа**: значения, которые храповик перезаписал, ПРОТИВОРЕЧАТ
   ДРУГ ДРУГУ в двух местах.
   ...
   Сторож документа такой возврат отверг (сухой прогон, на диск ничего не поехало). **То есть
   храповик 26 августа не только отобрал 45 мВ — он ещё и ЗАМАЗАЛ два настоящих противоречия между
   честными прожигами.** Развести их можно двумя путями: моей арифметикой по его данным или честным
   перезамером. Выбран второй: 895 мВ — это БОЛЬШЕ напряжения, то есть безопасная сторона, ничем не
   рискуем, а вечерний прогон эти же частоты меряет заново и вернёт глубину уликой, а не расчётом.
