...
2. ~~**Судья фазы 2 эпика 02 (`plans/15`)**~~ — ✅ **ЗАКРЫТО 2026-09-05 (сессия 85, автономный цикл).**
   Проход сделан, расписка лежит в `plans/15_DONE` §9, тег поставлен. **Судья ОПРОВЕРГ заявление
   `STATUS.md`** («судья: СВЕРЕНО С ОГОВОРКАМИ» в 00:5x): расписки тогда не существовало, а названное
   время было раньше исполнения последнего шага в 01:1x. Строка исправлена в `STATUS.md`, в шапке
   `plans/13` и в строке ворот фазы 2 — три места, говоривших разное, теперь говорят одно.
   🔴 **Оговорки прохода названы поимённо и не спрятаны:** одиннадцать критериев проверены ПО
   СУЩЕСТВУ, а не в форме, которую назвал план (за три недели код прошёл восемь эпиков, имена блоков
   переехали), а F2-AC10 «ноль записей в GPU за фазу» ретроспективно НЕВЕРИФИЦИРУЕМО — прибора,
   хранящего счёт записей за 15–16 августа, не существует.
   🔴 **Находка прохода, годная для машины:** ярлык `F2-AC` в проекте НЕОДНОЗНАЧЕН — батарея движка
   печатает `F2-AC1…AC6`, принадлежащие ДРУГОМУ плану. Судья, сверяющий по совпадению ярлыка, заверил
   бы чужие критерии и остался бы доволен. Отсюда правило: **сверять по существу, а не по ярлыку.**
3. ~~**Строка фазы 2 в шапке `plans/47`**~~ — ✅ **ЗАКРЫТО 2026-09-05.** Шапка девять дней говорила
   «фаза 2 следующая», хотя фаза закрыта и помечена в тот же вечер. Правка сделана по правилу
   ревизии 68: два независимых свидетеля — тег `DONE` у `plans/49` и коммит `ca007b7`.
