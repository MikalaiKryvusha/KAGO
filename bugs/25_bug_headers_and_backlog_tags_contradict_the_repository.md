...
4. Follow-up (named, not done here): a cheap agreement check — the `/fable-judge` or a lint
   ...
   **Что нашлось на дне:** **12 файлов с тегом `DONE`, чья строка статуса до сих пор говорит
   ...
   База долга — `decisions/backlog-truth-baseline.json`, 25 строк, и она ТОЛЬКО УБЫВАЕТ: новое
   расхождение красное, починенное и оставшееся в базе — тоже красное. Доказано красным на обеих
   сторонах; 11 блоков набора, главный — «строка с ОБОИМИ словами законна» (без него сторож краснел
   бы на каждом живом эпике, и его бы выключили).
