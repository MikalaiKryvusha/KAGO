...
5. ✅ **ДВЕНАДЦАТЬ СТРОК ДОЛГА ПОГАШЕНЫ В ТОТ ЖЕ ДЕНЬ — база 25 → 13.** Это исполнение пункта 1
   ...
   **Шапки НЕ переписаны, а ДОПОЛНЕНЫ.** Новая строка статуса встаёт сверху и называет свидетеля;
   ...
   🟡 **Остались 13, и они ДРУГОГО класса:** файл БЕЗ тега, чья строка статуса говорит «закрыт»
   (`bugs/63`, `65`, `96`, `98`, `99` и восемь планов). Их нельзя закрыть тем же движением: поставить
   тег значит СНЯТЬ предмет с беклога совсем, и на это нужен разбор каждого, а не согласование двух
   строк. Именно об этом писала ревизия сессии 65: «поставить `DONE` по одному лишь противоречию —
   значит заменить одну ложь другой, и вторая будет дороже».
