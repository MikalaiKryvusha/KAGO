...
## 🔟 2026-09-07: ДЕСЯТАЯ ФИКСТУРА ЭТОГО КЛАССА — НАЙДЕНА БАТАРЕЕЙ, ПОЧИНЕНА В ТОТ ЖЕ ЧАС

**И нашлась она ровно потому, что этот тикет починил сохранение улики.** Без него вывод красного
набора умер бы вместе с прогоном, и всё, что осталось бы, — «`vfstep` мигнул».

**Симптом дословно повторил исходный:** батарея — `КРАСНЫЙ vfstep: код выхода 1 · красных строк 2`;
тот же набор в одиночку — **зелёный дважды подряд, код 0**.

**Улика, сохранённая батареей** (`runs/selftest/2026-09-07T13-12-39-834Z/vfstep.log`):

```
ПЛОХО B94-AC1: первый прожиг тройки ничего не архивирует — архивировать нечего
  -> получено ["…\vfstep-port-38832\vmin-collide\archive\pin-2145-555-2026-09-07T12-56-40-777Z.jsonl", true], ждали [null, false]
ПЛОХО B94-AC1: повторный прожиг той же тройки оставляет ДВА файла — получено [1,2], ждали [1,1]
```

**Диагноз читается прямо из улики, без единой догадки:** в песочнице лежит файл архива со штампом
**12:56** — от ПРОШЛОГО прогона. Песочница называлась `vfstep-port-${process.pid}` (имя врало ещё и
«port»): ключ — PID, который Windows ПЕРЕИСПОЛЬЗУЕТ, а батарея запускает полсотни процессов подряд.
Блок «первый прожиг ничего не архивирует» краснел потому, что архивировать БЫЛО что — чужое.

**Почему эта фикстура уцелела при починке 05.09:** тогда перевели на `mkdtempSync` девять фикстур,
найденных грепом по `process.pid`. Эта тоже ключевалась по pid, но её ИМЯ говорило «port», и глаз
прошёл мимо. Урок про имя, а не про греп: **имя, которое врёт о содержимом, прячет предмет от
поиска по смыслу.**

**Починка:** `mkdtempSync(join(tmpdir(), 'vfstep-selftest-'))` — та же форма, что у девяти соседок.
Столкновение теперь невозможно ПО ПОСТРОЕНИЮ (гарантия ОС), а не по удаче номера.

## ✏️ 2026-09-07: Я ЗАПИСАЛ СЮДА «ВОСПРОИЗВЕДЕНИЕ КОНКУРЕНЦИЕЙ» — И ЭТО БЫЛО НЕВЕРНО

Оставляю запись о собственной ошибке, потому что она про метод, а не про опечатку.

**Что я увидел.** Батарея, запущенная одновременно со смоуком двойника, дала `check КРАСНЫЙ(0)` —
набор умер, не напечатав ни строки. Совпадение с формой этого класса было полным, и я записал в
тикет: «класс воспроизведён намеренно, признак — внешняя нагрузка».

**Что оказалось.** Повтор батареи БЕЗ единого соседнего процесса дал **тот же `check КРАСНЫЙ(0)`**.
Настоящая причина — мой собственный блок, написанный ЧУЖИМ ДИАЛЕКТОМ `ok`: седьмые ворота
`npm run check` (`assert-dialect-lint`, `bugs/106`) отказали с кодом 1, и набор `check` в батарее
честно стал красным. Конкуренция была НИ ПРИ ЧЁМ — просто самое заметное, что происходило рядом.

**Урок, оплаченный дважды за день.** Я увидел красное, вспомнил похожий случай и назвал причину, не
проверив её отсутствием. Проверка стоила одного повторного прогона. Ровно то, о чём сам тикет и
писал: «записывать причину на этот класс без улики значило бы закрыть тикет догадкой».

**Причина смерти `dashboard` 04.09 по-прежнему НЕИЗВЕСТНА**, и ничего из сегодняшнего к ней не
добавляется.
