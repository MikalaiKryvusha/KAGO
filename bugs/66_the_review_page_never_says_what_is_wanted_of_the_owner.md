...
**Статус:** 🟠 В РАБОТЕ — найден ВЛАДЕЛЬЦЕМ 2026-08-29; оперплан ниже, написан 2026-08-30.
...
# План починки — оперативный (2026-08-30)
...
- [ ] **1. Вынести формулировку в одну функцию `askFor(doc, askText)`** — она возвращает три части
      постановки. Голос (`review.mjs:1078`) начинает звать её вместо своей строки. **Пара
      УБИРАЕТСЯ, а не заводится** (`PHILOSOPHY.md` → DRY).
- [ ] **2. Постановка на странице документа без вопросов** — блок в шапке, до текста документа:
...
- [ ] **3. Флаг `--ask "…"`** кладёт текст вызывающего; без флага — общая формула из `askFor`.
- [ ] **4. Сторожа в `tools/verify-review-contour.mjs`** — по одному на AC1, AC2, AC3.
      **Адресность мутаций объявляется ДО прогона** (канон судьи): мутант «снять постановку» →
      ...
      красных.
- [ ] **5. Правило повторного подъёма — в `interviews/README.md`** (P66-AC4).
- [ ] **6. Ворота:** `npm run verify:contour` и `npm run selftest:all` зелёные; рендер
