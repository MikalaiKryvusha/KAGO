...
**Status:** 🟡 PARTIAL — **Finding 3 WIRED 2026-09-04 15:2x (session 82, offline, with a plan amendment — see «Fix plan, step 1 — AMENDED» below):** input 2 rides the live sweep on all three riders (burn `--progress-file` outside `args` · probe relay · judge `--arm-m` + file), built from ONE function shared with the twin (`fuse.progressRiderArgs` → `engine.liveFuseRiders` / `twin.riders`); the threshold now follows the band's LOWEST clock (`fuse.PROGRESS_TICK_REF_MHZ`: 2692 → 1040 ms, 900 → 3109 ms, ≥ 2820 → 993 unchanged) because the archive shows the launch tick inverse to the clock and the furnace max was taken at 2820 MHz; `--progress-observe` = the file wired, no trip. Proof: `fuse` 77 → **82**, `stress-tester` 68 → **70**, `vf-step` +3, `engine` 471 → **476**; four mutations (scale · argv spread · set-path forward · probe flag) red on exactly their blocks, restored byte-for-byte; `twin --rehearse-death progress-stall` — three trips, every one with cause `progress-stall`, both hands, band goes on; **the rehearsal itself exits 1 on two checks that predate Ш5 (expect one trip and a stopped band) — `bugs/104`, found by the closing judge; an earlier «7/7» here was my filtered grep hiding a 🔴 line**. **Live witness pending** (gate 6a): before Monday's run, `npm run workloads:build` must re-prove `distinct=1` for a burn launched WITH the flag; the first-run mode (armed vs observe) is the owner's — `interviews/026` Q3. **Finding 2 FIXED 2026-09-04 12:0x (session 81, offline, on the double):** `sweepFrequency` now asks `stopWhen` before EVERY rung (`engine.mjs`, the top of `for (const rung of rungs)`), threaded from `sweepRange` as the same hook; a stop mid-descent closes the frequency `cut-short` on the last PASS (or halts it unwritten when nothing passed) and the band-level hook stops the band before the next frequency — one decision, two ask points. Proof: two blocks in `engine --selftest` (469 → **471**, «прожигов ровно 2» · «cut-short + fuse-rescue»); mutation «снять опрос в цикле ступеней» → `получено 6, ждали 2`, red on exactly its block; the four older fuse blocks (трип · названа своим именем · Ш5 ×2) stay green. **Live witness pending** (gate 6a): the next attended run must show a judge exit followed by ZERO further rungs. **Finding 1 — HALF WIRED 2026-09-05 (session 85, offline, on the double): the SEQLOCK half is built, the fencing half is not.** Hand 2 now validates across its OWN critical section exactly as `researches/31` §2.2 prescribes: it reads the SWEEP journal's intent counter before and after `zeroCurve`, and a counter that moved invalidates the read — **discarded, never interpreted**, including a read-back that came back clean (a clean read taken before somebody else's write is a statement about a card that no longer exists). One retry (named as my choice, with the reason: the racing writer is a single rung's write, already finished by the time it is detected; unbounded retry is forbidden — the hand talks to a dying driver). A second race in the same rescue reports `ok: false` whose detail begins **«ГОНКА С ПОЛОСОЙ, а не C3»** — so a third post-mortem cannot walk the same false trail. No source wired (`--sweep-journal` absent) ⇒ the guard is SILENT and behaviour is byte-identical to before: «not looked at» ≠ «looked and clean» (R4b). Plumbed end-to-end — `engine.mjs` → judge CLI `--sweep-journal` → `makeStockHand` extraArgs → hand CLI — on BOTH paths from one place (`journal.path`), so the twin proves the wiring even though it cannot reproduce the race (its hand holds its own card instance). Proof: `fuse` battery 5 new blocks (counter distinguishes the sweep's `state:'intent'` from the judge's `phase:'intent'` · race-once retries clean · race-twice named a race · **a real C3 with a still counter stays C3** · no source ⇒ never a race), and **five mutations, each reddening its own** (drop the retry · accept a clean read in a race · rename the race back to C3 · always claim a race · count the judge's intents too). **The fencing half of the fix plan (step 3: the sweep must not write while a trip is in flight, checked immediately before the atom's write) is NOT built** — and `bugs/108` shows why it needs the owner first: the band's read of «is the fuse on post?» is a 250 ms poll of a 60 ms state, which is a fork about the fuse itself (`interviews/022` Q3 = A). **Live witness pending** — the race itself has never been reproduced anywhere; the double cannot, and Monday's run is the first chance · **Filed:** 2026-09-04 12:0x (session 81) · **Found:** by reading three journals of the full pass 04.09 10:50 — `runs/sweep/journal.jsonl`, `runs/death-watch/2026-09-04T07-50-16-246Z-fuse.jsonl`, the Windows `System` log — against `runs/sweep-session80-polnyj-prohod.log` · **Severity: HIGH** — two of the three findings are engine defects on the live path; the third is a guard proved on the double and never mounted on the rocket (gate 6a).

## 🔓 РАЗБЛОКИРОВАНО 2026-09-07: ВТОРАЯ ПОЛОВИНА НАХОДКИ 1 БОЛЬШЕ НЕ УПИРАЕТСЯ В ВЛАДЕЛЬЦА

Строка статуса выше говорит: ограждающая половина (шаг 3 плана — полоса не смеет писать, пока трип в
полёте) не построена, потому что «`bugs/108` показывает, почему тут нужен сначала владелец».

**Оба условия сняты в один день, и второе важнее первого:**

1. **Владелец ответил** (`interviews/027` Q1 = D, 2026-09-07): череда спасений без продвижения полосы
   кончается тремя попытками, паузой, ещё тремя и закрытием прогона. Исполнено; `bugs/108` ЗАКРЫТ.
2. 🔴 **И САМА АРИФМЕТИКА БЛОКИРОВКИ ИЗМЕНИЛАСЬ.** Довод звучал так: «полоса читает „защита на
   посту?" опросом раз в 250 мс, а состояние живёт 60 мс из 3080 — 2 % времени». Числитель и
   знаменатель были взяты ИЗ ШТОРМА в 94 срабатывания. Шторм оказался ОДНИМ отказом, размноженным
   эхом собственного спасения (`bugs/111`), и после починки его нет: то же прогон даёт **одно**
   срабатывание. Между спасениями состояние «на посту» теперь живёт не 60 мс из 3080, а практически
   всё время прогона.

**Что это значит для шага 3.** Гонка, ради которой строилась бы ограда, стала на порядки реже — но
НЕ исчезла: сейлок руки 2 её по-прежнему обнаруживает, и живьём эта пара не встречалась ни разу.
Значит шаг 3 из «заблокирован владельцем» переходит в «обычная работа с обычным приоритетом», и
браться за него надо на свежую голову, а не перед живым прогоном владельца: это правка сейлока
аварийной защиты, тот же класс осторожности, что `plans/89` Ш1.
