...
**Статус:** ✅ **ЗАКРЫТ 2026-09-07 — ВСЕ ЧЕТЫРЕ ПУНКТА ПЛАНА ПОЧИНКИ ИСПОЛНЕНЫ, И У КЛАССА ТЕПЕРЬ
ЕСТЬ СТОРОЖ.** Тикет прожил семнадцать дней, и всё это время его собственный план лежал списком.

| пункт плана | улика |
|---|---|
| **1.** привести шапки в согласие | **13 файлов** приведены 07.09 (`bugs/14`, `16`, `20`, `28`, `29`, `31`, `40`, `48`, `50`, `68`, `74`, `100`, `plans/17`, `18`, `70`, `85`), каждому найден СВИДЕТЕЛЬ-КОММИТ; шапки не переписаны, а ДОПОЛНЕНЫ — прежние строки сохранены дословно |
| **2.** обновить `STATUS.md` → «Открытые баги» | 07.09: убраны четыре закрытых тикета, которых список держал; сверху встала **машинная сводка настоящего** по состояниям (34 открытых, из них 13 ждут живого свидетеля) |
| **3.** шапка карты архитектуры | закрыт раньше: `PROJECT_ARCHITECTURE_INTERNAL_MAP.md:42` — «Updated: 2026-08-22», оговорка «оркестратор спроектирован, но не написан» снята с явной пометкой об этом |
| **4.** дешёвая проверка согласия | **`tools/backlog-truth-lint.mjs`, восьмые ворота `npm run check`, набор `backlogtruth` (18 блоков) в батарее.** База долга **25 → 0**: расхождений в дереве НЕ ОСТАЛОСЬ, и любое новое краснеет немедленно |

🎯 **Главное — не число, а то, что класс перестал зависеть от внимания агента.** Он возвращался
трижды и каждый раз находился ГЛАЗАМИ (ревизии сессий 65 и 68, и 07.09 — четыре закрытых тикета в
списке открытых). Теперь его ловит машина, и правило «две правды не разойдутся молча» перестало быть
прозой. Разбор трёх заострений словаря сторожа — в пунктах 5–6 ниже; каждое вскрывало правду,
которую прежняя редакция маскировала.

> **Прежний статус (устарел):** 🔴 OPEN — found by the 2026-08-21 audit (`ideas/08`)
...
4. Follow-up (named, not done here): a cheap agreement check — the `/fable-judge` or a lint
   ...
   ✅ **ИСПОЛНЕНО 2026-09-07 (сессия 86, дневной цикл) — `tools/backlog-truth-lint.mjs`, ВОСЬМЫЕ
   ВОРОТА `npm run check`.** Пункт стоял ненаписанным с 21 августа, и за это время класс возвращался
   ТРИЖДЫ, каждый раз находясь ГЛАЗАМИ: ревизия сессии 65, ревизия сессии 68 и 07.09 — четыре
   закрытых тикета в списке «Открытые баги» `STATUS.md`, замеченные при сверке имён файлов.

   **Что сторож делает и, главное, чего НЕ делает.** Он не решает, закрыт предмет: урок сессии 65
   («маркер ✅ внутри документа чаще относится к ОДНОМУ ШАГУ, а не к предмету») здесь исполнен
   буквально. Он сверяет РОВНО ДВА сигнала, по которым считается беклог, — тег `_DONE_` в имени
   файла и СТРОКУ СТАТУСА документа. Разошлись — находка; кто прав, решает разбор.

   🔴 **И первая же его редакция была неверной, что записано замером, а не спрятано:** чтение всей
   шапки дало **35** «нарушений», из которых почти все ложные — в шапке эпика ✅ стоит у закрытых
   ФАЗ, пока сам эпик открыт. Сузив до строки `**Статус:**` и сделав словари ОБОЮДНЫМИ (строка,
   несущая и «закрыты фазы», и «открыта следующая», законна), получили **25** настоящих расхождений.

   **Что нашлось на дне:** **12 файлов с тегом `DONE`, чья строка статуса до сих пор говорит
   «ОТКРЫТ»** (`bugs/14`, `20`, `28`, `29`, `31`, `40`, `48`, `50`, `68`, `74`, `100`,
   `plans/70_DONE`), и 13 обратных. Это ровно исходная таблица этого тикета от 21.08 — часть строк
   так и не переписали. **Шапки НЕ переписаны скопом намеренно:** тег не всегда прав, и правило
   сессии 68 требует ВТОРОГО свидетеля на каждый.

   База долга — `decisions/backlog-truth-baseline.json`, и она ТОЛЬКО УБЫВАЕТ: новое расхождение
   красное, починенное и оставшееся в базе — тоже красное. Доказано красным на обеих сторонах;
   11 блоков набора, главный — «строка с ОБОИМИ словами законна» (без него сторож краснел бы на
   каждом живом эпике, и его бы выключили).

5. ✅ **ДВЕНАДЦАТЬ СТРОК ДОЛГА ПОГАШЕНЫ В ТОТ ЖЕ ДЕНЬ — база 25 → 13.** Это исполнение пункта 1
   плана починки («update the header»), которое семнадцать дней стояло списком без исполнения.

   **Каждой строке найден СВИДЕТЕЛЬ-КОММИТ, а не мой домысел** (правило сессии 68 — второй
   независимый свидетель):

   | файл | свидетель |
   |---|---|
   | `bugs/14`, `bugs/20`, `bugs/31` | `f29b1f3` «chore(ревизия беклога): bugs/14, 20, 31 закрыты DONE — их починки давно в коде» |
   | `bugs/28`, `bugs/48`, `bugs/50` | `2818e8b` «chore(ревизия беклога, вторая тройка): bugs/28, 48, 50 закрыты DONE» |
   | `bugs/29` | `6525ab9` «chore(ревизия беклога): bugs/29 закрыт DONE» |
   | `bugs/40` | `3405921` «chore(ревизия беклога): bugs/40 закрыт DONE — находка закрыта интервью» |
   | `bugs/68` | `6b09d4e` «test(bugs/68): долг по земле затравки закрыт — блок краснеет на мутации» |
   | `bugs/74` | `2acffc6` «fix(bugs/74): область сторожа вопросов сужена по тегу DONE» |
   | `bugs/100` | `8216b05` «fix(окно наблюдения, bugs/100 DONE)» |
   | `plans/70` | `cdfdc95` + шапка `plans/71_DONE` («Предыдущая фаза: `plans/70` — ✅ закрыта») — **два** свидетеля, и второй это строка РОДИТЕЛЯ |

   **Шапки НЕ переписаны, а ДОПОЛНЕНЫ.** Новая строка статуса встаёт сверху и называет свидетеля;
   прежняя сохранена под ней дословно с пометкой «устарела». Оригинал не переписывают под сегодняшнее
   знание — то же правило, по которому в этом тикете сохранены все прежние редакции.

6. 🔬 **ЕЩЁ ДВА ЗАОСТРЕНИЯ ПРИБОРА В ТОТ ЖЕ ДЕНЬ — И КАЖДОЕ ВСКРЫЛО НОВУЮ ПРАВДУ. База 13 → 1.**

   **Заострение второе — СЛОВАРЬ СОСТОЯНИЙ.** Разбирая оставшиеся 13 поимённо, увидел, что пять из
   них ложные: `bugs/96` («🟡 ЧАСТИЧНО ПОЧИНЕН»), `bugs/98` («ПОЧИНЕН, ОСТАТОК НАЗВАН»), `bugs/99`
   («🟡 … но»), `bugs/63` и `bugs/65` («🔧 МЕХАНИКА ПОЧИНЕНА»). Слово «починен» есть, а предмет
   открыт — и правильно. **У проекта ЧЕТЫРЕ состояния, а не два:** 🔴 открыт · 🟠 в работе ·
   🟡 частично · 🔧 починено, ждёт живого свидетеля · ✅ закрыт.
   ⚠️ **Цена первой редакции была бы высокой:** следуя ей, я пометил бы `DONE` пять ОТКРЫТЫХ
   тикетов — снял бы с беклога живую работу. И это же заострение ВСКРЫЛО находку, которую грубый
   словарь маскировал: `bugs/16_DONE` со строкой «🟡 partially fixed» (свидетель `109a9b8`).

   **Заострение третье — РЕГИСТР.** Вторая редакция искала `OPEN` заглавными и не видела `🔲 open`
   строчными; 🟢 (состояние ЖИВОГО предмета — `plans/10`, `26`, `47`, `58`) не значилось нигде.
   Регистронезависимый словарь плюс 🟢 нашли ещё **три** файла, три недели вравших о себе:
   `plans/17_DONE`, `plans/18_DONE`, `plans/85_DONE` (свидетели `bc1bf5f`, `a3d8aab`).

   **ОСТАЛСЯ ОДИН — `plans/54`** (развёртывание команды): «операция 1 исполнена · операция 2 —
   состав одобрен». Это НЕ расхождение двух сигналов, а непринятое решение — закончен план или нет.
   Согласованием строк такое не чинится; лежит в долге честно.

   🎯 **Закономерность дороже числа: КАЖДОЕ заострение вскрывало правду, которую прежняя редакция
   МАСКИРОВАЛА.** 35 ложных → 25 настоящих → 13 → 7 (+1 находка) → 4 (+3 находки) → 1. Сторож, чей
   словарь беднее предметной области, не просто шумит — он МОЛЧИТ там, где должен кричать.
   18 блоков набора; регресс словаря к любой прежней редакции красит свои.
...
1. ~~**Приёмка эпика 03 (`plans/16`)**~~ — ✅ **ЗАКРЫТО 2026-09-07 (сессия 86, дневной цикл).**
   Проход по десяти критериям сделан, каждый вердикт снят ПРОГОНОМ, все десять записаны в сам эпик
   (`plans/16` → «Вердикты приёмки, снятые 2026-09-07»). **Итог: ДЕВЯТЬ взяты, один (AC2) назван с разбором, почему не берётся существующими картами.**
   - ✅ AC1 (веток «мы на моке» ноль) · AC4 (65 утверждений ловушек, и неправильный движок краснеет)
     · AC5 (живая колонка 0 из 5, «не прогонялась») · AC6 (настоящая смерть процесса) · AC7 (сдвиги
     0/0) · AC8 (строку границы печатает каждый отчёт) · AC9 (другая геометрия тем же кодом) ·
     AC10 (замер против оценки, расхождение +11 % названо).
   - 🟡 **AC2 — единственное, что держит эпик открытым, и он ОКАЗАЛСЯ НЕ МЕЛКИМ.** Взялся за него в
     тот же день: заведена девятая ловушка `T9_edge_within_one_session` (край 1022,7 мВ при стоке
     1045, в пределах одной сессии спуска), и прогон по ней вскрыл причину, которой никто не знал —
     **спуск БЕЗ ЗАКРЕПЛЕНИЯ частоты края не встречает вообще:** карта выдаёт всё более низкую
     частоту вместе с напряжением и проходит до самого дна сетки (1020 мВ → 2835 МГц, 810 → 2062,
     450 → 180, строка закрылась `lever-limited`). Это свойство стенда, а не якоря ловушки: ни одна
     карта с любым краем не даст AC2 без замка. Ряд объявлен ЖДУЩИМ с причиной, снятой ЗАМЕРОМ
     (`openPhase` у ловушки + `pending` в наборе). Осталось — прогон с закреплением (`pinMhz`).
   - ✅ **AC3 ЗАКРЫТ В ТОТ ЖЕ ДЕНЬ:** заведена `provenanceLine()` — «ЕДИНИЦА ВОСПРОИЗВЕДЕНИЯ: зерно ·
     коммит · профиль карты», три блока в наборе `vgpu` (122 → 125), главный из них — «отпечаток
     профиля РАЗЛИЧАЕТ карты».

   🔴 **Эпик остаётся ОТКРЫТЫМ, и это результат приёмки, а не её провал.** Ровно то, против чего
   заведён этот тикет: тег `DONE` по «выглядит сделанным» — и есть его предмет. Два пробела названы
   поимённо вместе с тем, что надо сделать; оба мелкие.
