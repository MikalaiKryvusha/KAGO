...
| `npm run curve -- --selftest` | 44 blocks, no GPU. Mutation-proved with **13 mutations**, each reddening its own named block — including the historically WRONG version of the R13 ceiling (judging the whole curve's top instead of what we raised), which would refuse a document of all zeroes. |
