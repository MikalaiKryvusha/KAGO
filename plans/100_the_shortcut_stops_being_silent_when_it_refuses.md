...
**И вторая дверь того же класса — вход в систему (улика 2026-09-25, сессия 102).** После смены драйвера
610.88 → 616.92 (не позже 18.09 23:01) восстановление при входе отвергало `Optimised` по штампу (R6) при
КАЖДОМ входе — `runs/shell/boot-apply.jsonl`: `degraded-to-factory` 18.09 и 19.09, — и карта **неделю**
стояла на заводских молча; нашлось случайно, при чтении карты по другому поводу. Ш1 этого плана
(`consent`: «восстановление при входе») закрывает и эту дверь, если восстановление зовёт тот же применитель.
Вопрос владельцу о режиме до вечера проверки — `interviews/interview_031`.
