...
## 5а. ⚖️ РАСПИСКА СУДЬИ (`/fable-judge`, 2026-08-31 00:2x)

**Вердикт: ✅ СВЕРЕНО С ОГОВОРКАМИ.**

| заявление | что судья НАБЛЮДАЛ сам |
|---|---|
| строка жизни переживает смерть без штатного закрытия | ✅ перепрогон набора: `C1 … строк 3, возраст последней 454 мс` |
| семь случаев исполнены | ✅ 7 из 7 несут статус с уликой |
| контроли отработали | ✅ 6 из 6 (K5 и **K6 заведены по ходу**, см. ниже) |
| батарея зелёная | ✅ 42 набора, красных 0, 2072 блока |
| записей в GPU ноль | ✅ `watchdog --status`: не взведён |
| C6 без накопления | ✅ **300 строк за 300 с, дрейф 2,32 мс** |

**Охоты на подлоги — три чистых, одна дала находку:**

- **Пульс по таймеру** — чисто: три строки `heartbeat.log` за ночь, каждая соответствует
  закоммиченной работе и называет конкретные артефакты.
- **Бродячие процессы** — чисто: 0 живых судей и проб после прогонов (один был найден и убит —
  он остался от СЛОМАННОГО замера C3).
- **Маркер обновления контекста** — чисто: 37 минут, час не истёк.
- 🔴 **ОСЛАБЛЕНИЕ ПРОВЕРКИ — НАЙДЕНО.** Требование `R2` в тест-документе осталось в старой
  редакции («непустых полей из шести — 6 из 6»), тогда как код и блок C2 приняли уточнённое
  чтение (у непроведённого канала законен `null`). То есть документ говорил одно, а проверка
  делала другое, и расхождение жило только в комментарии к коду. **Исправлено судьёй:** `R2`
  переписан в EARS-форме с явной веткой `IF канал не проведён, THEN null, а не ноль`.

**Судья завёл ШЕСТОЙ контроль, которого не было в плане.** `C7` был зелёным и ДО правки, и после —
то есть по воротам 5 канона он не доказывал ничего. Мутация **K6** (сбрасывать кольцо каждый такт)
роняет ровно его: **56/1**. Теперь C7 — детектор, а не украшение.

**Оговорки, вошедшие в вердикт (обе объявлены самим исполнителем ДО суда, не найдены судьёй):**
`fsync` не доказан стендом (мутация K3 зелёная) · класс «машина замёрзла» не доказан, доказано
свойство «улика переживает отсутствие штатного закрытия».
...
### 🔴 И ВТОРОЕ: план предписывал поставить кольцу `every-second` — я ОТКАЗАЛСЯ, и вот почему

Шаг плана гласил: *«После починки: снять `@forensic fuse-ring` из долга, поставить
`DURABLE-AT: every-second`»*. **Не исполнено намеренно.** `bugs/78` закрыт не тем, что кольцо стало
секундным — оно им не стало и не должно: лечением был ВТОРОЙ СЛОЙ рядом. Кольцо осталось
best-effort дампом высокого разрешения, и `trip-only` — правда о нём. Поставить `every-second` ради
зелёного линтера значило бы соврать в маркере, то есть сделать ровно то, против чего механизм и
заведён. Долг остаётся и меняет смысл: не «незакрытый дефект», а «объявленное свойство, покрытое
соседом».

**Отсюда вопрос к самому правилу R4, и он крупнее обоих предыдущих.** Правило спрашивает «когда
улика становится долговечной» и сверяет ответ со списком запрещённых слов. Спрашивать оно должно
**«становится ли улика долговечной в момент ТОГО СОБЫТИЯ, КОТОРОЕ ОНА ОБЪЯСНЯЕТ»**: кольцо
объясняет трип и ложится на трипе — для своего события оно долговечно; оно же, объявленное
объяснением смерти машины, было бы негодным. То есть правильная проверка связывает `DURABLE-AT` с
`EXPLAINS`, а не со словарём.

**Изобретать это правило в ночь закрытия фазы я не стал.** Семантическое правило, придуманное под
давлением срока, — это в точности способ, которым родились дефекты 30 августа. Записано как
кандидат; решать с холодной головой, а границу — спросить у владельца (М4).
