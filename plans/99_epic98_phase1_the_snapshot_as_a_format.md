...
> **Создан:** 2026-09-09 (сессия 97) · **Родитель:** `plans/98` эпик 98, фаза 1 · **Статус:** 🟡
> **Ш1–Ш4 исполнены 2026-09-13 (сессия 98), офлайн, записей в карту ноль** — мутации краснят свои
> блоки; P99-AC1…AC4 взяты блоками, P99-AC2 дополнительно на НАСТОЯЩИХ данных. **Открыты Ш5–Ш6 — только
> при владельце у машины** (§8) · **Исходящее:** —
...
*Добавлено при исполнении 2026-09-13 (сессия 98):*

4. **`curveSnapshot` — ОБЯЗАТЕЛЬНЫЙ ключ, как остальные пять.** Развилка записана строкой `FORK:` у
   `SETTING_KEYS` в `profile-store.mjs`: необязательный ключ завёл бы второе соглашение внутри одного
   формата. Цена обязательного — миграция, и она проведена по описи ПОТРЕБИТЕЛЕЙ, а не файлов
   (EXP-0091): 8 файлов профилей и 8 мест, где профиль собирается в коде.
5. **Вместо `sourceCommit` — `source.sha256` байтов источника.** Идентификатор коммита назвал бы байты,
   которых могло не быть в снимке: развёртка пишет документ и после последнего коммита. Хеш байтов
   называет ровно замороженное; независимо сверен `Get-FileHash`.
6. **Имя файла выводится из момента снятия, отдельного поля `id` нет.** Пара «имя ↔ поле»
   предотвращена, а не сторожится: валидатор сверяет имя файла с `takenAt`.
7. **Неизменяемость — жёсткой ссылкой, а не проверкой существования.** `renameSync` на Windows
   замещает цель, и «отказ, если файл есть» было бы гонкой. Мутация С1 (замена на `renameSync`)
   краснит блок P99-AC1.
8. **Тело снимка судит `validateCurveDoc` целиком** — правил строк у снимка своих нет. Снимок
   документа, не проходящего собственный валидатор, не пишется и не загружается.
9. **Счётчик приёмки научен снимку в этой же фазе, хотя режимы на снимок ещё не переведены.** Иначе
   Ш5 молча снял бы строку «отгружённые режимы опираются на помеченную кривую» при неизменной опоре
   (R4b). Неподанный снимок строка называет «НЕ ПРОЧИТАН».
10. **Ветки ссылки и снимка в `effectiveCurveSetting` сведены в ОДИН путь**, различается только
    источник строк; тексты отказов ссылки сохранены дословно. Первая редакция этой сессии завела
    отдельного помощника для снимка — это была бы вторая копия арифметики, и она исправлена до прогона.

## 8. Ход исполнения — 2026-09-13, сессия 98

| шаг | состояние | улика |
|---|---|---|
| Ш1 формат | ✅ | `curve-store.mjs` §3c: `buildSnapshot` · `validateSnapshot` · `saveSnapshot` · `takeSnapshot` · `loadSnapshot`. `curve --selftest` 132 → **155** (20 блоков снимка + 3 счётчика). Мутации С1–С6 — каждая краснит ровно названные блоки |
| Ш2 поле в профиле | ✅ | `profile-store.mjs`: ключ, взаимоисключаемость, потолок при снимке, отображение в списке. `profiles --selftest` 53 → **64**. Мутации П1–П4 краснят свои; у П1 сверх названных покраснел блок «stock-default со снимком» — число настроек в его отказе выводится из того же списка ключей (неполнота перечня, не дефект кода). `npm run profiles` против живой карты: 8 из 8 приняты |
| Ш3 применитель | ✅ | `profile-manager.mjs`: один путь для ссылки и снимка, граница песочницы, поле отказа по источнику. `profile --selftest` 83 → **88**. Мутации Р1–Р5 краснят ровно названные блоки. **P99-AC2 на настоящих данных:** одно чтение таблицы и сдвигов карты, настоящая опора → вектор через `curveRef` и через снимок `2026-09-13T20-25-42` совпали побайтно (69 ненулевых сдвигов, намерение 3090 МГц, монотонен, опора — артефакт) |
| Ш4 команда снятия | ✅ | `npm run curve -- --snapshot` снял `curves/battle/2026-09-13T20-25-42.json`: краёв 5 из 389, не тронуто 295, sha256 источника `ba2536…e0f1` совпал с `Get-FileHash`. Первая строка вывода — «СНЯТО БЕЗ ВОРОТ ПЕРЕДАЧИ» |
| Ш5 перевод трёх профилей | 🔲 **при владельце** | условие «векторы совпали побайтно ДО перевода» уже выполнено на снимке выше; перевод — три строки JSON (`curveRef: null`, `curveSnapshot: "2026-09-13T20-25-42"`); откат — обратные три строки |
| Ш6 живой свидетель | 🔲 **при владельце** | клик по ярлыку `⚖️ Optimised` → задача возвращает 0 → дописать строку в рабочий документ → применить ещё раз → вектор тот же |

**Чего фаза пока НЕ доказала** — названо, чтобы не читалось как «готово»: ни один профиль ещё не
стоит на снимке, поэтому путь ярлыка владельца со снимком не пройден ни разу (ворота 6a).
