# Эпик 59 — виртуальная карта как РАВНОПРАВНЫЙ СТЕНД: движку всё равно, кого тюнить
...
> **Статус:** 🟡 РАСПЛАНИРОВАН — фаза 1 открыта (`plans/60`)
...
|---|---|---|
| **1 — инвентарь швов** (`plans/60`) | Точная опись КАЖДОГО касания железа и боевого состояния на пути `--sweep` (cmdSweep · vf-step · stress-tester · всадники · сэмплер): файл:строка · что зовёт · чем заменяется · риск. Канон: нет строки инвентаря — нет кода | опись полна (grep-доказательство: вызовы вне описи = 0); каждая строка несёт способ виртуализации; судья по описи |
| **2 — порт устройства S9** | Пять прямых `import('./nvapi.mjs')` в vf-step становятся ОДНИМ инъекционным портом; живой адаптер = сегодняшние вызовы (поведение бит-в-бит, батарея это держит), виртуальный = applier vgpu | батарея 0 красных; мутация «порт подменён мягким двойником» краснеет (I2); путь записи vf-step ВПЕРВЫЕ прогнан офлайн через инъекцию |
