...
- [ ] 1. **Extract candidates:** walk `GOAL.md` (+ `MASTER_PLAN.md` mode table, `STATUS.md`
      ...
      **Осталось в шаге 1:** участки :870–1100 и :1160–1360, :1505–1639 глазами (двойник —
      частично известен по сессии 59); сверка с таблицей режимов `MASTER_PLAN.md` и фактами
      STATUS 15–39 (что ОПРЕДЕЛЯЕТ, а не измеряет); пометить отменённые слои (пример уже
      найден: «запас = отказ + 10 мВ» :394 ОТМЕНЁН переанкеровкой :399).
