...
🔴 ~~**НАЗВАННЫЙ ДОЛГ, ЕДИНСТВЕННЫЙ:** движок пока НЕ передаёт `watchOnPost` в `closePoint`~~ —
✅ **ПОГАШЕН 2026-09-05 (сессия 85), `plans/88` Ш5.** Проводка ждала полуоткрытого окна, потому что
раньше нечего было передавать: состояние «сторож на посту» родилось вместе с ним (Ш3 той же сессии).

**Как погашен.** Предикат вынесен наружу отдельной функцией `watchOnPostNow({ armed, judgeExitCode })`
— «защита взведена И судья жив», обе половины обязательны: прогон наблюдения охраняемым не считается,
вышедший судья (любой код) означает прожиг без защиты. **Величина МОНОТОННА** — обе половины умеют
падать из истины в ложь и никогда обратно, — поэтому одно чтение в момент закрытия частоты судит
весь интервал, за который её жгли, и вход не подвержен гонке опроса из `bugs/108`.

Вход проведён В ОБА пути закрытия: спуск (`closePoint`) и **урожай** (`writeHarvestRows`). Провести
только в спуск значило бы провести метку наполовину — строка урожая это тоже выдержанный прожиг.

**Доказано сквозным прогоном на двойнике, обе стороны:**

| прогон | что стало со строками 2842 и 2835 МГц |
|---|---|
| `twin --smoke` (защита НЕ взведена) | обе получили **`origin:unwatched`** |
| `twin --smoke --armed` на том же документе | обе метку **потеряли**, помеченных строк 0 |

Плюс пять блоков (`engine` 479 → **484**) и четыре мутации, каждая красит свою; одна была ЗЕЛЁНОЙ и
её находка — предикат жил инлайн-замыканием в `mainSweep`, куда батарея не достаёт ([[EXP-0240]]).
Метрика владельца не сдвинулась: **краёв 4/389, под меткой 20 строк** — живая карта не трогалась.

**Статус:** ✅ **ИСПОЛНЕН ЦЕЛИКОМ.** Разметка — 2026-09-04 20:2x; единственный долг проводки погашен
2026-09-05 (`plans/88` Ш5), и теперь метка СНИМАЕТСЯ САМА перепрожигом под живой взведённой защитой —
ровно то, что требовал P87-AC7. Карта не тронута, записей в GPU ноль. Перепрожиг — при владельце у
машины, с 08.09.
