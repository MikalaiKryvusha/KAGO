...
> и их).
> **Статус:** 🟠 В РАБОТЕ — фаза 1 (`plans/68`).
...
|---|---|---|
| **1 — модель нагрузки** (`plans/68`) | Прожиг двойника перестаёт быть носителем-пустышкой: ватты по форме (сетка `furnace.cu`), каденция запусков по форме и рабочей точке, SDC через модель края, контрольные суммы по штампу, ватты кормят телеметрию (нагрев от нагрузки) | реестр паритета: строка «нагрузка» ✅ · батарея 0 красных · обычная карта байт-в-байт · репетиции эпиков 51/59 зелёные без правок |
| **2 — физика «всегда-вкл» + параметрический пакет** | Механизмы, которые живая карта делает ВСЕГДА, а двойник нёс только ловушками, становятся свойствами ФАЙЛА карты: дрейф таблицы (−1,7 МГц/°C у образца), пол карты (`origin:overshot`), перелёт потолка, связь temp↔край (у образца слабая — 1,85 мВ/°C); телеметрия параметризуется пакетом (P₀, k, предел, вентилятор, τ) | обычная rtx5070ti-карта воспроизводится байт-в-байт СВОИМ файлом · каждое новое свойство имеет блок + мутацию · ловушки T1–T8 не сдвинулись |
| **3 — генератор** | `card-generator.mjs`: семя + амплитуда хаоса (+ архетип-корнер) → файл `virtual-gpu_<семя>.json`; двухслойная выборка (архетипы = глобальный разброс, MC = локальный); карточка происхождения на каждый параметр (наш замер · публичный порядок · назначено); счётчик перевыборок | E67-AC2 целиком · сводка распределений по 100 картам машинная |
| **4 — полигон** | Пакетный прогон: N карт × полный цикл (развёртка → урожай → отчёт) под сторожами инвариантов; карта покрытия по осям; интегрированное сжатие ломающей карты (бисекция амплитуды + обнуление осей по одной, класс отказа сохраняется); минимальная карта → ловушка | E67-AC3 · E67-AC4 · время полигона названо числом |
| **5 — паритет-2 и приёмка** | Реестр паритета переиздаётся (что двойник и полигон ДОКАЗЫВАЮТ · чего НЕ доказывают); строка «вымысел²» во всех выходах; полигон-отчёт владельцу | E67-AC6 · E67-AC7 · «Решения без владельца» заполнены |
