...
- [x] **Ш1. Сопоставление криков драйвера с записями** (AC1) — **ИСПОЛНЕН 2026-09-08 14:1x.**

      | наблюдение | число |
      |---|---|
      | наша запись `3037 МГц ← 900 мВ` (глубина −250 мВ от стока) | 13:00:23 |
      | первый крик `nvlddmkm` | 13:00:25.716 — **через 2,72 с** |
      | период криков | **2296,9 мс, разброс 12 мс за 85 секунд** |
      | наших записей в карту ЗА ОКНО КРИКОВ | **0** |
      | длительность | 85,3 с (39 событий 153 + одно 14) |
      | синий экран | 13:02:23 — через 32 с после последнего крика |

      **ВЕРДИКТ Ш1: версия «наша запись уронила драйвер» НЕ ПОДТВЕРЖДАЕТСЯ для этой смерти.** Крики
      не привязаны к записям — запись была ОДНА, дальше драйвер повторял сам ровным периодом, то
      есть это его собственный цикл восстановления. Спуск на 900 мВ был по-настоящему глубоким, и
      карта его по-настоящему не вынесла. **Стена 3037 подлинная.**

      🔴 **И ГЛАВНАЯ НАХОДКА ШАГА, КОТОРАЯ ПРИНАДЛЕЖИТ УЖЕ ФАЗЕ 6в:** драйвер дал **85 секунд
      предупреждения** перед синим экраном. Это на два порядка больше форы, чем 1,2 с из телеметрии
      мощности (`researches/35` §1.1). Канал драйвера числился в эпике «вторым кандидатом, слабее»
      (`researches/15`) — этот замер поднимает его в первые. Оговорка честная: утром 08.09 драйвер
      НЕ кричал вовсе, значит сигнал специфичен, но не чувствителен, и голосовать в одиночку не может.

- [x] **Ш2. Таблица глубин стен** (AC2) — **ИСПОЛНЕН 2026-09-08 14:0x.** Стен 20. Критерий агента
      «≤ 2 ступеней сетки от стока» дал **ноль** — то есть был бесполезен и заменён проверкой
      ФИЗИКОЙ: стена не может стоять выше напряжения, на котором прошла БОЛЕЕ ВЫСОКАЯ частота.
      **7 стен из 20 противоречат физике**, худшая на 200 мВ (3052 @ 1135 против 3067 @ 935).
      Сторож `tools/hang-floor-physics-lint.mjs` в воротах, семь заморожены в базу долга.
