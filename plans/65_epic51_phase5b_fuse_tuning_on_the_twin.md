...
- **P65-AC1 — пол шума двойника ПЕРЕМЕРЕН, не унаследован.** `fuse --loaded-floor` в форме сна
  двойника, ≥ 90 с, под носителем прожига. Шкала — миллисекунды · прибор — `fuse --loaded-floor` ·
  цель — медиана и **max названы своим числом**. Причина: работаем на виртуальной машине, а живой
  пол (9,57 мс) снят на железе; наследовать числа между формами запрещено оплаченным уроком
  EXP-0165.
...
- [ ] **1. Пол шума двойника (AC1).** `fuse --loaded-floor --seconds 90` под носителем прожига
      двойника; медиана и max записать в §Итог. *Якорь родителя: «N и M выводятся ИЗ ЗАМЕРА».*
...
|---|---|---|
| пол шума двойника | `node automation-engine/lib/fuse.mjs --loaded-floor --seconds 90` | вывод + число в §Итог |
...
1. **Двойник — не кремний, и работаем на виртуальной машине: её пол шума может оказаться ВЫШЕ
   живого.** Тогда абсолютные N кривой на живую карту не переносятся. Реакция: план и заявляет
   ФОРМУ (где начинаются ложные, где теряется спасение), а точку выбирает живой пол; это пишется в
   §Итог явно, а не подразумевается. Оплаченный урок ровно про это — EXP-0165.
