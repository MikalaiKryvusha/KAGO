...
- [x] **1. Фон репетиции (AC1) — ИСПОЛНЕН 2026-08-29 10:4x, и по дороге найдено НЕ ТО, что искали.**
      Здоровый прогон `--smoke --armed` при N=60 прошёл без единого трипа, I1 цел — но **кольцо
      судьи сбрасывается на диск ТОЛЬКО при срабатывании**, поэтому у здорового прогона измерения
      фона нет вовсе. Фон снят вторым свидетелем — `fuse --jitter-floor --seconds 60`, тем же
      `--beat-sender`, что едет в прогоне: **медиана 2,02 · p99 4,02 · max 9,73 мс, доставка
      99,36 %.** ⚠️ **И это переворачивает риск 1:** живой пол под горном был **9,57 мс** — числа
      практически совпали, значит зазор ударов определяет ПЛАНИРОВЩИК, а не нагрузка на карту, и
      сторона ложных срабатываний переносима лучше, чем план предполагал. Граница ложных на
      двойнике доснимается сеткой (сценарий «здоровый»). *Якорь родителя: «N и M выводятся ИЗ
      ЗАМЕРА (запас от max шума), не из головы».*
- [x] **2. Шов: N наружу у репетиции.** Сегодня `makeTwinAssembly` прошивает `DERIVED_ARM_N_MS` в
...
- [x] **3. Классификатор исходов + блоки (AC3, AC5).** ⚠️ **Различитель из AC3 оказался ЛОВУШКОЙ и
      заменён.** По `beatSilenceMs` строки намерения перелёт и роковой останов НЕ различаются: судья
      трипает, как только тишина достигла N, поэтому записанная тишина всегда ≈ N — что при 29 мс,
      что при 2070. Различает ИСТОРИЯ — счёт закрытых зазоров кольца до трипа
      (`countStallsBeforeTrip`, порог 10 мс сторожа смерти; пол канала 9,73 мс лежит ПОД ним).
      Словарь закрыт ПЯТЬЮ именами: пятое (`чисто`) добавлено против лжи именования — здоровый
      прогон без трипа успех, а не пропуск. Исходное: Чистая функция над строками `fuse.jsonl`
...
- [x] **4. Сетка одной командой (AC2).** Пилот 10:45 — `N=60 · strangle → СПАСЕНО, остановов
      пережито 18/18`: арифметическое окно подтверждено сквозным прогоном. Сетка разложена по
      сценариям (Парето): удушение по 20·25·30·40·60·150 (граница преждевременных лежит у 29),
      здоровый по 10·15·20·40·60 (граница ложных лежит у пола канала 9,73). Исходное: Режим `--tune-n` в `twin-assembly.mjs` — не новый модуль:
...
- [x] **5. Прогон и сгущение.** Сгущение не понадобилось: грубая сетка легла без разброса между
      повторами, границы читаются с неё. Исходное: Сперва грубая сетка ×3 повтора; сгущать ТОЛЬКО вокруг найденной
      границы. Бюджет: ≈40 с на прогон × 5 значений × 3 сценария × 3 повтора ≈ 30 мин машинного
      времени.
- [x] **6. Итог (AC4) + отпечаток (AC6):** таблица в §Итог, границы названы числами.
- [x] **7. Развилка владельцу — НЕ ПОДНЯТА, и это ответ ПО ДАННЫМ, а не пропуск шага.** Условие
      шага («если данные скажут, что 60 не лучшая точка») не наступило: 40, 60 и 150 трипают в
      одном и том же месте, а всё, что ниже 30, покупает ранний трип ценой тройного сокращения
      запаса над джиттером. Боевое N не тронуто.
...
## Итог — ИСПОЛНЕН 2026-08-29 10:3x → 11:0x

**33 сквозных прогона на двойнике, живая карта не участвовала (I1 отпечатком до/после в каждой
сетке, строка доставки не сдвинулась).**

**Сырьё ЗАКОММИЧЕНО** (`benches/runs/` в игноре, а без строк вывод нечем перепроверить — тем же
правилом, что закоммичены фикстуры смертей): `benches/fixtures/tune-n/strangle_grid__captured.jsonl`
(18 строк) · `benches/fixtures/tune-n/healthy_grid__captured.jsonl` (15 строк). Каждая строка несёт
N, сценарий, повтор, исход, счёт остановов, тишину трипа, код возврата и путь песочницы.

### Кривая (три повтора на точку, разброса между повторами НЕТ)

| N, мс | удушение | здоровый | остановов пережито |
|---:|---|---|---|
| 10 | — | **ложно 1 · чисто 2** | — |
| 15 | — | **ложно 2 · чисто 1** | — |
| 20 | **преждевременно 3** | чисто 3 | 11 / 18 |
| 25 | **преждевременно 3** | — | 11 / 18 |
| 30 | **преждевременно 3** | — | 12 / 18 |
| 40 | **спасено 3** | чисто 3 | 18 / 18 |
| 60 | **спасено 3** | чисто 3 | 18 / 18 |
| 150 | **спасено 3** | — | 18 / 18 |

### Границы (P65-AC4)

- **Минимальное N без ложных срабатываний: 20 мс.** При 15 — двое ложных из трёх, при 10 — один
  из трёх; при 20 и выше ложных ноль. Согласуется с полом канала (max 9,73 мс) и его выбросами.
- **Минимальное N, спасающее на РОКОВОМ останове: 40 мс.** При 30 порог трипает на 13-м перелёте
  деградации (29 мс), при 20 и 25 — на 12-м (26 мс).
- **Рабочее окно, НАБЛЮДЁННОЕ: N ∈ [40 · 2070) мс.** Верх окна ставит роковой останов записи, а не
  сетка: при 150 всё ещё спасено.
- **Боевые 60 мс лежат внутри окна с запасом 1,5× снизу** — и это ТРЕТЬЕ обоснование числа,
  независимое от прежних двух.

### Что сетка изменила в понимании порога

1. **Арифметический прогноз (29 мс) оказался ОПТИМИСТИЧНЫМ ровно на дрожание канала.** Наблюдение:
   N = 30 всё ещё трипает на останове 29 мс — потому что к останову профиля прибавляется джиттер
   отправителя (p99 4,02 · max 9,73 мс). Честная нижняя граница — **29 + max джиттера ≈ 39 мс**, и
   сетка дала ровно её: граница лежит в (30 · 40].
2. **N = 40, 60 и 150 трипают в ОДНОМ И ТОМ ЖЕ месте** — на роковом останове, пережив все 18
   перелётов. Понижать порог ради «раннего спасения» имеет смысл только ниже 30, где трип приходит
   на ~3,9 с раньше, — и там же запас над джиттером схлопывается втрое. Развилку владельцу решено
   НЕ поднимать: данные подтверждают действующее число, а не спорят с ним (шаг 7 закрыт без
   интервью).
3. **Обе стороны кривой стоят на РАЗНЫХ основаниях, и это надо помнить при переносе на живую
   карту.** Сторона спасения — на реальных числах смерти (фикстура 2797 МГц) и переносится.
   Сторона ложных — на фоне двойника; она НЕ переносится напрямую, но два независимых замера
   почти совпали (пол канала 9,73 мс здесь против 9,57 мс под живым горном), что делает перенос
   куда правдоподобнее, чем план допускал.

### Побочная находка — заведена тикетом

**`bugs/67`: трип предохранителя останавливает полосу, а прогон возвращает КОД 0.** Поймано
случайно (три здоровых прогона при N = 10/15), подтверждено детерминированной пробой
`--twin-arm-n 1`. Человек в терминале видит строку остановки; ночной прогон, скрипт и сама эта
сетка получают тот же ноль, что и от чистого завершения. Вторая половина тикета: текст причины
зовёт читать `runs/death-watch/*-fuse.jsonl`, тогда как на двойнике журнал лежит в песочнице.

### Проверка

`fuse --selftest` 47 зелёных / 0 красных (шесть блоков — новые) · `twin --selftest` 18 блоков,
включая три на проброс порога и бит-в-бит дефолт (I4) · `npm run check` зелёный ·
I1 держится в обеих сетках и в пробе.

**M по-прежнему не взведён и вне этой фазы** — у входа 2 нет источника (см. §«Что ВНЕ этого плана»).
