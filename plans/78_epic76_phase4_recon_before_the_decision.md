...
- [ ] **4.5 — Хук агентской системы. 🔴 НЕ СДЕЛАН СЕГОДНЯ НАМЕРЕННО — это сама по себе развилка.**
      Хук нельзя определить механически: «агент стоит перед развилкой» — суждение, а не событие,
      которое видно оболочке. Значит варианты такие, и цена у каждого разная:

      | вариант | цена ошибки |
      |---|---|
      | напоминание на КАЖДОМ ходе | шум; через день его перестают читать (`bugs/75`) |
      | врезка в ворота сборки — есть, работает (`R9`/`R10`) | ловит ПОСЛЕ решения, а не до |
      | хук на слова-маркеры в ответе агента («выбрал», «решил», «вместо») | ложные тревоги на прозе |
      | врезка в чек-лист перед задачей (`AGENT_GUIDE.md`) | читается всегда, но это проза |

      **Разведки по этому выбору НЕ БЫЛО**, а решение меняет то, что владелец видит на каждом ходе.
      По собственному же навыку такое решать из головы нельзя. Следующий шаг — разведка о том,
      где такие напоминания живут в чужих системах и почему их снимают; затем решение.
      🔴 **Это и есть первое применение механизма к самому себе, и оно дало «стоп», а не «делаю».**
