...
- [x] **6. Ловушки полигона — ТРИ КАРТЫ по слову владельца** (2026-08-29 20:0x): «одну вещь ломает,
      вторую вещь ломает, обе вещи ломает». Раскрыто ниже, в разделе «Шаг 6».
      **Закрыт 2026-08-29 22:0x: 3 из 3, каждая краснит ровно своё.**
- [x] **7. Ворота E67-AC5** и полная батарея; `cardgen`/новый набор в `selftest:all` ВМЕСТЕ с кодом.
      **Взяты 2026-08-29 22:3x — раздел «Шаг 7» ниже.**
- [x] **8. Итог плана**: числом — N, время, покрытие, найденное.
      **Закрыт 2026-08-29 23:0x: 30/30, 1459,3 с, покрытие по шести осям — раздел «Шаг 8» ниже.** **Перепроверено 2026-09-04 (сессия 82, после починки маркера И2 — `bugs/103`): 30 of 30 (10 + 10 + 5 + 5, 1434 s of polygon time) на тех же семенах 1000…1029, тремя пакетами по десять (потолок фоновой задачи инструмента; один запуск целиком оборвался `^C` после 11 зелёных — причина не найдена).**
...
## Шаг 7 — ВОРОТА E67-AC5 (взяты 2026-08-29 22:3x)

> Дословно, `plans/67`: **«E67-AC5 — обычная карта не сдвинулась. Твин-прогон rtx5070ti без новых
> флагов — бит-в-бит»**. Это главный риск шага 6: физику карты правили в трёх местах.

**Как мерилось — не чтением диффа, а двумя прогонами одной команды на двух версиях кода.**
Состояние стенда (документ и журнал двойника живут между прогонами) снято в резерв и возвращено;
модель карты откатывалась на `4cc4153` и возвращалась `git checkout HEAD -- …`; откат назван ДО
правки.

| что сверялось | до правки | после правки | вердикт |
|---|---|---|---|
| вывод прогона `--sweep --card virtual --from 2842 --to 2812` (штампы времени, каталоги, pid нормализованы) | 110 строк | 110 строк | **совпал** |
| документ кривой: частота → напряжение → статус, БЕЗ всякой нормализации | 389 строк | 389 строк | **совпали бит-в-бит** |

**Сверка доказана КРАСНЫМ, иначе её зелёный не стоил бы ничего:** мутация края карты на +5 мВ дала
**5 разошедшихся строк** (первая — 2835 МГц: 860 мВ против 865). Прибор видит сдвиг обычной карты.

### 🔴 И находка о самой проверке, стоившая двух лишних мутаций

Первые ДВЕ мутации — обе по моей же новой правке — прошли ЗЕЛЁНЫМИ, и это едва не прочиталось как
слепота ворот. Улики говорят другое, и вывод сильнее исходного:

- мутация 1 (свойство выдачи достаётся всем картам) на полосе 2842 не двигает НИЧЕГО: потолок
  выдачи там — потолок кривой, то есть испытуемая частота, а лишние ступени лежат выше конверта и
  в обход не попадают вовсе;
- мутация 2 (граница частоты поднимается у всех карт) не двигает ничего НИ НА КАКОЙ полосе: граница
  умеет только РЕЗАТЬ (`Math.min`), а поднять выдачу выше потолка кривой способен лишь перелёт
  буста, которого у обычной карты нет.

**То есть инертность правки для обычной карты не «наблюдалась», а ДОКАЗАНА с двух сторон:**
наблюдением (два прогона совпали) и механизмом (оба новых пути закрыты полем, которого у обычной
карты нет, а продолженные ступени недостижимы ниже конверта). Урок — **EXP-0186**: мутация, прошедшая
зелёной, — это утверждение о ДОСТИЖИМОСТИ, и его надо прочитать, прежде чем чинить проверку.

**Батарея:** 40 наборов / 1647 зелёных блоков / 0 красных. Новые наборы фазы (`polygon` · `polyguard`
· `polytraps` · `shrink`) стоят в `tools/selftest-all.mjs` — проверено чтением списка, а не памятью.

---

## Шаг 8 — ИТОГ ФАЗЫ, ЧИСЛОМ (2026-08-29 23:0x)

### Полный пакет на 30 картах — первый ЧИСТЫЙ, и он же закрывает E67-AC3

```
ПОЛИГОН: карт 30 · пройдено 30 · сломано 0
ВРЕМЯ: 1459.3 с (48.6 с на карту)
ФАКТУРА ПРОЖИГОВ: собрана у 30 из 30 прогнанных карт
ЖИВЫЕ АРТЕФАКТЫ: отпечаток СОШЁЛСЯ до и после пакета
```

**Чистота соблюдена буквально:** за 24 минуты прогона не тронут НИ ОДИН `.mjs` — единственный
коммит окна (`7279f38`) касается трёх `.md`. Именно на этом сгорели два пакета сессии 63.

**Цена оказалась НИЖЕ прежней оценки:** 48,6 с на карту против 63,6 с по замеру пяти карт и 72,4 с
по замеру десяти. Прежняя оценка «≈32 минуты» переоценивала — вышло **24,3 минуты**. Разброс между
картами велик и объясним архетипом: `hot-unlucky` укладывается в 5,4–5,5 с (край близко, спуск
короткий), `drifty` доходит до 154,2 с.

### Карта покрытия — «30 карт» отличимы от «30 раз одна карта» (P71-AC5)

| ось | диапазон | на скольких картах |
|---|---|---|
| предел мощности, Вт | 250 … 350 | 30 |
| дрейф таблицы, МГц/°C | −2,723 … −0,534 | 24 |
| крутизна отказа, мВ | 2,178 … 5,731 | 30 |
| пол карты, мВ | 1083 … 1149 | 6 |
| сдвиг края, мВ/°C | 0,063 … 2,27 | 6 |
| архетипы | typical ×6 · cold-lucky ×6 · hot-unlucky ×6 · angry-governor ×6 · drifty ×6 | 30 |

### Критерии фазы

| критерий | состояние | чем закрыт |
|---|---|---|
| P71-AC1 прогон пакета | ✅ | 30 карт, 30 пройдено, 0 сломано, время числом |
| P71-AC2 сторожа судят и МОГУТ покраснеть | ✅ | подставные улики + откат починки `bugs/68` (сессия 63) + **три настоящих красных на ловушках** |
| P71-AC3 класс отказа = имя сторожа | ✅ | «улика [И3 журнал цел]: намерений без вердикта 1 (seq 2)» |
| P71-AC4 сжатие сохраняет класс | ✅ | подставной оракул, мутации (сессия 63) |
| P71-AC5 карта покрытия | ✅ | таблица выше |
| P71-AC6 обычная карта не сдвинулась | ✅ | шаг 7: 110/110 строк вывода, 389/389 строк документа |
| P71-AC7 каждая ловушка краснит РОВНО своё | ✅ | 3 из 3 |
| P71-AC8 на третьей краснеют ОБА | 🟡 | половина: порознь оба доказаны, сложение недостижимо структурно и это ЗАМЕРЕНО |
| P71-AC9 намерение записано в карте | ✅ | `card.trap` + `card.trap.blocked` |
| P71-AC10 ожидание объявлено ДО прогона | ✅ | одно опровергнуто (И3), одно заперто, оба записаны как есть |

### 🔴 Что этот пакет НЕ доказывает — граница, без которой число «30/30» опасно

Тридцать зелёных карт **не означают, что движок верен**. Они означают: на широком поле поведений
шесть сторожей честности не нашли нарушения инвариантов. Полигон по построению не смеет проверять,
верен ли НАЙДЕННЫЙ КРАЙ (риск №1 `plans/67`) — у вымысла правильного края нет. И самое важное после
этого вечера: **до ловушек этот зелёный был неотличим от зелёного полигона, который ловить не
умеет.** Теперь отличим — но ровно на два сторожа из шести (`bugs/69`, опись).

**Фаза 4 закрыта.** Открытым в неё уходит `P71-AC8` (наполовину, с названной причиной).
