...
- ✅ **2026-09-04 (сессия 82): M ВЗВОДИТСЯ НА ЖИВОМ ПУТИ** — проводка исполнена по `bugs/101` находка 3 после того, как отказ, «которого проект ни разу не наблюдал», наблюдён 04.09 (ядро висело 165 с). С поправкой плана: уставка выводится из НИЖНЕЙ частоты полосы (такт обратен частоте — архив `branchy`; `fuse.PROGRESS_TICK_REF_MHZ`), `--progress-observe` — наблюдение без трипа. Живой дым по-прежнему ждёт первого прогона при владельце; режим первого прогона — его слово, `interviews/026` Q3. Ниже — как было написано 29.08:
