...
- [x] **Ш1. Вход 3 в пробу и судью** ✅ 2026-09-08, коммит `70e4564`. — читать `mwBuf`, слать значение датаграммой (протокол уже
      однобайтный: `0x01` пульс, `0x02` прогресс → `0x03` мощность + значение). Поле в кольцо. AC1.
- [x] **Ш2. 🔴 Починить обнуление часов входа 2** ✅ 2026-09-08, коммит (см. журнал). (`fuse.mjs:1296`): ворота обязаны СТАВИТЬ НА
...
- [x] **Ш3. Прибор вывода порога из архива** ✅ 2026-09-08, коммит `ff1f31c`: порог 0,412, 7 смертей из 7, ложных 0 на 166 прожигах. — читает `runs/vmin/*.jsonl` и кольца, печатает порог
      и число ложных на архиве. AC2.
- [x] **Ш4. Репетиция `power-collapse`** ✅ 2026-09-08 — но НЕ на двойнике, а на ДВУХ ЖИВЫХ ЗАПИСЯХ
      2026-09-08 с одной частоты 3067 МГц и соседних ступеней: 935 мВ прошло, 925 мВ убило.
      Фикстуры `__fixtures__/power_collapse_3067mhz_death__captured.jsonl` и
      `power_healthy_3067mhz_935mv__captured.jsonl`. Это СИЛЬНЕЕ двойника: играется не модель
      смерти, а сама смерть, и контроль отличается от неё только напряжением. AC3.
- [x] **Ш5. Ложные — доказаны АРХИВОМ, а не полигоном.** ✏️ **ПОПРАВКА К СОБСТВЕННОМУ ПЛАНУ,
      2026-09-08:** полигон физически НЕ МОЖЕТ проверить вход 3. Он гоняет двойника, у двойника нет
      NVML, а его отправитель ударов синтетический и однобайтный — кормить вход 3 нечем.
      **Замена сильнее оригинала:** прибор Ш3 прогнал правило по **166 НАСТОЯЩИМ прожигам живой
      карты владельца** — 7 трипов, все семь на частотах с записанным полом зависания, ЛОЖНЫХ 0;
      92 прожига на ТЕХ ЖЕ частотах трипа не дали. Тридцать синтетических карт проверили бы
      ПРОВОДКУ, архив проверил КАЛИБРОВКУ, и на настоящей телеметрии. AC4 взят этим.
      🔴 **Что осталось за полигоном и сделано отдельно:** регрессия — правки предохранителя не
      сломали сквозной путь двойника.
      🔴 **Что этим НЕ доказано и записано в `GAP` сторожа:** поведение входа 3 на ЖИВОМ потоке
      (проба шлёт милливатты каждые 2 мс, архив — раз в 500 мс, разница в 250 раз).
- [x] **Ш6. Объявление ворот 5 + сторож «взведён ≠ доказан».** ✅ 2026-09-08. AC6: блок
      `@guard fuse-power-collapse` с четырьмя полями, `ON-REAL-PATH: NOT YET`. AC7:
      `tools/armed-proven-lint.mjs` — у каждого взводимого входа обязан быть блок батареи с
      меткой `[ДОКАЗЫВАЕТ --arm-X]`, утверждающий `tripped === true`. Входов 3, доказаны
      трипом 3. Вплетён девятой строкой в `npm run check`; красный доказан дважды: снять
      метку и оставить метку без утверждения о трипе.
- [x] **Ш7. ЖИВОЙ прогон ИСПОЛНЕН 2026-09-08 12:26 при владельце.** ✅
      `--sweep --from 2610 --to 2610 --max-depth 50 --dashboard --log entry3-witness`, код 0.
      Одна частота, один прожиг 10 с на ДОКАЗАННОМ 865 мВ, ступеней вниз ноль (подтверждено
      сухим прогоном ДО запуска). Карта выдала 2602 МГц, строка легла по выдаче.
      **Что увидели, и это больше, чем ждали:**
      - все ТРИ входа защиты взведены на живом пути впервые; журнал печатает состояние
        третьего из аргументов судьи;
      - кольцо несёт 13 секунд живых милливатт с такта 2 мс: 22–25 Вт простой → 230–240 Вт
        прожиг → 141 → 54 → 45 Вт конец. **Трипов 0**, спасений 0;
      - 🔴 **ложное срабатывание, названное риском заранее, НЕ выстрелило:** на конце прожига
        доля упала до 0,19 при пороге 0,412, и ворота погасили источник, потому что прожиг
        кончился;
      - **`bugs/116` получил живого свидетеля:** сводка напечатала «остановлено НАШИМ
        потолком глубины 1 · упёрлось в предел сдвига 0». До починки было бы «0 · 1»;
      - **`bugs/117` дыра 2 видна в кольце:** `worstProgressSilenceMs: null` вместо
        клипованного числа — погашенный источник больше не притворяется измеренным.
      🔴 **Чего прогон НЕ доказал и что записано в `GAP`:** вход 3 ни разу не ТРИПАЛ на живой
      карте — наблюдался только его молчаливый режим. Настоящее срабатывание увидит первый
      же край.
