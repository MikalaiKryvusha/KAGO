...
> разведка `researches/30` · **Статус:** ✅ ИСПОЛНЕН 2026-08-31, ночь; фазы 1 и 4 эпика закрыты,
...
|---|---|---|
| **P79-AC1** | Серия событий драйвера прекращает ступень | Шкала: секунд от подложенных событий до РЕШЕНИЯ · Прибор: `npm run drivervoice -- --bench-latency` (живая дорога `EventLogWatcher`) · **Порог: ≤ 5 с — ✅ ВЗЯТ: 291 · 302 · 284 мс** |
...
flowchart TD
    ...
    subgraph T2["ЯРУС 2 — реальное время, отдельный процесс (флаг --driver-voice-watch)"]
        W["наблюдатель<br/>driver-voice-watch.ps1<br/>EventLogWatcher = подписка,<br/>не опрос · 291 мс"]
        H1["рука: убить прожиг<br/>(ОБЩАЯ с предохранителем,<br/>не своя копия)"]
        H2["напряжение возвращает<br/>ОТКАТ САМОЙ СТУПЕНИ,<br/>как только spawnSync вернул"]
    end
...
- [x] **3.1. Модуль решения `automation-engine/lib/driver-voice.mjs`.** Чистая функция
...
- [x] **3.2. Блоки самопроверки на АРХИВЕ, а не на выдуманных данных.** Двадцать одна вспышка
...
- [x] **3.3. Сцепка с накопителем.** `stallLedgerVerdict` учится принимать запись с полем `kind`
...
- [x] **3.4. Ярус 1 — вердикт ступени.** `runRung` уже получает раздел сигналов от `event-logger`
...
- [x] **3.5. Наблюдатель `automation-engine/lib/driver-voice-watch.ps1`** — только ASCII в исходнике
...
- [x] **3.6. Ярус 2 — вход 3 предохранителя.** Байт `0x03` в протоколе судьи, счётчик в окне,
...
- [x] **3.7. Мутации, каждая красит ровно свой блок** (P79-AC6): порог 2 → 1 · окно 120 с → 0 ·
...
- [x] **3.8. Перепись R4b-signal** во внутренней карте (P79-AC5): строка «проголосовать не может»
      становится историей с датой и причиной; действующее правило — новое, со ссылкой на
      `researches/30`.
- [x] **3.9. Исправление числа «110 секунд»** в `bugs/76`, `bugs/79`, `plans/73` и `STATUS.md`.
      Число, которого никто не мерил как окно, не имеет права стоять в основании эпика.
- [x] **3.10. Батарея и ворота:** `npm run check` · `node tools/guard-lint.mjs` · `npm run selftest:all`
...
## ✅ ИСПОЛНЕНО (2026-08-31, ночь)

Шаги 3.1–3.10 пройдены. Разбор целиком — в `bugs/79` (раздел «СТАТУС: ЗАКРЫТ»); здесь только то,
что разошлось с планом, потому что расхождение плана с исполнением — это находка, а не мелочь.

### Шаг 3.6 исполнен ИНАЧЕ, чем написан, и вот почему

**План говорил:** вход 3 предохранителя, байт `0x03`, причина трипа `driver-cry`.
**Сделано:** отдельный наблюдатель, который убивает прожиг СВОЕЙ рукой (общей с предохранителем
функцией `makeImageKillHand`), предохранитель не тронут ни строкой.

**Причина — свойство предохранителя, которое план не учёл:** его трип ЗАВЕРШАЕТ судью, а полоса без
судьи встаёт (`stopWhen` читает код выхода судьи; прогон без предохранителя запрещён фазой 5 эпика
51). То есть голос через предохранитель означал бы ОСТАНОВКУ ПОЛОСЫ — ровно вариант C разведки, у
которого замерено две ложные остановки за семь дней прогонов. План был написан до того, как
жизненный цикл судьи был прочитан внимательно; чинить это переделкой судьи за час до живого прогона
на машине владельца — цена, несоразмерная выигрышу в один случай архива.

**Что от этого потерялось:** ничего из критериев. P79-AC1 взят той же дорогой доставки (291 мс).

### Что осталось долгом, названным вслух

- **Ярус 2 выключен по умолчанию** (`--driver-voice-watch`), `ON-REAL-PATH: NOT YET`.
- **Задержка замерена на канале «Windows PowerShell»**, а не на `nvlddmkm`: последний нельзя
  вызвать по требованию, не сломав карту. Доказана ДОРОГА доставки, а не то, что драйвер пишет в
  журнал сразу.
