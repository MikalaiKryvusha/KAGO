...
> фаза 4 — `plans/63` (✅)
> **Статус:** 🟠 шаги 1–2 ✅ (реестр 13 строк в `plans/59` · строка I3 одной дверью + сторож,
> батарея 33/1499/0). **Шаг 3 ОПРОВЕРГНУТ владельцем 2026-08-29 00:4x** («я ничего не видел.
> даже визуализатор не открылся») — показ без ОКНА вечером не является; блокер — `bugs/65`
> (окно наблюдения на твин-пути, ~1 офлайн-сессия). Шаг 4 (глаз владельца, T4) ждёт починки
> шага 3. Эпик 59 закрывается только словом владельца
...
- [x] 3. **Команда показа вечера** — 🔴 **ОПРОВЕРГНУТ ГЛАЗОМ ВЛАДЕЛЬЦА 2026-08-29 00:4x на
  первом показе:** *«я ничего не видел. даже визуализатор не открылся»*. Сценарий был собран из
  терминальных строк, а прибор владельца — ОКНО; на твин-пути оно не поднимается вовсе
  (`bugs/65`, причины E6/E11). Шаг переписывается ПОСЛЕ починки: команда показа обязана
  открывать окно наблюдения с данными двойника. Раздел «Виртуальный вечер владельца» ниже —
  устаревший, оставлен как улика дефекта сценария.
