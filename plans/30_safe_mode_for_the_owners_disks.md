...
> neighbouring agent proposed the two protection levels · **Status:** 🟡 ПОСТРОЕНО И ДОКАЗАНО ОФЛАЙН
> 2026-08-24 · **НА МАШИНЕ ВЛАДЕЛЬЦА НИ РАЗУ НЕ ВЗВОДИЛОСЬ** (см. §5) ·
...
## §5. Что построено 2026-08-24, и чем это замерено

`tools/safe-mode.mjs` — `--status` (по умолчанию, ТОЛЬКО ЧТЕНИЕ) · `--on [--offline-disks]` ·
`--off` · `--verify` · `--selftest`. Плюс гейт в старте развёртки (2.4) и разжатие при входе (2.6).

| прибор | число |
|---|---|
| `safe-mode --selftest` | **15 зелёных, 0 красных** (в батарее `selftest:all` под id `safemode`) |
| `profile --selftest` | 55 блоков, провалов 0 (+4 новых на разжатие при входе) |
| `engine --selftest` | 325 блоков (+5 на гейт) |
| `npm run check` | 53 файла, 0 падений |
| записей в карту | **0** |
| состояние машины владельца | **DISARMED — не тронута ни разу** |

### Три решения, принятые сверх плана, и почему

1. **Диски ключуются БУКВОЙ, а не номером.** План говорил «диски 0/1/2». Номера назначает Windows, и
   они едут при смене железа; инструмент с зашитыми номерами однажды зажал бы не тот диск МОЛЧА —
   худший из доступных отказов. Буквы разрешаются в номера на месте, а неразрешимая буква — отказ,
   а не догадка.
2. **Расписка защищена СТОРОЖЕМ, а не соглашением.** План говорил «класть расписку на C:». Это
   старательность. Вместо этого инструмент определяет, на каком диске лежит его СОБСТВЕННАЯ расписка,
   и **отказывается зажимать этот диск**. Правило переживает переезд репозитория; «положить на C:» —
   нет. Проверено блоком: расписка на E: → E: исключён с названной причиной.
3. **Неприменимое не считается ни за, ни против.** Отсутствующая служба FTP и диск, несущий расписку,
   не «невзведённые пункты». Считать их так — значит сделать состояние ВЕЧНО половинчатым, и гейт
   краснел бы на штатной машине. Это ловушка R12 · R13 · R17, и проект падал в неё дважды.

### Разведка 2.1 — находка, которой в плане не было

**qBittorrent пишет НЕ ТОЛЬКО на J:.** Из его конфига: `E:\Video`, `F:\Video`, `E:\Instals` — и
`D:\Soft`, `D:\Games`, `D:\Instals`, `D:\Music`. **D: ступень 2 не защищает** (там игры и файл
подкачки, §4). Значит для D: единственная защита — ступень 1, пауза клиента. Это меняет её оценку
против §1: ступень 1 не «дешёвая добавка к главному», а единственное, что стоит между торрент-клиентом
и D:.

Прочее состояние на 2026-08-24: `ftpsvc` Running (автозапуск) · qBittorrent pid 37096 · грязных битов
нет ни на одном томе · процессов, запущенных С защищаемых томов, — ноль.

### Чего ЕЩЁ НЕТ, названо честно

- **Живой взвод на машине владельца.** `--on` ни разу не запускался: он остановит FTP и закроет
  торрент-клиент, и это его слово, а не моё.
- **AC9 замерен только на E:** (замер 2026-08-23 при живом фильме). J: и F: не проверялись —
  `--verify` ровно для этого и написан, но гонять его надо при ВЗВЕДЁННОМ режиме.
- **AC7 после грязного выключения** проверяется только настоящим грязным выключением.
- **Шаг 2.5** — отдельная строка «режим остался взведён» в старте развёртки: гейт AC4 стоит и
  отказывает, но именной печати ещё нет.
- **Переживает ли `IsReadOnly` перезагрузку** — не замерено (см. §1).
