...
| 2 | **Голоса у драйвера** | у нас было **110 секунд** между первым криком `nvlddmkm` и смертью машины, и никакого права этим воспользоваться |
...
| 4 | ~~Шага от стока~~ (было НЕВЕРНО) | последний шаг перед смертью был **10 мВ вместо 5** — ваше слово: *«Шаг всегда должен отмеряться от стока»* |
...
- `nvlddmkm` 153 в 21:48:34 и 153 + 14 в 21:48:39 — **за 110 секунд до смерти**.
...
| **E73-AC2** | Шаг спуска НИКОГДА не крупнее шага зоны, вычисленной ОТ СТОКА | Шкала: ступеней, где фактический шаг > шага зоны · Прибор: сухой прогон полосы + блок · **Порог: 0. Сегодня — 1 из 1 на последнем шаге** |
| **E73-AC3** | Ошибка `nvlddmkm` во время прогона ОСТАНАВЛИВАЕТ спуск | Шкала: секунд от события в журнале Windows до остановки полосы · Прибор: подложенное событие на стенде · **Порог: ≤ 5 с. Сегодня — не останавливает никогда** |
...
flowchart TD
    ...
    subgraph FIX["ПОСЛЕ ЭПИКА"]
        ...
        D2 -->|"ФАЗА 3: голос"| STOP["полоса ВСТАЁТ<br/>≤ 5 с, карта откатывается"]
...
| **2** | **Шаг от стока** (`bugs/77`) | фаза 1 закрыта | E73-AC2 взят: 0 ступеней с шагом крупнее зоны, блок с мутацией |
| **3** | **Голос драйвера** (`bugs/79`) | фаза 2 закрыта | E73-AC3 взят: подложенное событие останавливает полосу ≤ 5 с; R4b-signal переписан |
