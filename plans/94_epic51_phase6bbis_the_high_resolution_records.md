...
- [~] **3. ГОРН, 0 ЗАПИСЕЙ. Починить такт и доказать его на всём прогоне** *(якорь: `bugs/128`
      ...
      **ОСТАЛОСЬ И ТРЕБУЕТ ГОРНА:** те же 20 секунд ПОД НАГРУЗКОЙ — гашение снято, но DPC-задержка
      от занятого драйвера этим не лечится, и её вклад ещё не измерен. Плюс сторож в батарею
      (`bugs/128` AC3): краснеет на записи, где p90 хуже объявленного вдвое.
      **P94-AC2 — ворота фазы, закрываются только замером под нагрузкой.**
