...
> **Родитель:** `bugs/76` · `plans/73` · **Статус:** 🔴 **БЛОКИРУЮЩИЙ. Фазы 1, 2 и 3 закрыты;
> E76-AC1 взят всеми тремя механизмами. Осталось: фаза 4 (М4 — разведка ДО решения, навык
> `/recon-before-decision` + хук) и фаза 5 (приёмка). Тикеты наверх УЖЕ отправлены — origin #35,
> #36, #37.**
...
> **Вовне:** ✅ **доставлено 2026-08-30** — origin **#35** (`bugs/KAIF/14`) · **#36**
> (`bugs/KAIF/15`) · **#37** (`bugs/KAIF/16`, заведён по ходу: общее правило `AUTH:` проглатывало
> исключение для багов KAIF, и тикеты стояли часами). `AUTH:` для багов KAIF больше не требуется —
> слово владельца 2026-08-30, вписано в `AGENT_GUIDE.md` → git workflow.
> **Дети:** `plans/75` (фаза 1) · `plans/77` (фаза 2) · `plans/74` (фаза 3) · `plans/78` (фаза 4).
...
| **3** | М3 — долговечность улики | фаза 2 закрыта | ✅ **ЗАКРЫТА 2026-08-31 00:3x** — `bugs/78` вылечен ВТОРЫМ СЛОЕМ (`@forensic fuse-alive`, `every-second`, `fsync`): 300 строк за 300 с, дрейф 2,32 мс, судья не замедлился. K1 упал ПЕРВЫМ, шесть контролей, судья: СВЕРЕНО С ОГОВОРКАМИ. Оговорки объявлены: `fsync` стендом недоказуем, класс «машина замёрзла» не доказан |
| **4** | **М4 — разведка и рефлексия ДО решения** | фаза 3 закрыта | ✅ **ЗАКРЫТА 2026-08-31 00:3x** — маркер `@fork`, правила `R9`/`R10` доказаны мутациями с точной адресностью, **М4 = 50−/51+**, навык `/recon-before-decision`, четыре собственные развилки размечены (три с честным `RECON: NOT YET`). Хук ОТКЛОНЁН разведкой `researches/28`, а не забыт |
...
| 4 — М4 | `plans/78` ✅ написан 2026-08-31 на закрытии фазы 3 | пишется при закрытии фазы 4 |
