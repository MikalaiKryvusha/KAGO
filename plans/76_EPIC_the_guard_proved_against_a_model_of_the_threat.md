...
> **Родитель:** `bugs/76` · `plans/73` · **Статус:** 🔴 **БЛОКИРУЮЩИЙ. Фаза 1 закрыта 2026-08-30;
> фаза 2 исполнена и ждёт одной развилки владельца; фазы 3–5 открыты.**
> **Карта не нужна ни на одной фазе.**
> 🔴 **Владелец нужен РАНЬШЕ приёмки — одним вопросом:** чем меряется «100 тестов на механизм»
> (`plans/77` §7). Две линейки дают разный ответ, и от него зависит, закрыта ли фаза 2.
> **Вовне:** развилка о линейке владельцу · `bugs/KAIF/14` и `15` в origin (ждут `AUTH:`).
...
Прибор построен (`plans/77`) и печатает долю сам. **Измеренное — ниже; линейка ждёт владельца:**

| механизм | негативных | позитивных | порог 50/50 |
|---|---|---|---|
| **М1** — против чего доказан | 30 | 12 | 🔴 недобрано 20− и 38+ |
| **М2** — видели на реальном пути | **94** | **52** | ✅ взят |
| **М3** — улика переживёт событие | 23 | 10 | 🔴 недобрано 27− и 40+ |
...
| **2** | М2 — на реальном пути | фаза 1 закрыта | 🟡 **ПОЧТИ: правило R7 стоит, R8 добавлен (`bugs/81`), долю МЕХАНИЗМА печатает прогон (`plans/77`), М2 = 94− / 52+ — порог владельца ВЗЯТ.** Не закрыта одним: линейка счёта — развилка владельца (`plans/77` §7) |
