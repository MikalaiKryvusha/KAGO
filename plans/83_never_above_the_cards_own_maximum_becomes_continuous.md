...
| **P83-AC3** | Откат снимает замок, и это доказывается НАБЛЮДЕНИЕМ ПОД НАГРУЗКОЙ, а не полем драйвера | ✏️ **Переписан 2026-09-04 (сессия 78):** прежний прибор `nvidia-smi -q -d PERFORMANCE` поля «я под замком» на драйвере 610.88 не публикует НИ В ОДНОМ разделе (сверено чтением 01.09; то же — `profile-manager.mjs` → `factoryStateVerdict`), а на отгружаемой границе 180…3090 снятие замка частотой НЕ НАБЛЮДАЕМО: карта и без замка выше 3090 не идёт. Поэтому критерий берётся УЧЕБНОЙ границей ниже естественной частоты, той же, что измерила механизм в Ш4. **Шкала:** максимум выданной частоты под прожигом, МГц. **Прибор:** `-lgc 180,1500` → `npm run stress -- --workload furnace --sustain 10` при `npm run mon -- --seconds 15 --out runs/lock-drill-on.jsonl` → `npm run profile -- --reset` → тот же прожиг с `runs/lock-drill-off.jsonl`. **Порог:** под замком ≤ 1500 (Ш4: 1492…1500, ни одной пробы выше); после сброса максимум ВЫШЕ 1500 (Ш4: без границы тот же прожиг ~2887) и не выше 3090. **Рельс S1:** пишет в карту (`-lgc`, прожиг) — только при владельце. 🔲 не взят |
...
✏️ **ПЕРЕПИСАН 2026-09-04 (сессия 78), через трое суток, а не в тот же час** — редакция в таблице
критериев выше. Три решения, каждое по замеру, а не по вкусу: **(1)** прибор — сэмплер под прожигом,
потому что это единственное, чем замок вообще наблюдался (Ш4); **(2)** граница — учебная 180…1500, а
не отгружаемая 180…3090, потому что на отгружаемой снятие замка частотой не отличимо от его наличия
(карта и так не идёт выше 3090); **(3)** критерий остаётся НЕ ВЗЯТЫМ и требует владельца у машины —
переписать мерку не значит пройти её. Числа порога — из Ш4 (1492…1500 под границей, ~2887 без неё).
