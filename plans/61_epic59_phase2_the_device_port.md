...
- [ ] 1. Прочитать тело `runStep` целиком; выписать все точки касания (опись даёт 17 адресов).
- [ ] 2. Порт: `device = null` в сигнатуре; `liveDevice()` — обёртка сегодняшних импортов;
  замена точек на `dev.*`; чистые функции остаются на статическом импорте nvapi.
- [ ] 3. Селфтест: фейковое устройство + сквозной офлайн-блок (P61-AC1) + блоки-свидетели.
- [ ] 4. Мутации P61-AC3; батарея; строка доставки; опись; STATUS; коммит.
