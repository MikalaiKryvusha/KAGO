...
### ✏️ ГИПОТЕЗА ПРОВЕРЕНА ПРАВКОЙ И ОКАЗАЛАСЬ НЕПОЛНОЙ — правка ОТКАЧЕНА, ответ уточнён

**Что предполагалось:** у частоты выше верха записанной кривой `oracle.servingVoltageMv` не находит
ни одной записи и возвращает МАКСИМУМ таблицы, отсюда «выдача выше стока».

**Проверено правкой 10:2x:** вопрос о напряжении ограничен сверху тем, что кривая предлагает
(`min(выданная, предложенная)`) — как у живого атома. **Правка ОТКАЧЕНА, и вот почему это ценнее
самой правки:**

1. **Гипотеза верна, но лечит не то.** Остановка «выдача выше стока» действительно ушла. Но вместе
   с ней **пробой перестал ловиться ВООБЩЕ**: `пропущено 0`, держателей нет, полоса закрылась целиком.
   То есть до правки дефект ловился ПОБОЧНО, чужой проверкой, а прямой проверки на стенде нет.
2. **Прямой проверки нет по причине, которую надо чинить в фазе 2:** подменный атом
   (`trap-suite.makeSweepStepFn`) **не гоняет `judgeDeliveredClock` вовсе** — у него нет сэмплера, а
   живая проверка потолка судит по пробам `clocks.gr` ПОД НАГРУЗКОЙ. Свой блок потолка у стенда
   есть, но он судит ТАБЛИЦУ (`offeredAfterMhz` против потолка), а таблица здесь как раз в порядке.
3. **И правка сломала соседа:** T7 покраснел. Красный сторож остановил работу до понимания
   (`GPU_TUNING_RAILS.md`, STOP-линия 6), и понимание оказалось важнее правки.

**Значит первый шаг фазы 2 звучит иначе и точнее:** не «атом спрашивает не о той частоте», а
**«атом стенда не выносит суждения о выданной частоте вообще»**. Пока `judgeDeliveredClock` не
проходит через стенд, ловушка T8 не может позеленеть по правильной причине.
