...
> **Статус:** 🟢 **Ш1–Ш7 ИСПОЛНЕНЫ (2026-08-31, сессии 71–73).** Открыты Ш8 (репетиция обеих фаз)
> и Ш9 (живая приёмка при владельце). Тега `DONE` нет до Ш9 ·
> **Карта:** не нужна до фазы приёмки · **Записей в GPU: 1** (замерная, §3)
>
> 🔤 **ТЕРМИНОЛОГИЯ ЭТОГО ПЛАНА ЗАМЕНЕНА 2026-08-31 ПО ЗАКАЗУ ВЛАДЕЛЬЦА.** Он трижды за пять минут
> сказал, что не понимает моего языка («что такое трип?» · «ты каким-то сленгом пользуешься» ·
> «рука 1 и рука 2 — тоже не понятно»), и заказал словарь. Действующие имена — `GLOSSARY.md`:
> **срабатывание САЗ** (было «трип») · **АВН**, аварийное выключение нагрузки (было «рука 1») ·
> **ВЗН**, возврат заводского напряжения (было «рука 2») · **АПВ** (было «перевзведение») ·
> **уставка** (был «порог N»). Текст ниже сохраняет прежние слова там, где он ЗАКРЫТ и датирован:
> оригинал не переписывают под сегодняшний словарь.
...
## 3. ✅ РАЗВИЛКА «что делать эти ~58 секунд» ЗАКРЫТА ЗАМЕРОМ — П1 **[ИИ]**

> 🔴 **ПОПРАВКА 2026-09-08, СЛОВОМ ВЛАДЕЛЬЦА:** *«решение владельца П1 — я НЕ ПОМНЮ ТАКОГО
> РЕШЕНИЯ!!!»* и *«ты сам какую-то ебанину пишешь, потом это читаешь и интерпретируешь как моё
> слово!»*. Он прав. **П1 ВЫБРАЛ АГЕНТ, НЕ ВЛАДЕЛЕЦ.** Слово владельца в этой развилке ровно одно —
> «давай как ты считаешь», то есть передача выбора. Всё остальное ниже — решение агента, и оно
> помечено **[ИИ]**. В `fuse.mjs` семнадцать дней стояло «решение владельца П1»; приписка снята.
>
> **Правило, из этого следующее и действующее дальше:** всё, что решил агент, подписывается
> **[ИИ]**. Словом владельца считается только то, что он написал в чате или в `interviews/`, и
> цитируется дословно.

**Владелец: «давай как ты считаешь»** — то есть выбор передан агенту. Агент решил не выбирать
вслепую, а мерить: 58 секунд были одним наблюдением с карты, которой только что было плохо.
...
- [x] ✅ **Ш5. ЗАЩИТА ПЕРЕЖИВАЕТ СРАБАТЫВАНИЕ И ВОЗВРАЩАЕТСЯ НА ПОСТ — ИСПОЛНЕН 2026-08-31 (сессия 73).**
      🔤 **Шаг получил ОТРАСЛЕВОЕ ИМЯ: АПВ — автоматическое повторное включение** (релейная защита;
      разведка `researches/32`, заказана владельцем: *«если есть уже понятные термины в индустрии,
      широко используемые — то и мы должны ими пользоваться»*). Отрасль дала и готовое различение,
      которое я собирался изобретать: **успешное АПВ** против **неуспешного**.
      **Что стоит:** `rearmCount` (расписка о собственном решении защиты) · ожидание расписки ВЗН
      вместо выхода · сброс состояния **явным списком девяти полей**, каждое с причиной, и отдельно
      названы три оставленных НАМЕРЕННО · граница ожидания — **наблюдение, а не таймаут**: ждём,
      пока жив процесс руки возврата (`isAliveFn`, сигнал 0 через ОТДЕЛЬНУЮ дверь от `killFn`) ·
      полоса ЖДЁТ АПВ (`stopWhen` стал асинхронным) и под невзведённой защитой не жжёт.
      **Решение принимает ОДИН узел** — защита; полоса его ЧИТАЕТ строкой протокола, а не повторяет.
      **Замер на двойнике:** репетиция смерти — **9 срабатываний, 9 успешных АПВ**, полоса прошла
      всю полосу. До правки защита умирала на первом же (31.08: закрыто 0 из 20).
- [x] 🔴 **Ш6 ОТМЕНЁН ВЛАДЕЛЬЦЕМ В ДЕНЬ СВОЕГО ИСПОЛНЕНИЯ И СНЯТ ИЗ КОДА 2026-08-31 (сессия 74).**
      Утром я провёл спасения в накопитель третьим родом `STALL_KIND.rescue`, днём владелец это
      отменил дословно (`interviews/024` = E, 17:33): *«Сколько угодно спасений разрешаю»*.
      Род и ключ из словаря источников убраны; спасения уехали в `report.rescues` и в свою строку сводки
      (P81-AC6). Разбор и ворота приёмки — `bugs/90` → «Починка». Что из шага ОСТАЛОСЬ в силе — ниже.
      🔴 **По дороге найден настоящий дефект, и найден КРАСНЫМ БЛОКОМ, а не чтением:** `out.hungBy`
      ставился НИЖЕ обеих веток спасения, а обе выходят раньше — **накопитель не мог получить от
      спасения ни одной записи, сколько бы источников в него ни вписали.** Класс `bugs/84` слово в
      слово. Поле теперь ставится там, где спасение опознаётся.
      **И ЭТА НАХОДКА ПЕРЕЖИЛА ОТМЕНУ — она и есть то, что осталось в силе.** Поле `out.hungBy`
      по-прежнему ставится там, где спасение опознаётся; без этой правки `report.rescues` не получал бы
      от спасения ни одной записи так же, как не получал накопитель, и строка сводки P81-AC6 врала бы
      нулём. Отменено было НАЗНАЧЕНИЕ канала, а не проводка к нему.
      🔴 **А риск (б) («череда спасений») закрытым НЕ считается и счётчиком не лечится:** его лечат
      причиной — `bugs/86` (срыв замка) и `bugs/92` (порча записей драйвером). Замер 9 срабатываний на
      трёх частотах остаётся верным фактом о ПОЧАСТОТНОЙ единице накопителя (`bugs/90`).
- [x] ✅ **Ш7. МУТАЦИИ — ВОСЕМЬ, КАЖДАЯ КРАСИТ СВОЁ (2026-08-31).**
      **М1** возврат на пост без расписки → набор ГИБНЕТ (код 1, сводной строки нет) ·
      **М1б** смерть руки не наблюдается → 2 · **М2** счёт принимает отказные расписки → 1 ·
      **М3** состояние не сбрасывается → 2 · **М4** защита не взводится обратно → 3 ·
      **М5** полоса не ждёт АПВ (снят `await`) → 1 ровно свой · **М6** спасения мимо накопителя → 2
      ровно свои · **М7** возврат дефекта `bugs/84` → 2.
      🔴 **М1 и М4 сперва прошли ЗЕЛЁНЫМИ, и обе оказались находками ([[EXP-0205]]):**
      **М1** — находка о МОЁМ методе: я считал только красные строки, а канон судит набор ТРЕМЯ
      чтениями (код выхода · красные строки · сводная строка на месте), и набор, умерший на
      исключении, провалов не печатает. Измеритель починен.
      **М4** — находка о ПОКРЫТИИ: снятие взведения не краснило ничего, потому что второго
      срабатывания никто не просил. Опыт достроен до ДВУХ тишин, и это суть шага: одна доказывает
      «защита не вышла», вторая — что она СНОВА СПОСОБНА СРАБОТАТЬ. Слепая защита хуже вышедшей,
...
| **P81-AC4** | Судья взведён к следующей ступени | Шкала: ступеней после спасения без взведённого судьи · **Порог: 0** · 🔴 **НАРУШЕН на живом пути 04.09 10:56 — ПЯТЬ ступеней** (seq 857…861) после выхода судьи кодом 2: `stopWhen` спрашивался раз на ЧАСТОТУ (`bugs/101` находка 2). ✅ **Починено 2026-09-04 (сессия 81):** опрос перед каждой ступенью в `sweepFrequency`, блоки «прожигов ровно 2» · «cut-short + fuse-rescue», мутация «снять опрос» → 6 вместо 2. Живого свидетеля ждёт |
| **P81-AC5** | Накопитель по-прежнему останавливает — **но по ПУЛЬСУ и КАНАЛУ ДРАЙВЕРА, не по спасениям** | **Порог: тот же — 3 за прогон, 2 у стока.** ✅ ВЗЯТ 2026-08-31 (сессия 74): оба блока накопителя не правились ни строкой и зелены |
| **P81-AC6** | Число спасений за вечер ВИДНО в сводке | **Порог: 1 строка.** Владелец назвал зависание нашим провалом — показатель обязан быть на виду. ✅ **ВЗЯТ 2026-08-31 (сессия 74):** строка `СРАБАТЫВАНИЙ АВАРИЙНОЙ ЗАЩИТЫ: …` печатается ВСЕГДА, включая ноль и «не смотрел», и проверяется КАК СТРОКА. До этого дня критерий держался на записях накопителя — то есть на том, что владелец отменил |
