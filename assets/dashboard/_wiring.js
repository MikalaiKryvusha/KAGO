...
function frame() {
  ...
  // ШАПКА НАД ЗАВЕРШЁННЫМ ПРОГОНОМ — ТРЕТЬЕ УТВЕРЖДЕНИЕ, а не то же самое (`bugs/100`, класс EXP-0078).
  // Сервер держит завершённый прогон текущим ещё FINISHED_GRACE_MS, чтобы ждавший результата его
  // увидел; всю эту минуту строка состояния и вкладка говорили «завершён», а шапка — «выполняется».
  // Один экран, два утверждения — и взгляд падает на шапку. Та же ветка, что у вкладки в `paint()`.
  head(pulse.run && pulse.run.finished === true
    ? 'Прогон завершён — окно ждёт следующего'
...
function refreshCurve(force) {
  ...
  fetch(curveUrl(qs), { cache: 'no-store' })
    ...
    .then((svg) => {
      ...
      // состоянием — самая дешёвая ложь прибора): нет прогона · прогон завершён · ступень отмечена.
      const state = (!pulse || pulse.noRun) ? 'прогона нет — документ как есть'
        : (r.finished === true ? 'прогон завершён — документ после него, ступени под тестом нет' : 'ступень под тестом отмечена');
      note.textContent = state + ' · картинка обновлена ' + p2(d.getHours()) + ':' + p2(d.getMinutes()) + ':' + p2(d.getSeconds());
    })
    .catch((e) => {
