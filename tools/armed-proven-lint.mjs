...
function selftest() {
  ...
  ok('боевой файл: вход 3 стоит в долге поля ЧЕСТНО — полевого трипа у него нет',
    readFieldDebt().has('--arm-p') && !fieldBlocks(readFileSync(FUSE, 'utf8')).has('--arm-p'),
    'вход 3 либо ушёл из долга без полевого блока, либо получил блок — проверить, настоящий ли он');
