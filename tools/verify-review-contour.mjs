...
// (KAIF-VERSION-OK: ниже названа версия, в которой ПРИШЁЛ канон судьи, — история, а не заявление о версии)
...
// `bugs/71` шаг 3 — ТЕ ЖЕ ДВЕ ВЕТКИ, НО У ВОРОТ ПОДЪЁМА, А НЕ У СТОРОЖА.
//
// Сторож судит, когда его кто-то запускает; подъём происходит каждый раз. Ровно поэтому блоки ниже
// проверяют не `checkUnreachableChoices`, а `answerabilityRefusals` — вторую, независимую точку,
// через которую документ обязан пройти прежде, чем владельца позовут к странице.
block('G12e', 'ПОДЪЁМ отказывает над документом с вариантами таблицей (bugs/71 шаг 3)', async () => {
  const doc = core.parseInterview(docWithTableVariants(), { file: 'table.md' });
  // Сначала МЕХАНИЗМ: разбор действительно не видит вариантов, иначе блок зеленел бы по чужой
  // причине (EXP-0016) — то же требование, что уже стоит в G12b.
  must(doc.questions.length === 1 && doc.questions[0].options.length === 0,
    `фикстура не воспроизводит дефект: вопросов ${doc.questions.length}, `
    + `вариантов ${doc.questions[0] ? doc.questions[0].options.length : '—'}`);
  must(doc.questions[0].answerLine !== null && doc.questions[0].answerLine !== undefined,
    'фикстура потеряла слот ответа — тогда отказ пришёл бы по СТАРОЙ причине, а не по новой');

  const refusals = core.answerabilityRefusals(doc);
  must(refusals.length === 1, `ожидался ровно один отказ, получено ${refusals.length}: ${tail(JSON.stringify(refusals), 240)}`);
  must(/разобрано 0/u.test(refusals[0].what), `отказ не про потерянные варианты: ${refusals[0].what}`);
  must(/A, B/u.test(refusals[0].what),
    `отказ не называет БУКВЫ, которые владелец должен был увидеть: ${refusals[0].what}`);
  must(/строками списка/u.test(refusals[0].fix), `отказ не говорит, ЧТО чинить: ${refusals[0].fix}`);
});

block('G12f', 'ОДНА буква в прозе подъём НЕ ломает — ворота молчат там, где работа законна (G9)', async () => {
  const oneLetter = [
    '# Интервью 907 — одна буква в прозе',
    '', '**Topic:** проверочный документ', '**Status:** 🔴 ждёт владельца', '', '---', '',
    '## Вопрос 1. Свободный вопрос без вариантов?',
    '',
    'В прошлый раз владелец выбрал вариант **A**, и с тех пор ничего не менялось.',
    '',
    '| что | сколько |',
    '|---|---|',
    '| **A** прошлый выбор | 45 мВ |',
    '',
    '**Ответ:**',
    '',
  ].join(LF);
  const doc = core.parseInterview(oneLetter, { file: 'one-letter.md' });
  must(doc.questions.length === 1 && doc.questions[0].options.length === 0,
    'фикстура не та: ожидался один вопрос без разобранных вариантов');
  must(core.answerabilityRefusals(doc).length === 0,
    'ворота отказали свободному вопросу, в теле которого одна буква в прозе — это ложная тревога '
    + 'того самого класса R17: сторож, краснеющий на состоянии, через которое работа законно '
    + 'проходит, описывает допущение, а не ловит дефект');
});
