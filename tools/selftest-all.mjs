...
const SUITES = [
  ...
  // ВОШЁЛ 2026-08-31 ВМЕСТЕ С САМИМ ИНСТРУМЕНТОМ, по той же причине, что `pulse` и `prayer`.
  // ДОКАЗАТЕЛЬСТВО ИНЕРТНОСТИ, рядом с записью: инструмент только ЧИТАЕТ файлы проб в `runs/vmin/`
  // и ничего не пишет — ни в карту, ни на диск; ветка `--selftest` вдобавок работает на фикстурах
  // в памяти, а единственный файл, который она открывает, — контрольный `pin-2145-555.jsonl`.
  { id: 'hold', npm: 'npm run hold -- --selftest', argv: ['tools/hold-vs-raise.mjs', '--selftest'],
    what: 'держалась ли частота: превышение над заказом против размера подъёма', done: /^САМОПРОВЕРКА ЗАВЕРШЕНА:/mu },
  // ВОШЁЛ ВМЕСТЕ С СОБСТВЕННЫМ КОДОМ, А НЕ «ПОТОМ» — правило самого этого списка, оплаченное
  // находкой 2026-08-23: батарея не звала четверть наборов проекта, и 122 блока не считал никто.
  // ДОКАЗАТЕЛЬСТВО ИНЕРТНОСТИ, рядом с записью: ветка `--selftest` работает на фикстурах В ПАМЯТИ
  // и не открывает НИ ОДНОГО файла — ни боевого кольца, ни временного; боевые кольца прибор читает
  // только когда ему назвали пути в командной строке.
  // ⚡ МАТРИЦА ОТКАЗОВ ПРЕДОХРАНИТЕЛЯ (`bugs/132`) — вошла вместе со своим кодом. Инертность:
  // работает на фикстурах в памяти, читает ровно один файл — исходник судьи, ради сторожа пары
  // «модель ↔ живой цикл»; ни карты, ни сокета, ни записи на диск.
  { id: 'fusematrix', npm: 'node automation-engine/lib/fuse-scenarios.mjs', argv: ['automation-engine/lib/fuse-scenarios.mjs'],
    what: 'матрица отказов: что каждый вход видит в каждом сценарии, дыры названы вслух', done: /^ИТОГ: сценариев /mu },
  { id: 'ratiofloor', npm: 'npm run ratiofloor -- --selftest', argv: ['tools/ratio-floor-from-rings.mjs', '--selftest'],
    what: 'пол ложных входа 3 по кольцам судьи: хвост после горна ≠ провал под нагрузкой (plans/94 Ш6)', done: /^ИТОГ: /mu },
  // ВОШЁЛ 2026-08-31 ВМЕСТЕ С ПЕРВОЙ СВОЕЙ САМОПРОВЕРКОЙ (`bugs/74`). До этого дня сторож, стерегущий
  // КАНОН, не имел ни одной собственной проверки, и батарея его не звала — то есть доказать его
  // правку было нечем, а сужение его области нельзя было отличить от выключения.
  // ДОКАЗАТЕЛЬСТВО ИНЕРТНОСТИ, рядом с записью: ветка `--selftest` НЕ ЧИТАЕТ дерево проекта вовсе —
  // она делает фикстуры в `$TEMP` (`mkdtempSync`) и зовёт `collectMarkdown` по этому каталогу;
  // ни один файл проекта не открывается и не пишется, боевой файл долга не трогается.
  { id: 'questions', npm: 'npm run questions -- --selftest', argv: ['tools/questions-guard.mjs', '--selftest'],
    what: 'сторож места вопросов: область по тегу DONE, доказанная КРАСНЫМ на открытом двойнике', done: /^САМОПРОВЕРКА ЗАВЕРШЕНА:/mu },
  ...
  // ВОШЁЛ 2026-08-31 ВМЕСТЕ С САМИМ КОДОМ, а не «потом»: правило, которое прекращает ступень на
  // карте владельца, не имеет права родиться вне батареи (`bugs/27` — набор, который никто не
  // гоняет, зелен до первого взгляда).
  // ДОКАЗАТЕЛЬСТВО ИНЕРТНОСТИ, рядом с записью: ветка `--selftest` зовёт ТОЛЬКО чистые функции над
  // литералами и над двумя СНЯТЫМИ фикстурами журнала Windows; наблюдатель поднимается лишь в
  // режимах `--watch` и `--bench-latency`, а его проводка проверяется ПОДСТАВНЫМИ швами
  // (`spawnFn`, `killImages`, `writeLine`) — ни PowerShell, ни `taskkill`, ни `runs/` в наборе не
  // трогаются. ⚠️ Модуль ИМПОРТИРУЕТ `fuse.mjs` ради ОДНОЙ руки (убить прожиг по именам) — своя
  // копия разошлась бы со списком образов; у `fuse.mjs` есть сторож главного модуля, при импорте
  // он ничего не запускает и порта не занимает.
  { id: 'drivervoice', npm: 'npm run drivervoice -- --selftest', argv: ['automation-engine/lib/driver-voice.mjs', '--selftest'],
    what: 'голос драйвера: порог серии и счёт по снятому архиву 179 событий', done: /^САМОПРОВЕРКА: блоков /mu },
  ...
  // Инертность: ветка `--selftest` НИ ОДНОГО файла не читает вовсе — фикстуры это строки исходника,
  // собранные тут же в памяти, и `violationsIn` чистая. Дерево проекта не открывается (`scanFiles`
  // зовётся только из боевой ветки). Вошёл 2026-09-05 вместе с линтером (`bugs/106`).
  // Инертность: сторож ЧИТАЕТ имена файлов и первые 14 строк документов; ни карты, ни журнала,
  // ни продакшен-файлов он не касается, а фикстуры набора живут в памяти (строки, не диск).
  { id: 'backlogtruth', npm: 'npm run backlogtruth -- --selftest', argv: ['tools/backlog-truth-lint.mjs', '--selftest'],
    what: 'согласие имени файла и строки статуса (bugs/25 п.4): две правды о состоянии предмета', done: /^СОГЛАСИЕ БЕКЛОГА: \d+ блоков/mu },
  { id: 'okdialect', npm: 'npm run okdialect -- --selftest', argv: ['tools/assert-dialect-lint.mjs', '--selftest'],
    what: 'диалект ok в батареях (bugs/106): блок, который не умеет краснеть', done: /^САМОПРОВЕРКА assert-dialect-lint/mu },
  ...
  // ВОШЁЛ 2026-08-30 ВМЕСТЕ СО СВОИМ КОДОМ (эпик 76 фаза 1, механизм М1). Слово владельца в ту же
  // ночь: «механизмы, стерегущие класс, должны быть покрыты тестами». Механизм, чей набор гоняется
  // только руками, — это набор, который однажды не прогонят; батарея и есть то, что делает
  // покрытие обязательным, а не добровольным.
  // ДОКАЗАТЕЛЬСТВО ИНЕРТНОСТИ: набор состоит из чистых функций над строками в памяти — ни карты,
  // ни диска (обход дерева живёт в командной части и в наборе не участвует), ни сети.
  { id: 'guardlint', npm: 'node tools/guard-lint.mjs --selftest', argv: ['tools/guard-lint.mjs', '--selftest'],
    // ⚠️ ЯРЛЫК БЕЗ ЧИСЕЛ — НАМЕРЕННО. Первая редакция несла «108 фикстур … 5 правил» и отстала в
    // тот же вечер, когда набор вырос до 171 и шести правил. Число, записанное в двух местах,
    // расходится на первой же правке; настоящее печатает сам прогон («блоков N»), и оно одно.
    what: 'декларация угроз сторожей: фикстуры по осям, позитивные и негативные', done: /^ЗЕЛЕНО:/mu },
  ...
  // ВОШЁЛ 2026-08-30 (сессия 65). ПОЧЕМУ ЕГО ЗДЕСЬ НЕ БЫЛО — НЕИЗВЕСТНО: причина нигде не
  // записана, а значит это пробел, а не решение. Набор контура держит ПЯТЬ сторожей того, что
  // видит ВЛАДЕЛЕЦ (постановка страницы · текст вызывающего · пара «страница ↔ голос» · заглушка
  // шаблона · заголовок-цитата), и до сегодня ни один из них батарея не гоняла: регрессия в них
  // прошла бы мимо единственной команды, которой этот проект меряет здоровье.
  // `countsBlocks: false` — И ЭТО ЗАМЕР, А НЕ ЛЕНЬ. Общий парсер считает зелёные строки по `OK`
  // и `✅` (GREEN_LINE), а этот набор печатает `PASS`/`FAIL` — блоков он насчитал бы НОЛЬ, а ноль
  // вместо числа в сводке хуже отсутствия. Поэтому набор отдаёт свою сводную строку дословно,
  // ровно как `check`. **Покраснеть он умеет:** проверено мутацией — падающий блок даёт код
  // возврата 1, целый код 0, а батарея читает код наравне со строками и сводкой.
  // ДОЛГ, НАЗВАННЫЙ ЗДЕСЬ: научить GREEN_LINE/RED_LINE словам `PASS`/`FAIL` — тогда набор войдёт
  // в общий счёт блоков. Правка общая для всех сорока наборов, поэтому не делается мимоходом.
  { id: 'contour', npm: 'npm run verify:contour', argv: ['tools/verify-review-contour.mjs'],
    countsBlocks: false,
    what: 'контур согласований: страница, ответы, сторожа видимого владельцу (bugs/01·04·41·66·70)',
    done: /^\d+ block\(s\), \d+ failed/mu },
