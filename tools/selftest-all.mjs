...
const SUITES = [
  ...
  // гоняет, зелен до первого взгляда). ИНЕРТНОСТЬ ТРИВИАЛЬНА И ПРОВЕРЯЕТСЯ ЧТЕНИЕМ: модуль
  // импортирует только `fs`/`path`/`url`, ни карты, ни PowerShell, ни `runs/` — он считает по двум
  // СНЯТЫМ фикстурам журнала Windows.
