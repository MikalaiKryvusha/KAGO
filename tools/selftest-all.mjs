...
const SUITES = [
  ...
  // его просто забыли.
  //
  // ПЯТЫЙ, `thermal-ladder --selftest`, ОСТАЁТСЯ СНАРУЖИ и назван здесь, чтобы его не искали заново:
  // он ЕДИНСТВЕННЫЙ из пяти импортирует `profile-manager` и `watchdog`. Импорт не равен вызову — но
  // правило требует ДОКАЗАТЕЛЬСТВА, а не отсутствия подозрений, и такого доказательства для него
  // ещё никто не снимал. Снять его — работа на один заход: показать, что путь `--selftest` не зовёт
  // ни один из импортированных писателей.
