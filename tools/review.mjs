...
/**
 * СПРАВКА — ОДИН ТЕКСТ НА ОБА ВЫХОДА (DRY). Печатался в двух местах и потому расходился: `--help`
 * вообще не доходил до справки, его съедал разбор флагов («неизвестный флаг: --help», код 2), а
 * справка без аргументов знала лишь половину флагов. Пара «справка ↔ разбор» УБРАНА, а не сторожится.
 */
function printUsage() {
  console.log('Использование: node tools/review.mjs <документ.md|папка> [флаги]');
  console.log('  ЧТО ЭТО: рендерит документ вычитки в страницу и по умолчанию ПОДНИМАЕТ СЕРВЕР,');
  console.log('           который держит вызывающего до решения владельца. Разовый рендер — --no-serve.');
  console.log('');
  console.log('  --batch            взять все ЖДУЩИЕ документы папки, а не один файл');
  console.log('  --no-serve         только отрисовать страницу и выйти, сервер не поднимать');
  console.log('  --no-open          не открывать окно браузера');
  console.log('  --no-signal        не подавать сигнал владельцу');
  console.log('  --out <файл.html>  куда положить страницу');
  console.log('  --by <имя>         от чьего имени записывается решение');
  console.log('  --timeout <сек>    сколько ждать решения');
  console.log('  --quiet-from <ЧЧ:ММ> · --quiet-to <ЧЧ:ММ>   тихие часы: сигнал молчит');
  // `bugs/66`: флаг, которого не видно в справке, для следующей сессии не существует — а этот
  // отвечает на вопрос, который владелец задал вслух дважды: «что от меня нужно?».
  console.log('  --ask "…"          ЧЕМ объяснить владельцу, зачем документ перед ним. Едет и в');
  console.log('                     постановку на странице, и в голос сигнала — источник один.');
  console.log('                     Без флага — общая формула.');
  console.log('');
  console.log('  ⚠️ БЕЗ --no-serve ЭТО СЕРВЕР: он НЕ ВЕРНЁТ управление, пока владелец не решит.');
}

function main(argv) {
  // 🔴 `--help` ОТВЕЧАЕТ И ВЫХОДИТ — ПЕРВОЙ ВЕТКОЙ, ДО РАЗБОРА ФЛАГОВ И ДО ЛЮБЫХ ПОСЛЕДСТВИЙ.
  // Владелец 08.09: *«Она не на тебя должна опираться, а на код»*. Прежде разбор флагов бросал
  // «неизвестный флаг: --help» и прибор уходил кодом 2: спрашивающий «что ты умеешь» получал отказ.
  // Порядок веток здесь — сам предохранитель, и его прогоном держит `tools/help-guard.mjs`.
