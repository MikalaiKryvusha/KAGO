...
// ---- 3в. ИКОНКА ВКЛАДКИ — микросхема KAGO вместо браузерного глобуса (`ideas/15`)
//
// ПРЯМАЯ ПРОСЬБА ВЛАДЕЛЬЦА 2026-08-30, дословно: *«у страницы визуализатора дефолтная иконка в виде
// глобуса. Нужно нашу иконку микросхемы в высоком разрешении поставить»*. В разметке иконки не было
// ВООБЩЕ — ни файла, ни `<link rel="icon">`, — поэтому браузер рисовал свою заглушку.
//
// ПОЧЕМУ ЗДЕСЬ, А НЕ В МАКЕТЕ. `SRC` — это ПРИНЯТЫЙ владельцем макет, и его вердикт вкуса есть
// канон (шапка этого файла). Иконка вкладки к принятой ВЁРСТКЕ не относится, поэтому внедряется
// сборкой — ровно тем же приёмом, что проводка и звук ниже, и по той же причине: принятый артефакт
// владельца остаётся нетронутым.
//
// ⚠️ ИДЕЯ 15 УКАЗЫВАЛА ПРАВИТЬ `assets/dashboard/_preview.html` — ЭТО НЕВЕРНО, И ПРОВЕРЕНО
// ПРОГОНОМ: `_preview.html` не источник, а ВЫВОД этой же сборки (§6 ниже). Правка там не доезжает
// до `sweep.html` вовсе. Поправка внесена в тикет.
//
// ПОЧЕМУ data:-URI, А НЕ ФАЙЛ: страницу поднимает локальный сервер прогона со своими песочными
// путями, и отдельный запрос за `/favicon.ico` — лишний маршрут, которого там может не оказаться.
//
// 🔴 ИСТОЧНИК — СОБРАННЫЙ ЛОГОТИП, А НЕ `chip-base.svg`. ЭТО ПОЧИНКА, И ОНА ОПЛАЧЕНА ГЛАЗОМ
// ВЛАДЕЛЬЦА 2026-08-31, дословно: *«икона в окне браузера неправильная, нет снежинки и огонька»*.
//
// Здесь стоял вектор `chip-base.svg`, взятый ради «высокого разрешения» из просьбы про иконку. Но
// `chip-base.svg` — это БАЗА, и его собственная шапка говорит об этом прямо: «strict graphite body
// + gold pins, NOTHING ON THE FACE». Лицо — ❄️ и 🔥 из Fluent Emoji 3D — приклеивается отдельным
// проходом ImageMagick и живёт только в СОБРАННОМ мастере (`assets/logo/README.md`). То есть во
// вкладку уехал чип без лица, и ровно то, что владелец заказывал 2026-08-14 («нужны иконки
// снежинки и пламени как в емодзиках делают»), из иконки выпало.
//
// Урок класса: «взять исходник картинки» и «взять картинку» — РАЗНЫЕ действия, когда картинка
// собирается из слоёв. Векторность была моим доводом, лицо — его заказом; довод об исполнении не
// спорит с тем, ЧТО исполняется.
//
// Берётся `kago-logo.webp` (15 КБ), а не `kago-logo.png` (154 КБ): это ТОТ ЖЕ мастер, уже принятый
// владельцем и уже показываемый НА САМОЙ СТРАНИЦЕ как логотип. Одна картинка в двух местах — не
// пара «истина ↔ зеркало», а один источник; растровый мастер 1024×1024 в base64 удвоил бы вес
// страницы ради размера, которого вкладке не нужно.
const LOGO = join('assets', 'logo', 'kago-logo.webp');
must(existsSync(LOGO), `нет собранного логотипа ${LOGO}`);
const logoBytes = readFileSync(LOGO);
// СТОРОЖ ПРОТИВ ВОЗВРАТА ЭТОГО ЖЕ ДЕФЕКТА: база и собранный мастер — разные файлы, и подстановка
// базы обратно обязана уронить сборку, а не тихо отдать владельцу чип без лица.
const CHIP_BASE = join('assets', 'logo', 'chip-base.svg');
must(!existsSync(CHIP_BASE) || !logoBytes.equals(readFileSync(CHIP_BASE)),
  'иконкой вкладки взята БАЗА чипа (chip-base.svg) — на ней нет ни снежинки, ни пламени; '
  + 'нужен СОБРАННЫЙ мастер assets/logo/kago-logo.webp');
const logoB64 = logoBytes.toString('base64');
must(s.includes('<meta charset="utf-8">'), 'в макете нет объявления кодировки, некуда ставить иконку');
s = s.replace('<meta charset="utf-8">',
  `<meta charset="utf-8">\n<link rel="icon" type="image/webp" href="data:image/webp;base64,${logoB64}">`);
