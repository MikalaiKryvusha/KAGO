...
//
// ПОЧЕМУ ВЕКТОР И data:-URI, А НЕ ФАЙЛ: страницу поднимает локальный сервер прогона со своими
// песочными путями, и отдельный запрос за `/favicon.ico` — лишний маршрут, которого там может не
// оказаться. Вектор даёт «высокое разрешение» на любом экране без набора растровых размеров.
// Копии SVG здесь НЕТ: он читается из `assets/logo/chip-base.svg` на каждой сборке (DRY — вторая
// копия картинки была бы парой «истина ↔ зеркало» на ровном месте).
const CHIP = join('assets', 'logo', 'chip-base.svg');
must(existsSync(CHIP), `нет вектора иконки ${CHIP}`);
const chipB64 = Buffer.from(readFileSync(CHIP)).toString('base64');
must(s.includes('<meta charset="utf-8">'), 'в макете нет объявления кодировки, некуда ставить иконку');
s = s.replace('<meta charset="utf-8">',
  `<meta charset="utf-8">\n<link rel="icon" type="image/svg+xml" href="data:image/svg+xml;base64,${chipB64}">`);
