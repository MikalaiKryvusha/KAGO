 ...
 * у закрытых ФАЗ, пока сам эпик открыт. Сузив до строки `**Статус:**`, получили 25; заострив
 * словарь до четырёх состояний проекта (см. ниже) — **6**. Каждое сужение замерено, а не выбрано.
 *
 ...
 * ═══════════════════════════════════════════════════════════════════════════════════════════════
 *
 * На день заведения расхождений было 25. Покраснеть на всех сразу значило бы отдать проекту красное
 ...
 *
 * **И она сократилась в тот же день: 25 → 6.** Двенадцать строк погашены приведением шапок в
 * согласие (каждой найден свидетель-коммит, `bugs/25` п.1), ещё семь оказались ЛОЖНЫМИ и умерли
 * вместе с заострением словаря. Остались шесть — все планы, которым нужна археология по строке
 * РОДИТЕЛЯ, а не согласование двух строк.
 *
 ...
 *        КРАСНЕЕТ: новым расхождением или устаревшей строкой базы. ЗАМЕР: 25 → 6 на 2026-09-07.
 */
 ...
 *
 * 🔴 СЛОВАРЬ МАРКЕРОВ — НЕ ВЫДУМАН, А СНЯТ С ПРОЕКТА, И ЭТО ВТОРАЯ ЕГО РЕДАКЦИЯ.
 *
 * Первая знала только два состояния — «открыт» и «закрыт» — и объявила расхождением ПЯТЬ тикетов,
 * которые открыты совершенно законно: `bugs/96` («🟡 ЧАСТИЧНО ПОЧИНЕН»), `bugs/98` («ПОЧИНЕН,
 * ОСТАТОК НАЗВАН»), `bugs/99` («🟡 ПОЧИНЕН И НАБЛЮДЁН… но»), `bugs/63` и `bugs/65` («🔧 МЕХАНИКА
 * ПОЧИНЕНА»). Слово «починен» в них есть, а предмет не закрыт — и правильно, что не закрыт.
 *
 * У проекта ЧЕТЫРЕ состояния, а не два, и они видны в маркерах его же документов:
 *   🔴 открыт · 🟠 в работе · **🟡 частично** · **🔧 починено, ждёт живого свидетеля** · ✅ закрыт.
 * 🟡 и 🔧 — это НЕ «закрыт»: тикет с ними живёт дальше и тега не получает. Поэтому оба стоят в
 * словаре ОТКРЫТОГО, и «частично/остаток/хвост» рядом с ними.
 *
 ...
 * шумит: он подталкивает к неверному действию.
 */
export const OPEN_WORDS = /🔴|🟠|🟡|🟢|🔧|\bopen\b|открыт|в работе|ждёт|частично|остаток|хвост|распланирован|к исполнению/iu;
export const DONE_WORDS = /✅|\bdone\b|закрыт|починен|исполнен/iu;
...
export function selfTest() {
  ...
  // 🔴 ЧЕТЫРЕ СОСТОЯНИЯ ПРОЕКТА, А НЕ ДВА — блоки на вторую редакцию словаря. Первая знала только
  // «открыт/закрыт» и объявила расхождением ПЯТЬ законно открытых тикетов; следуя ей, я пометил бы
  // DONE живую работу. Каждая строка ниже взята из НАСТОЯЩЕГО тикета, а не выдумана.
  ok('🟡 ЧАСТИЧНО ПОЧИНЕН — это ОТКРЫТ, а не закрыт (форма bugs/96)',
    judgeDoc('96_x.md', S('🟡 ЧАСТИЧНО ПОЧИНЕН 2026-09-01 — голос драйвера доехал')).kind === 'ok');
  ok('🔧 МЕХАНИКА ПОЧИНЕНА — это ОТКРЫТ: ждёт живого свидетеля (форма bugs/63 и bugs/65)',
    judgeDoc('63_x.md', S('🔧 МЕХАНИКА ПОЧИНЕНА 2026-08-30 по варианту A владельца')).kind === 'ok');
  ok('«ПОЧИНЕН, ОСТАТОК НАЗВАН» — это ОТКРЫТ: остаток есть работа (форма bugs/98)',
    judgeDoc('98_x.md', S('🔧 ПОЧИНЕН — СТИРАНИЕ УСТРАНЕНО, ОСТАТОК НАЗВАН')).kind === 'ok');
  // И обратная сторона: 🟡 в файле С ТЕГОМ — расхождение, и грубый словарь его МАСКИРОВАЛ.
  // Найдено этим самым заострением: `bugs/16_DONE` со строкой «🟡 partially fixed».
  ok('🟡 в файле с тегом DONE — расхождение (так нашёлся bugs/16, скрытый первой редакцией)',
    judgeDoc('16_DONE_x.md', S('🟡 partially fixed — первая починка опровергнута')).kind === 'done-tag-open-line');

  // 🔴 РЕГИСТР — ТРЕТЬЕ ЗАОСТРЕНИЕ, И ОНО ВСКРЫЛО ЕЩЁ ТРИ НАХОДКИ. Вторая редакция искала `OPEN`
  // заглавными и не видела `🔲 open` строчными — так `plans/17_DONE`, `18_DONE` и `85_DONE` три
  // недели врали о себе незамеченными. Словарь теперь регистронезависим, и блоки стерегут это.
  ok('строчное `open` читается как ОТКРЫТ — регистр не смеет прятать расхождение',
    judgeDoc('17_DONE_x.md', S('🔲 open · ZERO GPU WRITES for the whole phase')).kind === 'done-tag-open-line');
  ok('🟢 — это состояние ЖИВОГО предмета, а не закрытого (форма планов 10, 26, 47, 58)',
    judgeDoc('26_EPIC_x.md', S('🟢 распланирован 2026-08-23 · фаза 1 ЗАКРЫТА словом владельца')).kind === 'ok');
  ok('строчное `done` тоже читается — словарь закрытого симметричен словарю открытого',
    judgeDoc('9_x.md', S('steps 1–5 done, closed')).kind === 'no-tag-done-line');
