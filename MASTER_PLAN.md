...
печатает `npm run curve -- --progress` первой строкой (с 25.09, Ф1 эпика 101). Определение «проверен» — `ЗАКАЗ.md`
§2: проверка целого режима смесью нагрузок + неделя обычного пользования без сбоя.
**На 25.09: проверено целиком 0 из 4.** ⚠️ **С 18.09 23:01 драйвер карты — 616.92 (был 610.88), и
`Optimised` при каждом входе отвергается воротами R6 по штампу — карта стоит на заводских**
(`runs/shell/boot-apply.jsonl`, найдено 25.09). До того `Optimised` с 14.09 стоял на эксперименте №1 (кривая,
...
|---|---|
| оболочка у владельца: 4 ярлыка, трей с меню, автозапуск, откат (с 14.08) | ни одного режима на новом драйвере 616.92 (с 18.09 при входе стоит завод) |
| свой мост к NVAPI и применитель с чтением после записи и откатом | живого запуска проверки режима на карте (дым Ш8) |
| журнал упреждающей записи (пережил 19 смертей машины) | трёх режимов из четырёх |
| построитель кривой: тренд краёв и **тренд + запас по семи полосам** (25.09); редактор кривой | эталонов прожига и опоры применения на драйвере 616.92 |
...
| **прибор проверки режима** `npm run validate` (25.09, офлайн-половина доказана на записях) и метрика «режимов проверено Y/4» | |
...
| **Ф1** | одна команда проверяет режим и выносит вердикт с таблицей выгоды; кривая «тренд + запас по полосам» | самопроверки краснеют на мутациях · сухой прогон без записи · дым 60 с на стоке | `plans/102` | 🟡 офлайн-часть сделана 25.09 (сессия 102): кривая по полосам, `npm run validate`, метрика; живая часть — карточный день (эталоны и опора на драйвере 616.92, дым на стоке) |
...
| ⚖️ Optimised | эксперимент №1 был в пользовании 14.09–18.09: FPS как на прежней кривой, 250 Вт, 68–69 °C, обороты 53–58 %; **с 18.09 не применяется — новый драйвер 616.92, штамп профиля 610.88** | проверка смесью на новом драйвере · спуск запаса |
...
| 🔄 Stock Default | `npm run profile -- --reset` — полный сброс; ярлык до 25.09 сбрасывал мощность и частоту, но не сдвиги кривой (`bugs/140`, по коду); 25.09 утром починено в коде, сторож с мутацией | живой свидетель починки `bugs/140` (при владельце) |
