...
- **Четыре смерти 08.09 из четырёх — ПОСЛЕ записи кривой и ДО начала нагрузки**, одна доказана
  потактно: 10 с между записью и смертью, прожиг не стартовал (`researches/36` §1.2–1.3). То есть
  карта умирала на простое или переходе с глубоко сдвинутой кривой — ровно там, где отрасль и
  ловит нестабильность (2.2).
- Равномерный подъём всей кривой на сотни МГц при испытании одной частоты делает опасными ВСЕ
  ...
  (аудит 4, §4 К3).
- 10 из 48 записанных отказов — на глубине ≤ 25 мВ: срабатывания защиты, а не кремния; сторож физики
  стен печатает 13 противоречий из 28.
- **Верх диапазона (> 2950 МГц) дал 08.09–13.09 шесть смертей** — это частоты, которых карта под
  тяжёлой нагрузкой не посещает.
...
2. **Кривая = модель + вектор запаса по полосам.** Полос семь: ниже 2157 (тяжёлая нагрузка сюда не
   ...
   2700–2800 · 2800–2900 · 2900–2950 · выше 2950 (под тяжёлой нагрузкой не посещается; тренд с
...
4. **Проверка обязана включать простой и переходы** (3.3 + 2.2): смерти 08.09 жили именно там.
...
Одно действие владельца остаётся за ним по его же решению 28.08 (Q3 = A): **утвердить новую
