...
## 7. ADDENDUM 2026-08-23 — как закрытый словарь несёт ЧИСЛО

> **Зачем дополнение, а не новый документ.** `interviews/013` ответами Q1=A и Q2=C открыл в этом же
> словаре два новых вопроса, и один из них разведка 2026-08-22 не покрывала. Дробить знание о
> теговом облаке по двум файлам хуже, чем дописать раздел: `R14d` уже отсылает читателя сюда.
>
> **Область — ровно один вопрос:** Q2 требует пометить строку как «снятую с промахом **N** ступеней
> сетки». N — величина. Словарь ЗАКРЫТ (§2.3). Значит либо словарь перестаёт быть закрытым, либо
> число живёт не в теге. Разведка ниже решает, что из двух.

### 7.1 Отраслевой ответ: значение неограниченной мощности в метке — названный антипаттерн

Ближайшая зрелая практика — метки (labels) в системах метрик, где ровно эта развилка стоит каждый
день: метка это `ключ:значение` из контролируемого набора, и вопрос «класть ли туда величину»
отвечен прямо и давно.

Официальная рекомендация Prometheus, **дословно**:

> «Do not use labels to store dimensions with high cardinality (many different label values), such as
...
Практика вокруг неё называет и механизм отказа, и лечение: неограниченные значения меток —
**ведущая** причина взрыва мощности; «наиболее действенное вмешательство — предотвращение: проектируй
метки с ОГРАНИЧЕННЫМ набором значений»; а рекомендованное лечение неограниченного значения —
**заменить его ограниченной корзиной** (bucket), а саму величину держать вне метки
([Last9](https://last9.io/blog/how-to-manage-high-cardinality-metrics-in-prometheus/) ·
[OpenObserve](https://openobserve.ai/blog/prometheus-data-cardinality/) ·
[Dash0](https://www.dash0.com/faq/what-is-cardinality)).

**Антипаттерн, который мы едва не построили:** `origin:overshoot-4`, `origin:overshoot-6`,
`origin:overshoot-25`… Словарь перестаёт быть перечислимым — то есть теряет ровно то свойство, ради
которого §2.3 его и закрыла. Отказ по имени (`словарь ЗАКРЫТ: …`) выродился бы в список, растущий с
каждым прогоном, и код владельца, который должен «понимать по тегам, что за точка», ветвиться по нему
не смог бы.

### 7.2 Что это значит для нашей строки

Тег отвечает на вопрос **«какого рода этот факт»** — значение ограниченное, словарь остаётся
перечислимым. Величина живёт отдельно. У нас для «отдельно» есть ровно два места, и оба уже
существуют:

| куда | что это | цена |
|---|---|---|
| `provenBy` (проза) | уже есть, уже несёт подпись под измерением | человек прочитает, код — нет |
| новое поле строки | машинно-читаемо | **`ROW_KEYS` — список ТОЧНОГО совпадения** (§1.1): новое поле это правка контракта документа и миграция всех строк |

Разведка не выбирает между ними — это решение меты-плана. Она устанавливает только то, что **в теге
числа быть не должно**, и что оба места честнее третьего варианта (открыть словарь).

### 7.3 Находка, идущая против первого побуждения

Первое побуждение агента было `origin:overshot` — один тег на любой промах. Разведка его подтверждает
как ФОРМУ (ограниченное значение), но добавляет условие, которого в побуждении не было: тег обязан
называть **класс факта, а не его величину**, и потому должен ставиться ТОЛЬКО там, где промах меняет
смысл строки. Правило владельца (`GOAL.md` → «ТО ЖЕ ПРАВИЛО НА ОСИ НАПРЯЖЕНИЯ») эту границу уже
провело: **одна** ступень вверх — ПОПАДАНИЕ, обычная строка, метить нечего; **не ближайшая** — «другая
запись таблицы». Значит тег ставится на второе и не ставится на первое, иначе он пометит половину
документа и перестанет что-либо отличать.

### 7.4 Второй вопрос дополнения: чего стоит ДОБАВИТЬ значение в закрытый словарь (Q1)

Локальная разведка, а не отраслевая — считано по коду 2026-08-23:

- Словарь читают **два** модуля: `curve-store.mjs` (сам словарь, `TAG_OF_STATUS`, `closePoint`) и
  `engine.mjs`. Стенд `trap-suite.mjs` и документ `curves/measured.json` несут литералы.
- `statusFromTags` — упорядоченный список `if`, и порядок в нём ЗНАЧИМ: он решает, какой тег
  выигрывает при выводе legacy-статуса. Новый `stop:*` обязан встать в этот порядок осознанно.
- Есть блок-сторож: расхождение `CURVE_STATUS` и `TAG_OF_STATUS` краснеет. То есть новое значение
  нельзя добавить в одну сторону молча — машинерия против этого уже стоит.
- `closePoint` принимает legacy-`status`, а НЕ теги. Кумулятивный тег Q2 через этот вход не проходит
  вовсе — нужен отдельный вход, и это правка API, а не строчка в словаре.

**Вывод для меты-плана:** Q1 дёшев (значение + пара в двух картах + место в порядке), Q2 дороже
(вход в `closePoint` + решение о месте для N). Считать их «одной правкой словаря» было упрощением —
общий у них только словарь, а цена разная.

## 8. Источники дополнения

- [Metric and label naming · Prometheus](https://prometheus.io/docs/practices/naming/) — первоисточник, цитата дословно в §7.1
...
- [The Prometheus Cardinality Bomb · OpenObserve](https://openobserve.ai/blog/prometheus-data-cardinality/)
- [What Is Cardinality? · Dash0](https://www.dash0.com/faq/what-is-cardinality)
